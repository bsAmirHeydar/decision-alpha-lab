...
def test_reference_handoff_denies_execution(repo_root):
 h=json.loads((repo_root/'src/engine/legacy/acl_os_reference/fixtures/acl_07/reference_validation/handoff/acl08_handoff.json').read_text()); assert not h['live_order_submission_allowed'] and not h['capital_activation_allowed']
def test_reference_diagnostic_not_promoted(repo_root):
 r=json.loads((repo_root/'src/engine/legacy/acl_os_reference/fixtures/acl_07/reference_validation/reports/diagnostic_isolation_report.json').read_text()); assert r['passed'] and not r['diagnostic_promoted']
