...
import yaml
from tools.strategy_factory.acl_os.acl_03.policies import SCHEMA_ROOT,POLICY_ROOT
...
def test_cli_help():
    p=subprocess.run([sys.executable,"-m","tools.strategy_factory.acl_os.acl_03.cli","--help"],cwd=REPO_ROOT,text=True,capture_output=True);assert p.returncode==0;assert "compile" in p.stdout
