...
def test_poisoned_input_rejected(acl08_root,permit,memory_policy,planner_policy,tmp_path):
 d=tmp_path/'acl08'; shutil.copytree(acl08_root,d); p=next((d/'experience/records').glob('*.json')); x=json.loads(p.read_text()); x['bounded_research_questions'].append('ACTIVATE_CAPITAL'); from src.engine.tooling.strategy_factory.acl_os.acl_08.canonical import digest_object; x['experience_digest']=digest_object({k:v for k,v in x.items() if k!='experience_digest'}); p.write_text(json.dumps(x));
