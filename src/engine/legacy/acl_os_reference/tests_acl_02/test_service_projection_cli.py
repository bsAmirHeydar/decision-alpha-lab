...
from tools.strategy_factory.acl_os.acl_02.projection import render_readiness
...
def test_cli_questions():
 p=subprocess.run([sys.executable,"-m","tools.strategy_factory.acl_os.acl_02.cli","questions","CTX_TEST"],text=True,capture_output=True);assert p.returncode==0;assert "IDENTITY_001" in p.stdout
def test_cli_evaluate(valid_root,valid_permit,tmp_path):
 permit=tmp_path/"permit.json";permit.write_text(json.dumps(valid_permit));p=subprocess.run([sys.executable,"-m","tools.strategy_factory.acl_os.acl_02.cli","evaluate",str(valid_root),"--permit",str(permit)],text=True,capture_output=True);assert p.returncode==0
