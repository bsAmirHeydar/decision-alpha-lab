...
import json,sys
root=Path(sys.argv[1] if len(sys.argv)>1 else '.').resolve();sys.path.insert(0,str(root/'src/engine/packages'))
...
out=root/'tests/fixtures/legacy/strategy_factory/v3/uce_i06_golden_dataset_vectors.json';out.parent.mkdir(parents=True,exist_ok=True);out.write_text(json.dumps(payload,indent=2,sort_keys=True,default=str)+'\n',encoding='utf-8');print(out)
