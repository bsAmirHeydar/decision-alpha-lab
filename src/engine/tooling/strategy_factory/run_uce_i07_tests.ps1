...
python (Join-Path $root "src\engine\tooling\strategy_factory\check_uce_i07_boundaries.py") $root
python (Join-Path $root "src\engine\tooling\strategy_factory\check_uce_i07_mql5_static.py") $root
python (Join-Path $root "src\engine\tooling\strategy_factory\validate_uce_i07_delivery.py") $root
python -m compileall -q (Join-Path $root "src\engine\packages\strategy_factory_trainers_v3")
python -m strategy_factory_trainers_v3.cli conformance
python (Join-Path $root "src\engine\tooling\strategy_factory\generate_uce_i07_vectors.py") $root --verify-only
