def validate(value,schema,path='$'):
 ...
 if t=='object':
  assert isinstance(value,dict),f'{path}: expected object';required=set(schema.get('required',[]));assert required<=set(value),f'{path}: missing {sorted(required-set(value))}'
