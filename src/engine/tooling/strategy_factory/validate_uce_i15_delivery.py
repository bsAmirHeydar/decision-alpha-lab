...
import json,hashlib
ROOT=find_repository_root(__file__);errors=[];index=ROOT/'releases/history/ucee/indexes/UCEE_I15_FILE_INDEX.txt'
...
if len(list((ROOT/'src/engine/packages/strategy_factory_tournament_v3').glob('*.py')))!=22:errors.append('expected 22 Python modules')
if len(list((ROOT/'schemas/legacy/strategy_factory/v3').glob('tournament_*.schema.json')))!=25:errors.append('expected 25 tournament schemas')
if len(list((ROOT/'docs/history/systems/ucee/implementation_program/phase_deliveries/uce_i15').glob('*.md')))!=43:errors.append('expected 43 delivery documents')
if len(list((ROOT/'docs/history/obsidian/deep/01_concepts').glob('UCE-I15_*.md')))!=12:errors.append('expected 12 atomic concepts')
