...
def main()->int:
    parser=argparse.ArgumentParser();parser.add_argument("root",nargs="?",default=".");args=parser.parse_args();root=Path(args.root).resolve();sys.path.insert(0,str(root/"src/engine/packages"))
    ...
    out=root/"examples/legacy/strategy_factory/uce_i02";out.mkdir(parents=True,exist_ok=True)
