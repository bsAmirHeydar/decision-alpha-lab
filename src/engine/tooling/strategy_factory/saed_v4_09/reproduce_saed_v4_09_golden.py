...
cube=load('releases/history/strategy_factory/artifacts/saed_v4_08/GOLDEN_OUTCOME_CUBE.JSON');handoff=load('releases/history/strategy_factory/artifacts/saed_v4_08/V4_08_TO_V4_09_HANDOFF.JSON');profile=ExecutionTwinProfile.from_mapping(load('examples/legacy/strategy_factory/saed_v4_09/execution_twin_profile.json'))
twin=build_execution_twin(cube,handoff,profile);out=ROOT/'releases/history/strategy_factory/artifacts/saed_v4_09';out.mkdir(parents=True,exist_ok=True)
...
vectors=load('tests/fixtures/legacy/strategy_factory/saed_v4_09/SAED_V4_09_CONFORMANCE_VECTORS.json')
write_json(out/'CONFORMANCE_RESULTS.JSON',run_vectors(vectors,cube,handoff,load('examples/legacy/strategy_factory/saed_v4_09/execution_twin_profile.json')))
