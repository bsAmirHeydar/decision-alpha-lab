...
def _norm(v: Any) -> Any:
    ...
    if isinstance(v, float):
        if not math.isfinite(v): raise ValueError('ACL11_NON_FINITE_NUMBER')
        return int(v) if v.is_integer() else float(format(v,'.15g'))
    return v
def canonical_bytes(v: Any) -> bytes:
    return json.dumps(_norm(v),sort_keys=True,separators=(',',':'),ensure_ascii=False).encode('utf-8')
...
def stable_id(prefix: str,*parts: str,length: int=24) -> str:
    return f"{prefix}_{hashlib.sha256('|'.join(parts).encode()).hexdigest()[:length].upper()}"
...
def verify_embedded_digest(doc: dict[str,Any],field: str) -> bool:
    value=doc.get(field)
    return isinstance(value,str) and DIGEST_RE.fullmatch(value) is not None and value==digest_object({k:v for k,v in doc.items() if k!=field})
