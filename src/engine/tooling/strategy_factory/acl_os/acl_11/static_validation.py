...
def validate_registry(root: Path) -> dict:
    errors=[]; schemas=list((root/'registry/history/acl/acl_11/schemas/v1').glob('*.json')); policies=list((root/'registry/history/acl/acl_11/policies/v1').glob('*.json'))
    for p in schemas:
    ...
    hits=[]
    for p in (root/'mql5/legacy/strategy_factory_lab/Include/AlphaLab/ACL_OS/ACL11').glob('*'):
        if p.is_file():
