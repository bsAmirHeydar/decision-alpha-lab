...
def run_qa(repo_root:Path) -> dict:
    reference=repo_root/'src/engine/legacy/acl_os_reference/fixtures/acl_10/reference_promotion'; static=validate_registry_files(); replay=verify_generated_root(reference); delivery=validate_delivery(repo_root)
    proc=subprocess.run([sys.executable,'-m','compileall','-q',str(repo_root/'src/engine/tooling/strategy_factory/acl_os/acl_10')],capture_output=True,text=True)
