...
def build_event_ledger(run_id:str,occurred_at:str,items:list[tuple[str,dict]])->dict:
    ...
    for seq,(etype,payload) in enumerate(items,1):
        body={'schema_version':'1.0.0','event_id':stable_id('SECEVT',run_id,str(seq),etype),'sequence':seq,'event_type':etype,'occurred_at':occurred_at,'previous_event_digest':prev,'payload':payload,'production_key_authority':False,'runtime_activation_authority':False,'live_order_authority':False,'capital_authority':False}
        event={**body,'event_digest':digest_object(body)}; events.append(event); prev=event['event_digest']
    return with_digest({'schema_version':'1.0.0','security_hardening_run_id':run_id,'event_count':len(events),'events':events,'terminal_event_digest':prev},'ledger_digest')
def verify_event_ledger(doc:dict)->bool:
    ...
    for i,e in enumerate(doc.get('events',[]),1):
        ...
        if any(e.get(k) for k in ['production_key_authority','runtime_activation_authority','live_order_authority','capital_authority']): return False
        prev=e['event_digest']
