...
def validate_authority(permit:dict[str,Any],handoff:dict[str,Any])->dict[str,Any]:
    ...
    if not verify_embedded_digest(permit,'permit_digest'): raise AuthorityError('invalid permit digest')
    checks={
      'phase':permit['phase']=='ACL-08',
      'action':permit['action']==REQUIRED_ACTION,
      'validation_bound':permit['validation_id']==handoff['validation_id'],
