...
def build_event_ledger(report_id:str,events:list[tuple[str,dict[str,Any]]],at:str)->dict[str,Any]:
    rows=[]; prev='GENESIS'
    for i,(kind,payload) in enumerate(events,1):
        body={'schema_version':'1.0.0','event_id':stable_id('REPEVT',report_id,str(i),kind),'sequence':i,'event_type':kind,'occurred_at':at,'previous_event_digest':prev,'payload':payload,'live_order_submission_allowed':False,'capital_activation_allowed':False}
        row=with_digest(body,'event_digest'); rows.append(row); prev=row['event_digest']
    return with_digest({'schema_version':'1.0.0','report_id':report_id,'event_count':len(rows),'events':rows,'terminal_event_digest':prev},'ledger_digest')
