...
def run(root:Path)->dict:
    commands=[[sys.executable,'-m','compileall','-q',str(root/'src/engine/tooling/strategy_factory/acl_os/acl_14')],[sys.executable,'-m','pytest','-q',str(root/'src/engine/legacy/acl_os_reference/tests_acl_14')],[sys.executable,'-m','pytest','-q',str(root/'src/engine/legacy/acl_os_reference/tests_acl_13')]]
    results=[]
    for cmd in commands:
