...
def build(repo_root:Path=REPO_ROOT)->RepositoryRegistry:
    policies=PolicyBundle.load(repo_root/'registry/history/acl/acl_01/policies/v1')
    ...
    for path in sorted((repo_root/'registry/history/acl/acl_01/schemas/v1').glob('*.schema.json')):
...
def main(argv=None)->int:
    import argparse
    p=argparse.ArgumentParser(); p.add_argument('--output',type=Path,default=REPO_ROOT/'registry/history/acl/acl_01/reference/bootstrap_registry.json'); p.add_argument('--repo-root',type=Path,default=REPO_ROOT); args=p.parse_args(argv)
