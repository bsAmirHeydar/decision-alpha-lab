...
def run_qa(repo_root:Path)->dict:
    reference=repo_root/'src/engine/legacy/acl_os_reference/fixtures/acl_07/reference_validation'
    static=validate_registry_files()
    replay=verify_generated_root(reference)
    delivery=validate_delivery(repo_root)
    compile_proc=subprocess.run([sys.executable,'-m','compileall','-q',str(repo_root/'src/engine/tooling/strategy_factory/acl_os/acl_07')],capture_output=True,text=True)
    checks={
