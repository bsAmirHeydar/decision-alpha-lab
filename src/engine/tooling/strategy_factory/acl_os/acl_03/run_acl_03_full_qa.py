...
def main()->int:
    p=subprocess.run([sys.executable,"-m","pytest","-q","src/engine/legacy/acl_os_reference/tests_acl_03"],cwd=REPO_ROOT,text=True,capture_output=True)
    ...
    dump_json(REPO_ROOT/"releases/history/acl_os/reports/ACL_OS_03_QA_REPORT.json",out);print(json.dumps(out,indent=2,sort_keys=True));return 0 if out["passed"] else 1
