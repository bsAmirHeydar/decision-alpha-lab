...
def run() -> dict:
    ...
    commands=[
      _command("acl05_pytest",[sys.executable,"-m","pytest","-q","src/engine/legacy/acl_os_reference/tests_acl_05"]),
      _command("acl04_regression",[sys.executable,"-m","pytest","-q","src/engine/legacy/acl_os_reference/tests_acl_04"]),
      _command("acl05_compileall",[sys.executable,"-m","compileall","-q","src/engine/tooling/strategy_factory/acl_os/acl_05"]),
    ]
    replay=verify_frozen_batch(REPO_ROOT/"src/engine/legacy/acl_os_reference/fixtures/acl_05/reference_batch")
