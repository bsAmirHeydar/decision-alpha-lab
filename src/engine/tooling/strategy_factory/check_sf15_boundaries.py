...
experts=list((root/'mql5/Experts/StrategyFactory').glob('SF15_*.mq5'))+list((root/'mql5/Tests/Experts/StrategyFactory').glob('SF15_*.mq5'))
...
for p in (root/'src/engine/packages/strategy_factory_inference').glob('*.py'):
    text=p.read_text(encoding='utf-8')
