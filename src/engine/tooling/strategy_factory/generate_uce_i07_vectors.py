...
def main():
 p=argparse.ArgumentParser();p.add_argument('root',nargs='?',default='.');p.add_argument('--verify-only',action='store_true');a=p.parse_args();root=Path(a.root).resolve();sys.path.insert(0,str(root/'src/engine/packages'))
 ...
 target=root/'tests/fixtures/legacy/strategy_factory/v3/uce_i07_trainer_conformance_vectors.json';payload=json.dumps(result,indent=2,sort_keys=True,default=default)+"\n"
 rtarget=root/'tests/fixtures/legacy/strategy_factory/v3/uce_i07_registry_snapshot.json';rpayload=json.dumps(registry,indent=2,sort_keys=True,default=default)+"\n"
 if a.verify_only:
  bad=[]
