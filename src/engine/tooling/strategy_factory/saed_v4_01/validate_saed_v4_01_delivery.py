...
ROOT=find_repository_root(__file__);idx=ROOT/'releases/history/saed/indexes/SAED_V4_01_FILE_INDEX.txt';ledger=ROOT/'releases/history/saed/hashes/SAED_V4_01_FILE_HASHES.sha256';errors=[];files=[x.strip() for x in idx.read_text().splitlines() if x.strip()] if idx.exists() else []
