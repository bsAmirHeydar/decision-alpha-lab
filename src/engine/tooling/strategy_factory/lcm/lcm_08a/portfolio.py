...
def _granularity(path: str, identity: dict) -> str:
    low = path.lower()
    name = Path(path).stem
    if identity.get("protected_platform_asset") or "/contexts/" in low and "contexts/legacy/strategy_factory/authored/" in low:
    ...
    if low.endswith("/experiment.py") or (low.startswith("contexts/legacy/lab_experiments/") and Path(path).name == "experiment.py"):
