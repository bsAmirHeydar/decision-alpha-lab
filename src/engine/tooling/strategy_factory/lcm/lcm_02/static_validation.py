...
def validate(repo_root: Path):
    base=repo_root/'registry/legacy_context_migration/lcm_02'
    json_count=0
    for p in sorted(base.rglob('*.json')):
        json.loads(p.read_text(encoding='utf-8')); json_count+=1
    required=[repo_root/'docs/alpha_lab_master_architecture/context_lifecycle_os/17_LEGACY_MIGRATION_PROGRAM/11_PHASE_DELIVERIES/LCM_02/00_MOC.md',repo_root/'docs/alpha_lab_master_architecture/context_lifecycle_os/17_LEGACY_MIGRATION_PROGRAM/05_PHASES/LCM_02_CLASSIFICATION_OWNERSHIP_AND_AUTHORITY.md']
