...
def main(argv=None) -> int:
    ...
    cmd.add_argument("--dual-run-root", required=True)
    ...
    if args.command == "verify-patch":
        ...
        errors = verify_package(Path(args.dual_run_root))
        ...
        errors = run_qa(Path(args.dual_run_root), Path(args.schema_root), Path(args.module_root))
    if errors:
        print("\n".join(errors))
        return 1
    print("PASS")
