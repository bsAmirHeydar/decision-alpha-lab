...
def latest_characterization(repo):
    roots=sorted((repo/'registry/history/lcm/characterizations').glob('CHARACTERIZATION_*'))
...
def load(repo):
    ...
    ident_root=repo/'registry/history/lcm/identities'/handoff['identity_run_id'];class_root=repo/'registry/history/lcm/classifications'/read_json(ident_root/'input/lcm02_binding.json')['classification_id'];survey_id=read_json(class_root/'reports/classification_summary.json')['survey_id'];survey_root=repo/'registry/history/lcm/surveys'/survey_id
