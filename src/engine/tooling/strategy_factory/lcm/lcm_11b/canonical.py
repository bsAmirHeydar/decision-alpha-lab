...
def sha256_text(value:str)->str:return "sha256:"+hashlib.sha256(value.encode("utf-8")).hexdigest()
...
def stable_id(prefix:str,*parts:object,length:int=32)->str:
    raw="|".join(str(x) for x in parts);return f"{prefix}_{hashlib.sha256(raw.encode()).hexdigest()[:length].upper()}"
def token(value:str,limit:int=32)->str:
    cleaned=re.sub(r"[^A-Za-z0-9]+","_",value).strip("_").upper();return (cleaned or "UNKNOWN")[:limit]
