...
def main(argv=None) -> int:
    ...
    cmd.add_argument("--reconciliation-root", required=True)
    ...
    if args.command == "verify-patch":
        ...
        errors = verify_package(Path(args.reconciliation_root))
    elif args.command == "verify-installation":
        errors = verify_package(Path(args.reconciliation_root))
        index = [line.strip() for line in Path(args.patch_index).read_text(encoding="utf-8").splitlines() if line.strip()]
        errors.extend(f"INDEX_MISSING:{relative}" for relative in index if not (Path(args.repo_root) / relative).is_file())
    elif args.command == "qa":
        errors = run_qa(Path(args.reconciliation_root), Path(args.schema_root), Path(args.module_root))
    if errors:
        print("\n".join(errors))
        return 1
    print("PASS")
