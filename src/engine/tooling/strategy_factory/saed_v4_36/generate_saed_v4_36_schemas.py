...
ROOT=find_repository_root(__file__);ART=ROOT/'releases/history/strategy_factory/artifacts/saed_v4_36';SCH=ROOT/'schemas/legacy/strategy_factory/saed_v4_36'
def schema_of(v):
...
def main():
    ...
    for p in sorted(ART.glob('GOLDEN_*.JSON')):
        data=json.loads(p.read_text());s={'$schema':'https://json-schema.org/draft/2020-12/schema','$id':f'https://alpha-lab.local/saed/v4-36/{p.stem.lower()}.schema.json','title':p.stem,'description':'Closed deterministic SAED V4-36 research-memory and active-planner reference artifact schema.',**schema_of(data)}
