...
m=json.loads(manifest_path.read_text(encoding='utf-8'));q=json.loads(qa_path.read_text(encoding='utf-8'));s=json.loads((ROOT/'releases/history/strategy_factory/program/status/SAED_V4_22.json').read_text(encoding='utf-8'))
...
c=json.loads((ROOT/'releases/history/strategy_factory/artifacts/saed_v4_22/GOLDEN_GENERATIVE_STRESS_CERTIFICATE.JSON').read_text(encoding='utf-8'))
...
h=json.loads((ROOT/'releases/history/strategy_factory/artifacts/saed_v4_22/V4_22_TO_V4_23_HANDOFF.JSON').read_text(encoding='utf-8'))
assert h['next_phase']=='SAED_V4_23' and all(h['entry_gates'].values()) and h['research_only'] and not any(h['authority'].values())
b=json.loads((ROOT/'releases/history/strategy_factory/artifacts/saed_v4_22/GOLDEN_BUDGET_SNAPSHOT.JSON').read_text(encoding='utf-8'))
assert b['hidden_evaluation_queries']==0 and b['protected_evidence_exposures']==0
f=json.loads((ROOT/'releases/history/strategy_factory/artifacts/saed_v4_22/GOLDEN_FIDELITY_REPORT.JSON').read_text(encoding='utf-8'));i=json.loads((ROOT/'releases/history/strategy_factory/artifacts/saed_v4_22/GOLDEN_INVARIANT_REPORT.JSON').read_text(encoding='utf-8'));e=json.loads((ROOT/'releases/history/strategy_factory/artifacts/saed_v4_22/GOLDEN_EXPLOITABILITY_REPORT.JSON').read_text(encoding='utf-8'))
