...
def verify_chain(records:list[dict[str,Any]],domain:str)->dict[str,Any]:
    ...
    for expected,record in enumerate(records,1):
        if record.get("sequence")!=expected or record.get("previous_hash")!=previous or record.get("chain")!=domain:
            raise IntegrityError("chain ordering or linkage failure")
        candidate=dict(record); observed=candidate.pop("entry_hash",None)
        if observed!=content_hash(candidate): raise IntegrityError("chain entry hash failure")
        previous=observed
    return {"verified":True,"entry_count":len(records),"terminal_hash":previous,"chain":domain}
