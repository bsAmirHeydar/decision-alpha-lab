...
// - The detector remains the single source of truth for Start, Leg1, Waist, Leg2, internal 1/2 and confirmation.
// - The chart renderer draws ONLY the requested visual grammar:
//     1) Start -> Leg1 as a straight line
//     2) Leg1 -> Waist -> Leg2 as a smooth curved correction/continuation path
//     3) F1 label plus internal 1/2 NUMBERS ONLY
// - It does NOT draw lines/arcs from Leg2 into internal 1/2 or final confirmation.
...
void M0007_DrawF1Path(const M0007_F1Event &e, const string p, const color clr)
{
   ...
   // Part 1: true origin -> top/bottom of Leg1. This is the only straight F1 leg line.
   M0007_DrawTrendRaw(p + "L0", e.Start.time, e.Start.price, e.H1.time, e.H1.price, clr, 2, STYLE_SOLID);

   // Part 2a: Leg1 -> Waist/Correction. Smooth curve, not internal 1/2.
   ...
   // Part 2b: Waist/Correction -> Leg2. Smooth curve completes the visible F1 body.
                        ...
                        const bool show_f1_label)
{
   ...
   if(show_f1_label)
   {
      // Label can move to final confirmation only as a label anchor; no extra line is drawn there.
      ...
      M0007_DrawTextRaw(p + "F", anchor_t, f1_p, "F1", clr, 16,
                        bullish ? ANCHOR_LEFT_LOWER : ANCHOR_LEFT_UPPER);
   }

   // Internal 1/2 are numbers only. No lines are drawn between Leg2, 1, 2, or confirmation.
