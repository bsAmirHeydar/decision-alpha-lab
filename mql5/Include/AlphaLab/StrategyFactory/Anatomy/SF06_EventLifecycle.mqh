...
string SF06_LifecycleCanonical(const SF06_LifecycleRecord &v){return v.schema+"|"+v.aggregate_id+"|"+LongToString(v.sequence)+"|"+IntegerToString((int)v.from_state)+"|"+IntegerToString((int)v.to_state)+"|"+LongToString(v.transition_time.utc_epoch_milliseconds)+"|"+v.reason_code+"|"+v.source_hash+"|"+v.previous_record_hash;}
