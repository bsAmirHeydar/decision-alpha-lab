...
string SF06_ObservationCanonical(const SF06_AnatomyObservation &v){return v.schema+"|"+v.plugin_id+"|"+v.plugin_version+"|"+v.symbol+"|"+IntegerToString(v.timeframe_seconds)+"|"+IntegerToString((int)v.kind)+"|"+LongToString(v.occurred_at.utc_epoch_milliseconds)+"|"+LongToString(v.known_at.utc_epoch_milliseconds)+"|"+SF01_CanonicalDouble(v.reference_price)+"|"+SF01_CanonicalDouble(v.extreme_price)+"|"+SF01_CanonicalDouble(v.close_price)+"|"+v.source_bar_id+"|"+v.source_hash+"|"+v.market_event_cluster_id;}
