...
public:
   ...
   virtual SF02_ServiceHealth Health(const long now)const{SF02_ServiceHealth h;h.service_id=ServiceId();h.service_kind=ServiceKind();h.status=(m_ready&&!m_sealed)?SF02_HEALTH_HEALTHY:(m_sealed?SF02_HEALTH_DEGRADED:SF02_HEALTH_UNHEALTHY);h.detail="records="+LongToString(m_telemetry.records_written)+",sealed="+(m_sealed?"true":"false");h.observed_at_utc_msc=now;return h;}
