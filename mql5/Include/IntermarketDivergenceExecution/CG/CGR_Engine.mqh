#ifndef __CGR_ENGINE_MQH__
#define __CGR_ENGINE_MQH__

#include <IntermarketDivergenceExecution/CG/CGR_Display.mqh>
...
public:
   void Init(SCGTTimeConfig &time_config,SCGRReferenceConfig &ref_config,bool &enabled[],const bool show_panel,const bool print_on_new_minute)
   {
      m_time_config=time_config;
      m_ref_config=ref_config;
   ...
   bool Pulse()
   {
      ...
      SCGRGroupReferenceState states[];
      SCGRReferencePair flat_refs[];
      int group_start[];
      int group_count[];

      ArrayResize(states,0);
      ArrayResize(flat_refs,0);
      ...
      for(int i=0;i<CGT_GROUP_COUNT;i++)
      {
         ...
         SCGRGroupReferenceState state;
         int ref_count=m_reference_field.BuildReferencesForGroup(time_snapshot,cycle,refs,state);
         ...
         int start=ArraySize(flat_refs);
         group_start[g]=start;
         group_count[g]=ref_count;
         if(ref_count>0)
         {
            ArrayResize(flat_refs,start+ref_count);
            for(int r=0;r<ref_count;r++)
               flat_refs[start+r]=refs[r];
         }
      }

      if(m_show_panel)
         Comment(m_display.BuildPanel(time_snapshot,m_ref_config,states,flat_refs,group_start,group_count,m_reference_field));
