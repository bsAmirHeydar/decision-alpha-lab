#ifndef __CGD_ENGINE_MQH__
#define __CGD_ENGINE_MQH__

#include <IntermarketDivergenceExecution/CG/CGD_Display.mqh>
...
private:
   ...
   SCGDDivergenceConfig m_divergence_config;
   CCGT_TimeAnatomy   m_time;
   CCGD_DivergenceField m_divergence_field;
   CCGD_Display       m_display;
   bool               m_show_panel;
...
public:
   void Init(SCGTTimeConfig &time_config,SCGDDivergenceConfig &divergence_config,bool &enabled[],const bool show_panel,const bool print_on_new_minute)
   {
      m_time_config=time_config;
      m_divergence_config=divergence_config;
      ...
      m_divergence_field.Configure(m_divergence_config);
   }
   ...
   bool Pulse()
   {
      ...
      SCGDGroupDivergenceState states[];
      SCGDDivergenceCandidate flat_candidates[];
      int group_start[];
      int group_count[];
      ArrayResize(states,0);
      ArrayResize(flat_candidates,0);
      ...
      for(int i=0;i<CGT_GROUP_COUNT;i++)
      {
         ...
         SCGDDivergenceCandidate candidates[];
         SCGDGroupDivergenceState state;
         int candidate_count=m_divergence_field.BuildDivergencesForGroup(time_snapshot,cycle,candidates,state);
         ...
         int start=ArraySize(flat_candidates);
         group_start[g]=start;
         group_count[g]=candidate_count;
         if(candidate_count>0)
         {
            ArrayResize(flat_candidates,start+candidate_count);
            for(int c=0;c<candidate_count;c++)
               flat_candidates[start+c]=candidates[c];
         }
      }

      if(m_show_panel)
         Comment(m_display.BuildPanel(time_snapshot,m_divergence_config,states,flat_candidates,group_start,group_count,m_divergence_field));
