...
public:
   ...
   bool Init()
   {
      if(!m_state.configured)
      {
         ...
         Print("STC LEVEL08 init failed: ", m_state.init_error);
         return false;
      }
      ...
      if(!STC_ValidateConfig(m_cfg, validation_error, validation_warning))
      {
         ...
         Print("STC LEVEL08 config validation failed: ", validation_error, " warning=", validation_warning);
         return false;
      }
      m_state.init_warning = validation_warning;
      m_state.output_root_common = m_cfg.output_root_common;
      m_state.sanity_file_common = STC_JoinPath(m_state.output_root_common, "stc_level08_build_sanity.csv");
      m_state.runtime_events_file_common = STC_JoinPath(m_state.output_root_common, "stc_level08_runtime_events.csv");
      m_state.time_audit_file_common = STC_JoinPath(m_state.output_root_common, "stc_level08_time_audit.csv");
      m_state.check_candle_audit_file_common = STC_JoinPath(m_state.output_root_common, "stc_level08_check_candles.csv");
      m_state.w_level_audit_file_common = STC_JoinPath(m_state.output_root_common, "stc_level08_w_levels.csv");
      m_state.hunt_audit_file_common = STC_JoinPath(m_state.output_root_common, "stc_level08_reference_hunts.csv");
      m_state.smt_candidate_audit_file_common = STC_JoinPath(m_state.output_root_common, "stc_level08_smt_candidates.csv");
      m_state.signal_registry_file_common = STC_JoinPath(m_state.output_root_common, "stc_level08_signal_registry.csv");
      m_state.paper_entry_file_common = STC_JoinPath(m_state.output_root_common, "stc_level08_paper_entries.csv");
      ...
      if(!STC_EnsureCommonFolderTree(m_state.output_root_common))
      {
         ...
         Print("STC LEVEL08 folder setup failed: ", m_state.init_error);
         return false;
      }

      if(!STC_AcquireInstanceLock(m_cfg, m_state))
      {
         Print("STC LEVEL08 instance lock failed: ", m_state.init_error);
         return false;
      }
      ...
      STC_AppendRuntimeEventCsv(m_cfg, m_state, "INIT", "level08 risk plan and paper entry model initialized; no-order paper entry planning enabled; no real orders");
      ...
      if(m_state.init_warning != "")
         Print("STC LEVEL08 validation warning: ", m_state.init_warning);

      Print("STC LEVEL08 initialized. sanity_file=", m_state.sanity_file_common,
      ...
      Print("STC LEVEL08 initial time *** ", STC_TimeSnapshotOneLine(m_time));
      return true;
   }
   ...
   void Pulse(const datetime server_time)
   {
      ...
      if(m_cfg.write_heartbeat)
      {
         if(m_state.last_heartbeat_server_time <= 0 || server_time - m_state.last_heartbeat_server_time >= m_cfg.heartbeat_seconds)
         {
            ...
            string details = "heartbeat; level08 risk plan paper entry; " + STC_TimeSnapshotOneLine(m_time) + "; auditedCheckCandles=" + IntegerToString((int)m_state.check_candles_audited) + "; auditedWLevels=" + IntegerToString((int)m_state.w_levels_audited) + "; auditedHuntRows=" + IntegerToString((int)m_state.hunt_rows_audited) + "; auditedSmtCandidateRows=" + IntegerToString((int)m_state.smt_candidate_rows_audited) + "; auditedSignalRows=" + IntegerToString((int)m_state.signal_rows_audited) + "; auditedPaperRows=" + IntegerToString((int)m_state.paper_entry_rows_audited) + "; no real orders";
            STC_AppendRuntimeEventCsv(m_cfg, m_state, "HEARTBEAT", details);
            Print("STC LEVEL08 heartbeat pulse=", m_state.pulse_count,
   ...
   void Deinit(const int reason)
   {
      if(m_state.initialized)
      {
         ...
         Print("STC LEVEL08 deinit reason=", reason, " pulses=", m_state.pulse_count);
      }
