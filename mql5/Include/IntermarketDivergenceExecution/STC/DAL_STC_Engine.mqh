...
public:
   ...
   bool Init()
   {
      if(!m_state.configured)
      {
         ...
         Print("STC LEVEL09 init failed: ", m_state.init_error);
         return false;
      }
      ...
      if(!STC_ValidateConfig(m_cfg, validation_error, validation_warning))
      {
         ...
         Print("STC LEVEL09 config validation failed: ", validation_error, " warning=", validation_warning);
         return false;
      }
      m_state.init_warning = validation_warning;
      m_state.output_root_common = m_cfg.output_root_common;
      m_state.sanity_file_common = STC_JoinPath(m_state.output_root_common, "stc_level09_build_sanity.csv");
      m_state.runtime_events_file_common = STC_JoinPath(m_state.output_root_common, "stc_level09_runtime_events.csv");
      m_state.time_audit_file_common = STC_JoinPath(m_state.output_root_common, "stc_level09_time_audit.csv");
      m_state.check_candle_audit_file_common = STC_JoinPath(m_state.output_root_common, "stc_level09_check_candles.csv");
      m_state.w_level_audit_file_common = STC_JoinPath(m_state.output_root_common, "stc_level09_w_levels.csv");
      m_state.hunt_audit_file_common = STC_JoinPath(m_state.output_root_common, "stc_level09_reference_hunts.csv");
      m_state.smt_candidate_audit_file_common = STC_JoinPath(m_state.output_root_common, "stc_level09_smt_candidates.csv");
      m_state.signal_registry_file_common = STC_JoinPath(m_state.output_root_common, "stc_level09_signal_registry.csv");
      m_state.paper_entry_file_common = STC_JoinPath(m_state.output_root_common, "stc_level09_paper_entries.csv");
      m_state.paper_outcome_file_common = STC_JoinPath(m_state.output_root_common, "stc_level09_paper_outcomes.csv");
      ...
      if(!STC_EnsureCommonFolderTree(m_state.output_root_common))
      {
         ...
         Print("STC LEVEL09 folder setup failed: ", m_state.init_error);
         return false;
      }

      if(!STC_AcquireInstanceLock(m_cfg, m_state))
      {
         Print("STC LEVEL09 instance lock failed: ", m_state.init_error);
         return false;
      }
      ...
      STC_AppendRuntimeEventCsv(m_cfg, m_state, "INIT", "level09 paper outcome simulator and trade journal initialized; paper outcome simulation enabled; no real orders");
      ...
      if(m_state.init_warning != "")
         Print("STC LEVEL09 validation warning: ", m_state.init_warning);

      Print("STC LEVEL09 initialized. sanity_file=", m_state.sanity_file_common,
      ...
      Print("STC LEVEL09 initial time *** ", STC_TimeSnapshotOneLine(m_time));
      return true;
   }
   ...
   void Pulse(const datetime server_time)
   {
      ...
      if(m_cfg.write_heartbeat)
      {
         if(m_state.last_heartbeat_server_time <= 0 || server_time - m_state.last_heartbeat_server_time >= m_cfg.heartbeat_seconds)
         {
            ...
            string details = "heartbeat; level09 paper outcome simulator; " + STC_TimeSnapshotOneLine(m_time) + "; auditedCheckCandles=" + IntegerToString((int)m_state.check_candles_audited) + "; auditedWLevels=" + IntegerToString((int)m_state.w_levels_audited) + "; auditedHuntRows=" + IntegerToString((int)m_state.hunt_rows_audited) + "; auditedSmtCandidateRows=" + IntegerToString((int)m_state.smt_candidate_rows_audited) + "; auditedSignalRows=" + IntegerToString((int)m_state.signal_rows_audited) + "; auditedPaperRows=" + IntegerToString((int)m_state.paper_entry_rows_audited) + "; auditedOutcomeRows=" + IntegerToString((int)m_state.paper_outcome_rows_audited) + "; no real orders";
            STC_AppendRuntimeEventCsv(m_cfg, m_state, "HEARTBEAT", details);
            Print("STC LEVEL09 heartbeat pulse=", m_state.pulse_count,
   ...
   void Deinit(const int reason)
   {
      if(m_state.initialized)
      {
         ...
         Print("STC LEVEL09 deinit reason=", reason, " pulses=", m_state.pulse_count);
      }
