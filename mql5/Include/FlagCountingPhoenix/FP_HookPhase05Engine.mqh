#ifndef __FP_HOOK_PHASE05_ENGINE_MQH__
#define __FP_HOOK_PHASE05_ENGINE_MQH__
#property strict

#include "FP_HookPhase05Export.mqh"

void FP_RunHookPhase05(const string symbol,
...
{
   FP_ResetHookPhase05Report(report);

   if(!FP_HookP05ShouldRun(cfg))
   {
      report.status = "HOOK_P05_SKIPPED";
   ...
   FP_HookP05BuildRecords(p04_records, cfg, records, report);
   FP_HookP05FinalizeReport(report);

   if(cfg.draw_type_label || cfg.draw_type_anchor || cfg.draw_type_comparison_lines)
      FP_HookP05DrawRecords(cfg, records, report);

   if(cfg.export_csv)
   {
      FP_HookP05ExportTypes(cfg, records, report);
      FP_HookP05ExportSummary(symbol, period, cfg, report);
   }

   if(cfg.print_summary)
      FP_PrintHookPhase05Report("FP_HOOK_P05", report);

   if(cfg.print_samples)
      FP_PrintHookPhase05Samples("FP_HOOK_P05", records, cfg.sample_limit);
}

#endif // __FP_HOOK_PHASE05_ENGINE_MQH__
