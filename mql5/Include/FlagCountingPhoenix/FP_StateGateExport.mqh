                                     ...
                                     FP_StateGateReport &report)
{
   ...
   string portfolio_text = "portfolio | rows=" + IntegerToString(snapshot.paper_portfolio_row_count) + " | " + snapshot.paper_portfolio_status + " | " + snapshot.paper_portfolio_distribution + " | net=" + FP_ExportDouble(snapshot.paper_portfolio_net_delta) + " | avgR=" + FP_ExportDouble(snapshot.paper_portfolio_avg_R);
   FP_StateGateExportWriteLine(handle, FP_StateGatePanelLineRow(cfg, snapshot, line_index++, -1, "GLOBAL", "PAPER_PORTFOLIO", -1, "PAPER_PORTFOLIO", snapshot.paper_portfolio_key, portfolio_text, snapshot.generated_at, "", "", ""));
   ...
   string trade_text = "persistent paper trades | rows=" + IntegerToString(snapshot.persistent_paper_trade_row_count) + " | " + snapshot.persistent_paper_trade_status + " | " + snapshot.persistent_paper_trade_distribution;
   FP_StateGateExportWriteLine(handle, FP_StateGatePanelLineRow(cfg, snapshot, line_index++, -1, "GLOBAL", "PERSISTENT_PAPER_TRADE", -1, "PERSISTENT_PAPER_TRADE", snapshot.persistent_paper_trade_key, trade_text, snapshot.generated_at, "", "", ""));

   string trade_lifecycle_text = "paper trade lifecycle | rows=" + IntegerToString(snapshot.persistent_paper_trade_lifecycle_row_count) + " | " + snapshot.persistent_paper_trade_lifecycle_status + " | " + snapshot.persistent_paper_trade_lifecycle_distribution;
   FP_StateGateExportWriteLine(handle, FP_StateGatePanelLineRow(cfg, snapshot, line_index++, -1, "GLOBAL", "PERSISTENT_PAPER_TRADE_LIFECYCLE", -1, "PERSISTENT_PAPER_TRADE_LIFECYCLE", snapshot.persistent_paper_trade_lifecycle_key, trade_lifecycle_text, snapshot.generated_at, "", "", ""));

   string performance_text = "performance | rows=" + IntegerToString(snapshot.paper_performance_row_count) + " | " + snapshot.paper_performance_status + " | " + snapshot.paper_performance_distribution + " | avgR=" + FP_ExportDouble(snapshot.paper_performance_avg_R);
   FP_StateGateExportWriteLine(handle, FP_StateGatePanelLineRow(cfg, snapshot, line_index++, -1, "GLOBAL", "PAPER_PERFORMANCE", -1, "PAPER_PERFORMANCE", snapshot.paper_performance_key, performance_text, snapshot.generated_at, "", "", ""));

   string path_quality_text = "path quality | rows=" + IntegerToString(snapshot.paper_path_quality_row_count) + " | " + snapshot.paper_path_quality_status + " | " + snapshot.paper_path_quality_distribution + " | avgMFE_R=" + FP_ExportDouble(snapshot.paper_path_quality_avg_mfe_R) + " | avgMAE_R=" + FP_ExportDouble(snapshot.paper_path_quality_avg_mae_R);
   FP_StateGateExportWriteLine(handle, FP_StateGatePanelLineRow(cfg, snapshot, line_index++, -1, "GLOBAL", "PAPER_PATH_QUALITY", -1, "PAPER_PATH_QUALITY", snapshot.paper_path_quality_key, path_quality_text, snapshot.generated_at, "", "", ""));

   string context_text = "context matrix | rows=" + IntegerToString(snapshot.context_performance_row_count) + " | " + snapshot.context_performance_status + " | best=" + snapshot.context_performance_best_context + " | avgR=" + FP_ExportDouble(snapshot.context_performance_best_avg_R) + " | smooth=" + FP_ExportDouble(snapshot.context_performance_best_smoothness);
   FP_StateGateExportWriteLine(handle, FP_StateGatePanelLineRow(cfg, snapshot, line_index++, -1, "GLOBAL", "CONTEXT_PERFORMANCE", -1, "CONTEXT_PERFORMANCE", snapshot.context_performance_key, context_text, snapshot.generated_at, "", "", ""));
   ...
   for(int slot=0; slot<snapshot.timeframe_count; slot++)
   {
      ...
      string regime = "regime | rows=" + IntegerToString(s.paper_regime_row_count) + " | " + s.paper_regime_context_family + " | " + s.paper_regime_result_bucket + " | " + s.paper_regime_outcome;
      FP_StateGateExportWriteLine(handle, FP_StateGatePanelLineRow(cfg, snapshot, line_index++, slot, "PAPER_REGIME", "PAPER_REGIME", -1, "PAPER_REGIME", s.paper_regime_key, regime, s.last_closed_bar_time, s.state_key, s.contract_status, s.entry_bridge_status));
...
string FP_StateGatePaperPortfolioHeader()
{
   string h = "";
   FP_ExportCsvAppend(h, "symbol");
   FP_ExportCsvAppend(h, "chart_timeframe");
   FP_ExportCsvAppend(h, "row_index");
   FP_ExportCsvAppend(h, "summarized_at");
   FP_ExportCsvAppend(h, "status");
   FP_ExportCsvAppend(h, "portfolio_status");
   FP_ExportCsvAppend(h, "portfolio_key");
   FP_ExportCsvAppend(h, "total_result_rows");
   FP_ExportCsvAppend(h, "win_like_rows");
   FP_ExportCsvAppend(h, "loss_like_rows");
   FP_ExportCsvAppend(h, "open_rows");
   FP_ExportCsvAppend(h, "waiting_rows");
   FP_ExportCsvAppend(h, "ambiguous_rows");
   FP_ExportCsvAppend(h, "unknown_rows");
   FP_ExportCsvAppend(h, "r_ready_rows");
   FP_ExportCsvAppend(h, "r_pending_rows");
   FP_ExportCsvAppend(h, "net_delta");
   FP_ExportCsvAppend(h, "avg_delta");
   FP_ExportCsvAppend(h, "avg_R");
   FP_ExportCsvAppend(h, "best_R");
   FP_ExportCsvAppend(h, "worst_R");
   FP_ExportCsvAppend(h, "result_distribution");
   FP_ExportCsvAppend(h, "execution_status");
   FP_ExportCsvAppend(h, "label");
   return h;
}

string FP_StateGatePaperPortfolioRowCsv(const FP_StateGateSnapshot &snapshot,
                                        const int index)
{
   FP_StateGatePaperPortfolioRow p = snapshot.paper_portfolio_rows[index];
   string line = "";
   FP_ExportCsvAppend(line, snapshot.symbol);
   FP_ExportCsvAppend(line, EnumToString(snapshot.chart_timeframe));
   FP_ExportCsvAppend(line, IntegerToString(p.row_index));
   FP_ExportCsvAppend(line, FP_ExportTime(p.summarized_at));
   FP_ExportCsvAppend(line, IntegerToString(p.status));
   FP_ExportCsvAppend(line, p.portfolio_status);
   FP_ExportCsvAppend(line, p.portfolio_key);
   FP_ExportCsvAppend(line, IntegerToString(p.total_result_rows));
   FP_ExportCsvAppend(line, IntegerToString(p.win_like_rows));
   FP_ExportCsvAppend(line, IntegerToString(p.loss_like_rows));
   FP_ExportCsvAppend(line, IntegerToString(p.open_rows));
   FP_ExportCsvAppend(line, IntegerToString(p.waiting_rows));
   FP_ExportCsvAppend(line, IntegerToString(p.ambiguous_rows));
   FP_ExportCsvAppend(line, IntegerToString(p.unknown_rows));
   FP_ExportCsvAppend(line, IntegerToString(p.r_ready_rows));
   FP_ExportCsvAppend(line, IntegerToString(p.r_pending_rows));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.net_delta));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.avg_delta));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.avg_R));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.best_R));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.worst_R));
   FP_ExportCsvAppend(line, p.result_distribution);
   FP_ExportCsvAppend(line, p.execution_status);
   FP_ExportCsvAppend(line, p.label);
   return line;
}

bool FP_StateGateExportPaperPortfolioCsv(const string path,
                                         const FP_StateGateSnapshot &snapshot,
                                         FP_StateGateReport &report)
{
   int handle = INVALID_HANDLE;
   ...
   {
      report.file_errors++;
      report.reason = report.reason + ";state_gate_paper_portfolio_open_failed";
      return false;
   }
   FP_StateGateExportWriteLine(handle, FP_StateGatePaperPortfolioHeader());
   for(int i=0; i<snapshot.paper_portfolio_row_count; i++)
      FP_StateGateExportWriteLine(handle, FP_StateGatePaperPortfolioRowCsv(snapshot, i));
   FileClose(handle);
   report.files_written++;
   return true;
}
...
string FP_StateGatePaperRegimeHeader()
{
   string h = "";
   FP_ExportCsvAppend(h, "symbol");
   FP_ExportCsvAppend(h, "chart_timeframe");
   FP_ExportCsvAppend(h, "slot");
   FP_ExportCsvAppend(h, "timeframe");
   FP_ExportCsvAppend(h, "attributed_at");
   FP_ExportCsvAppend(h, "closed_bar_time");
   FP_ExportCsvAppend(h, "closed_bar_close");
   FP_ExportCsvAppend(h, "status");
   FP_ExportCsvAppend(h, "attribution_status");
   FP_ExportCsvAppend(h, "context_family");
   FP_ExportCsvAppend(h, "source_context");
   FP_ExportCsvAppend(h, "mtf_context");
   FP_ExportCsvAppend(h, "mtf_direction_relation");
   FP_ExportCsvAppend(h, "mtf_side_relation");
   FP_ExportCsvAppend(h, "geometry_context");
   FP_ExportCsvAppend(h, "idea_family");
   FP_ExportCsvAppend(h, "idea_type");
   FP_ExportCsvAppend(h, "decision_type");
   FP_ExportCsvAppend(h, "extreme_source");
   FP_ExportCsvAppend(h, "extreme_side");
   FP_ExportCsvAppend(h, "hook_context");
   FP_ExportCsvAppend(h, "rally_context");
   FP_ExportCsvAppend(h, "lifecycle_path_state");
   FP_ExportCsvAppend(h, "outcome");
   FP_ExportCsvAppend(h, "result_bucket");
   FP_ExportCsvAppend(h, "price_delta");
   FP_ExportCsvAppend(h, "r_status");
   ...
   FP_ExportCsvAppend(h, "attribution_key");
   FP_ExportCsvAppend(h, "execution_status");
   FP_ExportCsvAppend(h, "label");
   return h;
}

string FP_StateGatePaperRegimeRowCsv(const FP_StateGateSnapshot &snapshot,
                                     const int index)
{
   FP_StateGatePaperRegimeRow r = snapshot.paper_regime_rows[index];
   string line = "";
   FP_ExportCsvAppend(line, snapshot.symbol);
   FP_ExportCsvAppend(line, EnumToString(snapshot.chart_timeframe));
   FP_ExportCsvAppend(line, IntegerToString(r.slot_index));
   FP_ExportCsvAppend(line, r.timeframe_label);
   FP_ExportCsvAppend(line, FP_ExportTime(r.attributed_at));
   FP_ExportCsvAppend(line, FP_ExportTime(r.last_closed_bar_time));
   FP_ExportCsvAppend(line, FP_ExportDouble(r.last_closed_bar_close));
   FP_ExportCsvAppend(line, IntegerToString(r.status));
   FP_ExportCsvAppend(line, r.attribution_status);
   FP_ExportCsvAppend(line, r.context_family);
   FP_ExportCsvAppend(line, r.source_context);
   FP_ExportCsvAppend(line, r.mtf_context);
   FP_ExportCsvAppend(line, r.mtf_direction_relation);
   FP_ExportCsvAppend(line, r.mtf_side_relation);
   FP_ExportCsvAppend(line, r.geometry_context);
   FP_ExportCsvAppend(line, r.idea_family);
   FP_ExportCsvAppend(line, r.idea_type);
   FP_ExportCsvAppend(line, r.decision_type);
   FP_ExportCsvAppend(line, r.extreme_source);
   FP_ExportCsvAppend(line, r.extreme_side);
   FP_ExportCsvAppend(line, r.hook_context);
   FP_ExportCsvAppend(line, r.rally_context);
   FP_ExportCsvAppend(line, r.lifecycle_path_state);
   FP_ExportCsvAppend(line, r.outcome);
   FP_ExportCsvAppend(line, r.result_bucket);
   FP_ExportCsvAppend(line, FP_ExportDouble(r.price_delta));
   FP_ExportCsvAppend(line, r.r_status);
   FP_ExportCsvAppend(line, FP_ExportDouble(r.r_multiple));
   FP_ExportCsvAppend(line, r.source_result_key);
   FP_ExportCsvAppend(line, r.source_lifecycle_key);
   FP_ExportCsvAppend(line, r.source_portfolio_key);
   FP_ExportCsvAppend(line, r.attribution_key);
   FP_ExportCsvAppend(line, r.execution_status);
   FP_ExportCsvAppend(line, r.label);
   return line;
}

bool FP_StateGateExportPaperRegimeCsv(const string path,
                                      const FP_StateGateSnapshot &snapshot,
                                      FP_StateGateReport &report)
{
   int handle = INVALID_HANDLE;
   ...
   {
      report.file_errors++;
      report.reason = report.reason + ";state_gate_paper_regime_open_failed";
      return false;
   }
   FP_StateGateExportWriteLine(handle, FP_StateGatePaperRegimeHeader());
   for(int i=0; i<snapshot.paper_regime_row_count; i++)
      FP_StateGateExportWriteLine(handle, FP_StateGatePaperRegimeRowCsv(snapshot, i));
   FileClose(handle);
   report.files_written++;
   return true;
}
...
string FP_StateGatePaperFilterHeader()
{
   string h = "";
   FP_ExportCsvAppend(h, "symbol");
   FP_ExportCsvAppend(h, "chart_timeframe");
   FP_ExportCsvAppend(h, "filter_index");
   FP_ExportCsvAppend(h, "filter_name");
   FP_ExportCsvAppend(h, "filter_family");
   FP_ExportCsvAppend(h, "filter_status");
   FP_ExportCsvAppend(h, "filter_rule");
   FP_ExportCsvAppend(h, "rows_before");
   FP_ExportCsvAppend(h, "rows_after");
   FP_ExportCsvAppend(h, "win_like_rows");
   FP_ExportCsvAppend(h, "loss_like_rows");
   FP_ExportCsvAppend(h, "open_rows");
   FP_ExportCsvAppend(h, "waiting_rows");
   ...
   FP_ExportCsvAppend(h, "avg_delta");
   FP_ExportCsvAppend(h, "avg_R");
   FP_ExportCsvAppend(h, "pass_rate");
   FP_ExportCsvAppend(h, "strongest_context");
   FP_ExportCsvAppend(h, "diagnostic_key");
   FP_ExportCsvAppend(h, "execution_status");
   FP_ExportCsvAppend(h, "label");
   return h;
}

string FP_StateGatePaperFilterRowCsv(const FP_StateGateSnapshot &snapshot,
                                     const int index)
{
   FP_StateGatePaperFilterRow f = snapshot.paper_filter_rows[index];
   string line = "";
   FP_ExportCsvAppend(line, snapshot.symbol);
   FP_ExportCsvAppend(line, EnumToString(snapshot.chart_timeframe));
   FP_ExportCsvAppend(line, IntegerToString(f.filter_index));
   FP_ExportCsvAppend(line, f.filter_name);
   FP_ExportCsvAppend(line, f.filter_family);
   FP_ExportCsvAppend(line, f.filter_status);
   FP_ExportCsvAppend(line, f.filter_rule);
   FP_ExportCsvAppend(line, IntegerToString(f.rows_before));
   FP_ExportCsvAppend(line, IntegerToString(f.rows_after));
   FP_ExportCsvAppend(line, IntegerToString(f.win_like_rows));
   FP_ExportCsvAppend(line, IntegerToString(f.loss_like_rows));
   FP_ExportCsvAppend(line, IntegerToString(f.open_rows));
   FP_ExportCsvAppend(line, IntegerToString(f.waiting_rows));
   FP_ExportCsvAppend(line, IntegerToString(f.ambiguous_rows));
   FP_ExportCsvAppend(line, IntegerToString(f.unknown_rows));
   FP_ExportCsvAppend(line, IntegerToString(f.r_ready_rows));
   FP_ExportCsvAppend(line, IntegerToString(f.r_pending_rows));
   FP_ExportCsvAppend(line, FP_ExportDouble(f.net_delta));
   FP_ExportCsvAppend(line, FP_ExportDouble(f.avg_delta));
   FP_ExportCsvAppend(line, FP_ExportDouble(f.avg_R));
   FP_ExportCsvAppend(line, FP_ExportDouble(f.pass_rate));
   FP_ExportCsvAppend(line, f.strongest_context);
   FP_ExportCsvAppend(line, f.diagnostic_key);
   FP_ExportCsvAppend(line, f.execution_status);
   FP_ExportCsvAppend(line, f.label);
   return line;
}

bool FP_StateGateExportPaperFiltersCsv(const string path,
                                       const FP_StateGateSnapshot &snapshot,
                                       FP_StateGateReport &report)
{
   int handle = INVALID_HANDLE;
   ...
   {
      report.file_errors++;
      report.reason = report.reason + ";state_gate_paper_filters_open_failed";
      return false;
   }
   FP_StateGateExportWriteLine(handle, FP_StateGatePaperFilterHeader());
   for(int i=0; i<snapshot.paper_filter_row_count; i++)
      FP_StateGateExportWriteLine(handle, FP_StateGatePaperFilterRowCsv(snapshot, i));
   FileClose(handle);
   report.files_written++;
   return true;
}
...
string FP_StateGatePersistentPaperTradeHeader()
{
   string h = "";
   FP_ExportCsvAppend(h, "symbol");
   FP_ExportCsvAppend(h, "chart_timeframe");
   FP_ExportCsvAppend(h, "slot");
   FP_ExportCsvAppend(h, "timeframe");
   FP_ExportCsvAppend(h, "registered_at");
   FP_ExportCsvAppend(h, "opened_bar_time");
   FP_ExportCsvAppend(h, "last_seen_bar_time");
   FP_ExportCsvAppend(h, "last_closed_bar_close");
   FP_ExportCsvAppend(h, "status");
   FP_ExportCsvAppend(h, "trade_status");
   FP_ExportCsvAppend(h, "lifecycle_status");
   FP_ExportCsvAppend(h, "trade_id");
   FP_ExportCsvAppend(h, "trade_key");
   FP_ExportCsvAppend(h, "trade_mode");
   FP_ExportCsvAppend(h, "policy_allowed_dry_run");
   FP_ExportCsvAppend(h, "policy_status");
   FP_ExportCsvAppend(h, "policy_name");
   FP_ExportCsvAppend(h, "direction");
   ...
   FP_ExportCsvAppend(h, "invalidation_price");
   FP_ExportCsvAppend(h, "destination_price");
   FP_ExportCsvAppend(h, "current_close");
   FP_ExportCsvAppend(h, "unrealized_delta");
   FP_ExportCsvAppend(h, "r_status");
   FP_ExportCsvAppend(h, "r_multiple");
   FP_ExportCsvAppend(h, "source_policy_key");
   FP_ExportCsvAppend(h, "source_decision_key");
   FP_ExportCsvAppend(h, "source_ledger_key");
   FP_ExportCsvAppend(h, "source_result_key");
   FP_ExportCsvAppend(h, "block_reason");
...
string FP_StateGatePersistentPaperTradeRowCsv(const FP_StateGateSnapshot &snapshot,
                                              const int index)
{
   FP_StateGatePersistentPaperTradeRow t = snapshot.persistent_paper_trade_rows[index];
   string line = "";
   FP_ExportCsvAppend(line, snapshot.symbol);
   FP_ExportCsvAppend(line, EnumToString(snapshot.chart_timeframe));
   FP_ExportCsvAppend(line, IntegerToString(t.slot_index));
   FP_ExportCsvAppend(line, t.timeframe_label);
   FP_ExportCsvAppend(line, FP_ExportTime(t.registered_at));
   FP_ExportCsvAppend(line, FP_ExportTime(t.opened_bar_time));
   FP_ExportCsvAppend(line, FP_ExportTime(t.last_seen_bar_time));
   FP_ExportCsvAppend(line, FP_ExportDouble(t.last_closed_bar_close));
   FP_ExportCsvAppend(line, IntegerToString(t.status));
   FP_ExportCsvAppend(line, t.trade_status);
   FP_ExportCsvAppend(line, t.lifecycle_status);
   FP_ExportCsvAppend(line, t.trade_id);
   FP_ExportCsvAppend(line, t.trade_key);
   FP_ExportCsvAppend(line, t.trade_mode);
   FP_ExportCsvAppend(line, FP_ExportBool(t.policy_allowed_dry_run));
   FP_ExportCsvAppend(line, t.policy_status);
   FP_ExportCsvAppend(line, t.policy_name);
   FP_ExportCsvAppend(line, t.direction);
   FP_ExportCsvAppend(line, t.decision_type);
   FP_ExportCsvAppend(line, FP_ExportDouble(t.entry_price));
   FP_ExportCsvAppend(line, FP_ExportDouble(t.invalidation_price));
   FP_ExportCsvAppend(line, FP_ExportDouble(t.destination_price));
   FP_ExportCsvAppend(line, FP_ExportDouble(t.current_close));
   FP_ExportCsvAppend(line, FP_ExportDouble(t.unrealized_delta));
   FP_ExportCsvAppend(line, t.r_status);
   FP_ExportCsvAppend(line, FP_ExportDouble(t.r_multiple));
   FP_ExportCsvAppend(line, t.source_policy_key);
   FP_ExportCsvAppend(line, t.source_decision_key);
   FP_ExportCsvAppend(line, t.source_ledger_key);
   FP_ExportCsvAppend(line, t.source_result_key);
   FP_ExportCsvAppend(line, t.block_reason);
   FP_ExportCsvAppend(line, t.execution_status);
   FP_ExportCsvAppend(line, t.label);
   return line;
}

bool FP_StateGateExportPersistentPaperTradesCsv(const string path,
                                                const FP_StateGateSnapshot &snapshot,
                                                FP_StateGateReport &report)
{
   int handle = INVALID_HANDLE;
   ...
   {
      report.file_errors++;
      report.reason = report.reason + ";state_gate_persistent_paper_trades_open_failed";
      return false;
   }
   FP_StateGateExportWriteLine(handle, FP_StateGatePersistentPaperTradeHeader());
   for(int i=0; i<snapshot.persistent_paper_trade_row_count; i++)
      FP_StateGateExportWriteLine(handle, FP_StateGatePersistentPaperTradeRowCsv(snapshot, i));
   FileClose(handle);
   report.files_written++;
   return true;
}
...
string FP_StateGatePersistentPaperTradeLifecycleHeader()
{
   string h = "";
   FP_ExportCsvAppend(h, "symbol");
   FP_ExportCsvAppend(h, "chart_timeframe");
   ...
   FP_ExportCsvAppend(h, "lifecycle_status");
   FP_ExportCsvAppend(h, "path_state");
   FP_ExportCsvAppend(h, "terminal_status");
   FP_ExportCsvAppend(h, "entry_touch_status");
   FP_ExportCsvAppend(h, "invalidation_touch_status");
   FP_ExportCsvAppend(h, "destination_touch_status");
   FP_ExportCsvAppend(h, "trade_id");
   ...
   FP_ExportCsvAppend(h, "destination_price");
   FP_ExportCsvAppend(h, "current_close");
   FP_ExportCsvAppend(h, "signed_delta");
   FP_ExportCsvAppend(h, "r_status");
   FP_ExportCsvAppend(h, "r_multiple");
   FP_ExportCsvAppend(h, "source_trade_key");
   FP_ExportCsvAppend(h, "source_policy_key");
   FP_ExportCsvAppend(h, "source_result_key");
   FP_ExportCsvAppend(h, "lifecycle_key");
   FP_ExportCsvAppend(h, "execution_status");
...
string FP_StateGatePersistentPaperTradeLifecycleRowCsv(const FP_StateGateSnapshot &snapshot,
                                                       const int index)
{
   FP_StateGatePersistentPaperTradeLifecycleRow l = snapshot.persistent_paper_trade_lifecycle_rows[index];
   string line = "";
   FP_ExportCsvAppend(line, snapshot.symbol);
   FP_ExportCsvAppend(line, EnumToString(snapshot.chart_timeframe));
   FP_ExportCsvAppend(line, IntegerToString(l.slot_index));
   FP_ExportCsvAppend(line, l.timeframe_label);
   FP_ExportCsvAppend(line, FP_ExportTime(l.evaluated_at));
   FP_ExportCsvAppend(line, FP_ExportTime(l.registered_at));
   FP_ExportCsvAppend(line, FP_ExportTime(l.opened_bar_time));
   FP_ExportCsvAppend(line, FP_ExportTime(l.last_seen_bar_time));
   FP_ExportCsvAppend(line, FP_ExportDouble(l.last_closed_bar_close));
   FP_ExportCsvAppend(line, IntegerToString(l.status));
   FP_ExportCsvAppend(line, l.lifecycle_status);
   FP_ExportCsvAppend(line, l.path_state);
   FP_ExportCsvAppend(line, l.terminal_status);
   FP_ExportCsvAppend(line, l.entry_touch_status);
   FP_ExportCsvAppend(line, l.invalidation_touch_status);
   FP_ExportCsvAppend(line, l.destination_touch_status);
   FP_ExportCsvAppend(line, l.trade_id);
   FP_ExportCsvAppend(line, l.trade_key);
   FP_ExportCsvAppend(line, l.direction);
   FP_ExportCsvAppend(line, l.decision_type);
   FP_ExportCsvAppend(line, FP_ExportDouble(l.entry_price));
   FP_ExportCsvAppend(line, FP_ExportDouble(l.invalidation_price));
   FP_ExportCsvAppend(line, FP_ExportDouble(l.destination_price));
   FP_ExportCsvAppend(line, FP_ExportDouble(l.current_close));
   FP_ExportCsvAppend(line, FP_ExportDouble(l.signed_delta));
   FP_ExportCsvAppend(line, l.r_status);
   FP_ExportCsvAppend(line, FP_ExportDouble(l.r_multiple));
   FP_ExportCsvAppend(line, l.source_trade_key);
   FP_ExportCsvAppend(line, l.source_policy_key);
   FP_ExportCsvAppend(line, l.source_result_key);
   FP_ExportCsvAppend(line, l.lifecycle_key);
   FP_ExportCsvAppend(line, l.execution_status);
   FP_ExportCsvAppend(line, l.label);
   return line;
}

bool FP_StateGateExportPersistentPaperTradeLifecycleCsv(const string path,
                                                        const FP_StateGateSnapshot &snapshot,
                                                        FP_StateGateReport &report)
{
   int handle = INVALID_HANDLE;
   ...
   {
      report.file_errors++;
      report.reason = report.reason + ";state_gate_persistent_paper_trade_lifecycle_open_failed";
      return false;
   }
   FP_StateGateExportWriteLine(handle, FP_StateGatePersistentPaperTradeLifecycleHeader());
   for(int i=0; i<snapshot.persistent_paper_trade_lifecycle_row_count; i++)
      FP_StateGateExportWriteLine(handle, FP_StateGatePersistentPaperTradeLifecycleRowCsv(snapshot, i));
   FileClose(handle);
   report.files_written++;
   return true;
}
...
string FP_StateGatePaperPerformanceHeader()
{
   string h = "";
   FP_ExportCsvAppend(h, "symbol");
   FP_ExportCsvAppend(h, "chart_timeframe");
   ...
   FP_ExportCsvAppend(h, "summarized_at");
   FP_ExportCsvAppend(h, "status");
   FP_ExportCsvAppend(h, "performance_status");
   FP_ExportCsvAppend(h, "source_lifecycle_rows");
   FP_ExportCsvAppend(h, "closed_rows");
   FP_ExportCsvAppend(h, "open_rows");
   FP_ExportCsvAppend(h, "pending_entry_rows");
   FP_ExportCsvAppend(h, "blocked_rows");
   FP_ExportCsvAppend(h, "ambiguous_rows");
   FP_ExportCsvAppend(h, "win_rows");
   FP_ExportCsvAppend(h, "loss_rows");
   FP_ExportCsvAppend(h, "r_ready_rows");
   FP_ExportCsvAppend(h, "r_pending_rows");
   FP_ExportCsvAppend(h, "win_rate_like");
   FP_ExportCsvAppend(h, "closed_win_rate_like");
   FP_ExportCsvAppend(h, "net_delta");
   FP_ExportCsvAppend(h, "avg_delta");
   FP_ExportCsvAppend(h, "net_R");
   FP_ExportCsvAppend(h, "avg_R");
   FP_ExportCsvAppend(h, "expectancy_R");
   FP_ExportCsvAppend(h, "best_R");
   FP_ExportCsvAppend(h, "worst_R");
   FP_ExportCsvAppend(h, "distribution");
   FP_ExportCsvAppend(h, "r_distribution");
   FP_ExportCsvAppend(h, "performance_key");
   FP_ExportCsvAppend(h, "execution_status");
   FP_ExportCsvAppend(h, "label");
   return h;
}

string FP_StateGatePaperPerformanceRowCsv(const FP_StateGateSnapshot &snapshot,
                                          const int index)
{
   FP_StateGatePaperPerformanceRow p = snapshot.paper_performance_rows[index];
   string line = "";
   FP_ExportCsvAppend(line, snapshot.symbol);
   FP_ExportCsvAppend(line, EnumToString(snapshot.chart_timeframe));
   FP_ExportCsvAppend(line, IntegerToString(p.row_index));
   FP_ExportCsvAppend(line, FP_ExportTime(p.summarized_at));
   FP_ExportCsvAppend(line, IntegerToString(p.status));
   FP_ExportCsvAppend(line, p.performance_status);
   FP_ExportCsvAppend(line, IntegerToString(p.source_lifecycle_rows));
   FP_ExportCsvAppend(line, IntegerToString(p.closed_rows));
   FP_ExportCsvAppend(line, IntegerToString(p.open_rows));
   FP_ExportCsvAppend(line, IntegerToString(p.pending_entry_rows));
   FP_ExportCsvAppend(line, IntegerToString(p.blocked_rows));
   FP_ExportCsvAppend(line, IntegerToString(p.ambiguous_rows));
   FP_ExportCsvAppend(line, IntegerToString(p.win_rows));
   FP_ExportCsvAppend(line, IntegerToString(p.loss_rows));
   FP_ExportCsvAppend(line, IntegerToString(p.r_ready_rows));
   FP_ExportCsvAppend(line, IntegerToString(p.r_pending_rows));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.win_rate_like));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.closed_win_rate_like));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.net_delta));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.avg_delta));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.net_R));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.avg_R));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.expectancy_R));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.best_R));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.worst_R));
   FP_ExportCsvAppend(line, p.distribution);
   FP_ExportCsvAppend(line, p.r_distribution);
   FP_ExportCsvAppend(line, p.performance_key);
   FP_ExportCsvAppend(line, p.execution_status);
   FP_ExportCsvAppend(line, p.label);
   return line;
}

bool FP_StateGateExportPaperPerformanceCsv(const string path,
                                           const FP_StateGateSnapshot &snapshot,
                                           FP_StateGateReport &report)
{
   int handle = INVALID_HANDLE;
   ...
   {
      report.file_errors++;
      report.reason = report.reason + ";state_gate_paper_performance_open_failed";
      return false;
   }
   FP_StateGateExportWriteLine(handle, FP_StateGatePaperPerformanceHeader());
   for(int i=0; i<snapshot.paper_performance_row_count; i++)
      FP_StateGateExportWriteLine(handle, FP_StateGatePaperPerformanceRowCsv(snapshot, i));
   FileClose(handle);
   report.files_written++;
   return true;
}
...
string FP_StateGatePaperPathQualityHeader()
{
   string h = "";
   FP_ExportCsvAppend(h, "symbol");
   FP_ExportCsvAppend(h, "chart_timeframe");
   ...
   FP_ExportCsvAppend(h, "evaluated_at");
   FP_ExportCsvAppend(h, "status");
   FP_ExportCsvAppend(h, "path_quality_status");
   FP_ExportCsvAppend(h, "path_quality_bucket");
   FP_ExportCsvAppend(h, "pullback_pressure_status");
   FP_ExportCsvAppend(h, "trade_id");
   FP_ExportCsvAppend(h, "direction");
   FP_ExportCsvAppend(h, "terminal_status");
   FP_ExportCsvAppend(h, "lifecycle_status");
   ...
   FP_ExportCsvAppend(h, "signed_delta");
   FP_ExportCsvAppend(h, "risk_distance");
   FP_ExportCsvAppend(h, "favorable_delta");
   FP_ExportCsvAppend(h, "adverse_delta");
   FP_ExportCsvAppend(h, "mfe_proxy");
   FP_ExportCsvAppend(h, "mae_proxy");
   FP_ExportCsvAppend(h, "mfe_R");
   FP_ExportCsvAppend(h, "mae_R");
   FP_ExportCsvAppend(h, "net_R");
   FP_ExportCsvAppend(h, "path_smoothness_score");
   FP_ExportCsvAppend(h, "source_lifecycle_key");
   FP_ExportCsvAppend(h, "path_quality_key");
   FP_ExportCsvAppend(h, "execution_status");
   FP_ExportCsvAppend(h, "label");
   return h;
}

string FP_StateGatePaperPathQualityRowCsv(const FP_StateGateSnapshot &snapshot,
                                          const int index)
{
   FP_StateGatePaperPathQualityRow p = snapshot.paper_path_quality_rows[index];
   string line = "";
   FP_ExportCsvAppend(line, snapshot.symbol);
   FP_ExportCsvAppend(line, EnumToString(snapshot.chart_timeframe));
   FP_ExportCsvAppend(line, IntegerToString(p.slot_index));
   ...
   FP_ExportCsvAppend(line, FP_ExportTime(p.evaluated_at));
   FP_ExportCsvAppend(line, IntegerToString(p.status));
   FP_ExportCsvAppend(line, p.path_quality_status);
   FP_ExportCsvAppend(line, p.path_quality_bucket);
   FP_ExportCsvAppend(line, p.pullback_pressure_status);
   FP_ExportCsvAppend(line, p.trade_id);
   FP_ExportCsvAppend(line, p.direction);
   FP_ExportCsvAppend(line, p.terminal_status);
   FP_ExportCsvAppend(line, p.lifecycle_status);
   FP_ExportCsvAppend(line, FP_ExportDouble(p.entry_price));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.current_close));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.destination_price));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.invalidation_price));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.signed_delta));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.risk_distance));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.favorable_delta));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.adverse_delta));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.mfe_proxy));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.mae_proxy));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.mfe_R));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.mae_R));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.net_R));
   FP_ExportCsvAppend(line, FP_ExportDouble(p.path_smoothness_score));
   FP_ExportCsvAppend(line, p.source_lifecycle_key);
   FP_ExportCsvAppend(line, p.path_quality_key);
   FP_ExportCsvAppend(line, p.execution_status);
   FP_ExportCsvAppend(line, p.label);
   return line;
}

bool FP_StateGateExportPaperPathQualityCsv(const string path,
                                           const FP_StateGateSnapshot &snapshot,
                                           FP_StateGateReport &report)
{
   int handle = INVALID_HANDLE;
   ...
   {
      report.file_errors++;
      report.reason = report.reason + ";state_gate_paper_path_quality_open_failed";
      return false;
   }
   FP_StateGateExportWriteLine(handle, FP_StateGatePaperPathQualityHeader());
   for(int i=0; i<snapshot.paper_path_quality_row_count; i++)
      FP_StateGateExportWriteLine(handle, FP_StateGatePaperPathQualityRowCsv(snapshot, i));
   FileClose(handle);
   report.files_written++;
   return true;
}
...
string FP_StateGateContextPerformanceHeader()
{
   string h = "";
   FP_ExportCsvAppend(h, "symbol");
   FP_ExportCsvAppend(h, "chart_timeframe");
   FP_ExportCsvAppend(h, "row_index");
   FP_ExportCsvAppend(h, "dimension_name");
   FP_ExportCsvAppend(h, "dimension_value");
   FP_ExportCsvAppend(h, "matrix_status");
   FP_ExportCsvAppend(h, "context_rule");
   FP_ExportCsvAppend(h, "source_regime_rows");
   FP_ExportCsvAppend(h, "rows_after");
   FP_ExportCsvAppend(h, "win_like_rows");
   FP_ExportCsvAppend(h, "loss_like_rows");
   FP_ExportCsvAppend(h, "open_rows");
   ...
   FP_ExportCsvAppend(h, "r_ready_rows");
   FP_ExportCsvAppend(h, "r_pending_rows");
   FP_ExportCsvAppend(h, "clean_path_rows");
   FP_ExportCsvAppend(h, "adverse_path_rows");
   FP_ExportCsvAppend(h, "pending_path_rows");
   FP_ExportCsvAppend(h, "ambiguous_path_rows");
   FP_ExportCsvAppend(h, "net_delta");
   FP_ExportCsvAppend(h, "avg_delta");
   FP_ExportCsvAppend(h, "net_R");
   FP_ExportCsvAppend(h, "avg_R");
   FP_ExportCsvAppend(h, "avg_mfe_R");
   FP_ExportCsvAppend(h, "avg_mae_R");
   FP_ExportCsvAppend(h, "avg_smoothness_score");
   FP_ExportCsvAppend(h, "win_rate_like");
   FP_ExportCsvAppend(h, "pass_rate");
   FP_ExportCsvAppend(h, "distribution");
   FP_ExportCsvAppend(h, "context_performance_key");
   FP_ExportCsvAppend(h, "execution_status");
   FP_ExportCsvAppend(h, "label");
   return h;
}

string FP_StateGateContextPerformanceRowCsv(const FP_StateGateSnapshot &snapshot,
                                            const int index)
{
   FP_StateGateContextPerformanceRow r = snapshot.context_performance_rows[index];
   string line = "";
   FP_ExportCsvAppend(line, snapshot.symbol);
   FP_ExportCsvAppend(line, EnumToString(snapshot.chart_timeframe));
   FP_ExportCsvAppend(line, IntegerToString(r.row_index));
   FP_ExportCsvAppend(line, r.dimension_name);
   FP_ExportCsvAppend(line, r.dimension_value);
   FP_ExportCsvAppend(line, r.matrix_status);
   FP_ExportCsvAppend(line, r.context_rule);
   FP_ExportCsvAppend(line, IntegerToString(r.source_regime_rows));
   FP_ExportCsvAppend(line, IntegerToString(r.rows_after));
   FP_ExportCsvAppend(line, IntegerToString(r.win_like_rows));
   FP_ExportCsvAppend(line, IntegerToString(r.loss_like_rows));
   FP_ExportCsvAppend(line, IntegerToString(r.open_rows));
   FP_ExportCsvAppend(line, IntegerToString(r.waiting_rows));
   FP_ExportCsvAppend(line, IntegerToString(r.ambiguous_rows));
   FP_ExportCsvAppend(line, IntegerToString(r.unknown_rows));
   FP_ExportCsvAppend(line, IntegerToString(r.r_ready_rows));
   FP_ExportCsvAppend(line, IntegerToString(r.r_pending_rows));
   FP_ExportCsvAppend(line, IntegerToString(r.clean_path_rows));
   FP_ExportCsvAppend(line, IntegerToString(r.adverse_path_rows));
   FP_ExportCsvAppend(line, IntegerToString(r.pending_path_rows));
   FP_ExportCsvAppend(line, IntegerToString(r.ambiguous_path_rows));
   FP_ExportCsvAppend(line, FP_ExportDouble(r.net_delta));
   FP_ExportCsvAppend(line, FP_ExportDouble(r.avg_delta));
   FP_ExportCsvAppend(line, FP_ExportDouble(r.net_R));
   FP_ExportCsvAppend(line, FP_ExportDouble(r.avg_R));
   FP_ExportCsvAppend(line, FP_ExportDouble(r.avg_mfe_R));
   FP_ExportCsvAppend(line, FP_ExportDouble(r.avg_mae_R));
   FP_ExportCsvAppend(line, FP_ExportDouble(r.avg_smoothness_score));
   FP_ExportCsvAppend(line, FP_ExportDouble(r.win_rate_like));
   FP_ExportCsvAppend(line, FP_ExportDouble(r.pass_rate));
   FP_ExportCsvAppend(line, r.distribution);
   FP_ExportCsvAppend(line, r.context_performance_key);
   FP_ExportCsvAppend(line, r.execution_status);
   FP_ExportCsvAppend(line, r.label);
   return line;
}

bool FP_StateGateExportContextPerformanceCsv(const string path,
                                             const FP_StateGateSnapshot &snapshot,
                                             FP_StateGateReport &report)
{
   int handle = INVALID_HANDLE;
   ...
   {
      report.file_errors++;
      report.reason = report.reason + ";state_gate_context_performance_open_failed";
      return false;
   }
   FP_StateGateExportWriteLine(handle, FP_StateGateContextPerformanceHeader());
   for(int i=0; i<snapshot.context_performance_row_count; i++)
      FP_StateGateExportWriteLine(handle, FP_StateGateContextPerformanceRowCsv(snapshot, i));
   FileClose(handle);
   report.files_written++;
   return true;
}
                                   ...
                                   FP_StateGateReport &report)
{
   ...
   FP_StateGateManifestKV(handle, "export_persistent_paper_trades_csv", FP_ExportBool(cfg.export_persistent_paper_trades_csv));
   FP_StateGateManifestKV(handle, "export_persistent_paper_trade_lifecycle_csv", FP_ExportBool(cfg.export_persistent_paper_trade_lifecycle_csv));
   FP_StateGateManifestKV(handle, "export_paper_performance_csv", FP_ExportBool(cfg.export_paper_performance_csv));
   FP_StateGateManifestKV(handle, "export_paper_path_quality_csv", FP_ExportBool(cfg.export_paper_path_quality_csv));
   FP_StateGateManifestKV(handle, "export_context_performance_csv", FP_ExportBool(cfg.export_context_performance_csv));
   ...
   FP_StateGateManifestKV(handle, "persistent_paper_trade_rows", IntegerToString(snapshot.persistent_paper_trade_row_count));
   FP_StateGateManifestKV(handle, "persistent_paper_trade_lifecycle_rows", IntegerToString(snapshot.persistent_paper_trade_lifecycle_row_count));
   FP_StateGateManifestKV(handle, "paper_performance_rows", IntegerToString(snapshot.paper_performance_row_count));
   FP_StateGateManifestKV(handle, "paper_path_quality_rows", IntegerToString(snapshot.paper_path_quality_row_count));
   FP_StateGateManifestKV(handle, "context_performance_rows", IntegerToString(snapshot.context_performance_row_count));
   ...
   FP_StateGateManifestKV(handle, "persistent_paper_trade_status", snapshot.persistent_paper_trade_status);
   FP_StateGateManifestKV(handle, "persistent_paper_trade_registered_rows", IntegerToString(snapshot.persistent_paper_trade_registered_rows));
   FP_StateGateManifestKV(handle, "persistent_paper_trade_lifecycle_status", snapshot.persistent_paper_trade_lifecycle_status);
   FP_StateGateManifestKV(handle, "paper_performance_status", snapshot.paper_performance_status);
   FP_StateGateManifestKV(handle, "paper_path_quality_status", snapshot.paper_path_quality_status);
   FP_StateGateManifestKV(handle, "context_performance_status", snapshot.context_performance_status);
   FP_StateGateManifestKV(handle, "projection_state", "phase28_context_performance_matrix");
                                 ...
                                 FP_StateGateReport &report)
{
   ...
   string persistent_paper_trades_path = FP_StateGateExportPath("persistent_paper_trades", cfg);
   string persistent_paper_trade_lifecycle_path = FP_StateGateExportPath("persistent_paper_trade_lifecycle", cfg);
   string paper_performance_path = FP_StateGateExportPath("paper_performance", cfg);
   string paper_path_quality_path = FP_StateGateExportPath("paper_path_quality", cfg);
   string context_performance_path = FP_StateGateExportPath("context_performance", cfg);
   ...
   if(cfg.export_paper_portfolio_csv)
      FP_StateGateExportPaperPortfolioCsv(paper_portfolio_path, snapshot, report);
   if(cfg.export_paper_regime_csv)
   ...
   if(cfg.export_persistent_paper_trades_csv)
      FP_StateGateExportPersistentPaperTradesCsv(persistent_paper_trades_path, snapshot, report);
   if(cfg.export_persistent_paper_trade_lifecycle_csv)
      FP_StateGateExportPersistentPaperTradeLifecycleCsv(persistent_paper_trade_lifecycle_path, snapshot, report);
   if(cfg.export_paper_performance_csv)
      FP_StateGateExportPaperPerformanceCsv(paper_performance_path, snapshot, report);
   if(cfg.export_paper_path_quality_csv)
      FP_StateGateExportPaperPathQualityCsv(paper_path_quality_path, snapshot, report);
   if(cfg.export_context_performance_csv)
      FP_StateGateExportContextPerformanceCsv(context_performance_path, snapshot, report);
