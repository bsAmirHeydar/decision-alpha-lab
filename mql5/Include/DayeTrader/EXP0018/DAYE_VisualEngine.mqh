...
public:
   ...
   {
      ZeroMemory(m_summary); m_initialized=false; m_last_visual_refresh_utc=0;
   }
   ...
   {
      m_config=visual_config; m_time_config=time_config;
      string reason="";
      if(!ValidateConfig(reason)) { Print("EXP0018 P10 invalid visual config reason=",reason); return false; }
      if(run_self_tests && !DAYE_RunEmbeddedVisualSelfTests()) return false;
      if(!m_render_engine.Initialize(render_config,time_config,run_self_tests,write_p08_audit,p08_audit_filename)) return false;
      m_initialized=true;
      Print("EXP0018 P10 Unified Visual Anatomy initialized. One source pipeline, all Core visual layers, no trading authority.");
                ...
                const bool show_chart_comment)
   {
      ...
      bool has_render_summary=m_render_engine.GetCurrentSummary(render_summary);
      DAYE_PairedPeriodSnapshot periods[];
      int period_count=m_render_engine.ExportSourcePeriods(periods);
      ...
      if(period_count<=0)
      {
         m_summary.status=DAYE_VISUAL_STATUS_SOURCE_NOT_READY; m_summary.reason_code="no_source_periods_exported";
      ...
      long charts[]; m_summary.target_chart_count=ResolveTargetCharts(symbol_a,symbol_b,charts);
      if(m_summary.target_chart_count<=0)
      {
         m_summary.status=DAYE_VISUAL_STATUS_WAITING_FOR_CHART; m_summary.reason_code="no_open_symbol_chart";
         if(print_visual_summary) Print(FormatSummary());
         return true;
      }
      datetime lookback_start_utc=processing_time_utc-(datetime)(m_config.lookback_weeks*7*86400);
      for(int i=0;i<ArraySize(charts);i++) ProcessChart(charts[i],periods,lookback_start_utc);
   ...
   void Shutdown(void)
   {
      ...
      Comment(""); m_render_engine.Shutdown(); ZeroMemory(m_summary); m_initialized=false; m_last_visual_refresh_utc=0;
   }
};
