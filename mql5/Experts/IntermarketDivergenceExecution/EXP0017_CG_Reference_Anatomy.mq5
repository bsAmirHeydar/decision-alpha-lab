...
#property description "EXP0017 Phase 02 - Cycle Group Reference Field Anatomy"
#property description "No trading, no hunt, no divergence. This expert only builds same-day CG reference high/low fields for two symbols."

#include <IntermarketDivergenceExecution/CG/CGR_Engine.mqh>
...
input int  InpMaxReferencesPerGroupShown = 2;
input bool InpShowOnlyGroupsWithReadyReferences = false;
...
int OnInit()
{
   ...
   time_config.max_previous_cycles_shown=InpMaxReferencesPerGroupShown;

   SCGRReferenceConfig ref_config;
   ref_config.symbol_a=InpSymbolA;
   ref_config.symbol_b=InpSymbolB;
   ref_config.broker_utc_offset_hours=InpBrokerUtcOffsetHours;
   ref_config.max_groups_shown=InpMaxGroupsShown;
   ref_config.max_references_per_group_shown=InpMaxReferencesPerGroupShown;
   ref_config.require_m1_history=InpRequireM1History;
   ref_config.show_only_groups_with_ready_references=InpShowOnlyGroupsWithReadyReferences;
   ...
   g_engine.Init(time_config,ref_config,enabled,InpShowChartPanel,InpPrintSummaryOnNewMinute);
   ...
   Print("EXP0017 Phase 02 Reference Field Anatomy initialized. No trading, no hunt, no divergence. Symbols: ",InpSymbolA," / ",InpSymbolB);
...
void OnTick()
{
   // Reference field is refreshed by OnTimer to keep the phase deterministic and light.
}
