...
def require_identifier(v,n):
    if not isinstance(v,str) or not re.fullmatch(r"[A-Za-z0-9_.:-]+",v): raise FPI08Error("FP_WRC_IDENTIFIER_INVALID",f"{n} invalid")
    return v
def require_sha256(v,n):
    if not isinstance(v,str) or not re.fullmatch(r"[0-9a-f]{64}",v): raise FPI08Error("FP_WRC_SHA256_INVALID",f"{n} must be lowercase sha256")
    return v
def require_semver(v,n):
    if not isinstance(v,str) or not re.fullmatch(r"\d+\.\d+\.\d+",v): raise FPI08Error("FP_WRC_SEMVER_INVALID",f"{n} invalid")
    return v
