...
def hash_unit(seed: str) -> float:
    return int(hashlib.sha256(seed.encode()).hexdigest()[:16],16) / float(0xFFFFFFFFFFFFFFFF)

def hash_signed(seed: str) -> float:
    return hash_unit(seed)*2.0-1.0
