...
def q(v:Any)->float:return float(dec(v).quantize(Q,rounding=ROUND_HALF_EVEN))
...
def hash_chain(records:list[dict],prefix:str)->list[dict]:
 ...
 for i,r in enumerate(records):
  x=deepcopy(r); x["ordinal"]=i; x["previous_hash"]=prev; x["event_id"]=stable_id(prefix,x); x["event_hash"]=content_hash(x); prev=x["event_hash"]; out.append(x)
 return out
