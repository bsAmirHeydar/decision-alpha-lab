...
def hash_chain(records:list[dict],prefix:str)->list[dict]:
    ...
    for ordinal,record in enumerate(records):
        item=deepcopy(record); item["ordinal"]=ordinal; item["previous_event_hash"]=previous
        item["event_id"]=stable_id(prefix,item); item["event_hash"]=content_hash(item); previous=item["event_hash"]; out.append(item)
    return out
