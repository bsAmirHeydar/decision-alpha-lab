...
class DataSourceConfig:
    ...
    primary_tick_jsonl: Path
    secondary_tick_jsonl: Path
    timezone: str
    price_basis: str
...
def load_activation_config(path: str | Path) -> ActivationConfig:
    ...
    if source.get("mode") != "PAIRED_TICK_JSONL":
        raise ValueError("only PAIRED_TICK_JSONL is permitted by this activation release")
    ...
    data_source = DataSourceConfig(
        ...
        primary_tick_jsonl=_path(source["primary_tick_jsonl"], "primary_tick_jsonl", base),
        secondary_tick_jsonl=_path(source["secondary_tick_jsonl"], "secondary_tick_jsonl", base),
        ...
        max_confirmation_freshness_ms=int(source.get("max_confirmation_freshness_ms", 900_000)),
    )
    if data_source.primary_symbol == data_source.secondary_symbol:
        raise ValueError("primary and secondary symbols must differ")
    for tick_path in (data_source.primary_tick_jsonl, data_source.secondary_tick_jsonl):
    ...
    total = sum((
        split_policy.oof_train_fraction,
        split_policy.oof_calibration_fraction,
        split_policy.oof_threshold_fraction,
        split_policy.oof_holdout_fraction,
        split_policy.final_train_fraction,
        split_policy.final_calibration_fraction,
        split_policy.final_threshold_fraction,
        split_policy.final_test_fraction,
    ))
    if abs(total - 1.0) > 1e-9:
        raise ValueError(f"split fractions must sum to one, got {total}")
    resource = raw.get("resource_policy", {})
    resource_policy = ResourcePolicy(
        seed=int(resource.get("seed", 1701)),
        max_rows=int(resource.get("max_rows", 2_000_000)),
        max_features=int(resource.get("max_features", 10_000)),
        max_memory_mb=int(resource.get("max_memory_mb", 8192)),
        max_wall_seconds=float(resource.get("max_wall_seconds", 3600.0)),
        max_workers=int(resource.get("max_workers", 1)),
    )
    selected = tuple(str(x) for x in raw.get("selected_task_ids", ()))
    families = tuple(str(x) for x in raw.get("family_filter", ()))
    return ActivationConfig(
        schema_version="1.0.0",
        run_id=str(raw["run_id"]),
        output_root=_path(raw["output_root"], "output_root", base),
