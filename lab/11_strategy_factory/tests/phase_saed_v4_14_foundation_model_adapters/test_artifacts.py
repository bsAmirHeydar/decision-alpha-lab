...
def test_artifact_exists_and_json(root,name):
 p=root/'lab/11_strategy_factory/artifacts/saed_v4_14'/name;assert p.is_file();json.loads(p.read_text())
def test_claims_conservative(load):
 c=load('lab/11_strategy_factory/artifacts/saed_v4_14/CLAIM_LEDGER.JSON')['claims'];assert c['foundation_model_adapter_boundary_implemented'] and not c['real_external_model_loaded'] and not c['production_authorization']
def test_conformance_passed(load):assert load('lab/11_strategy_factory/artifacts/saed_v4_14/CONFORMANCE_RESULTS.JSON')['passed']
