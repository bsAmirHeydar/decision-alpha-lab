...
def test_cli(root):
 r=subprocess.run([sys.executable,'-m','saed_v4_causal_treatment_policy_value.cli','--summary'],cwd=root,env={**__import__('os').environ,'PYTHONPATH':str(root/'lab/11_strategy_factory/python')},capture_output=True,text=True);assert r.returncode==0;rj=json.loads(r.stdout);assert rj['phase']=='SAED_V4_18' and rj['next_phase']=='SAED_V4_19' and not rj['production_authority']
