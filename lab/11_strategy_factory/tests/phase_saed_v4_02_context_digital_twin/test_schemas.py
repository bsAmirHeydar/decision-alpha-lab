...
def test_all_schemas_closed(root):
 files=sorted((root/'lab/11_strategy_factory/schemas/saed_v4_02').glob('*.schema.json'));assert len(files)>=28
 for p in files:
...
def test_unknown_field_rejected(root,name):
 s=json.loads((root/f'lab/11_strategy_factory/schemas/saed_v4_02/{name}.schema.json').read_text());assert s['additionalProperties'] is False
