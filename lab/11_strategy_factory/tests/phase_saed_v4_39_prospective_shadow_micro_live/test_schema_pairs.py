...
import pytest
ROOT=Path(__file__).resolve().parents[4];EX=ROOT/'lab/11_strategy_factory/examples/saed_v4_39';SC=ROOT/'lab/11_strategy_factory/schemas/saed_v4_39'
...
def validate(v,s):
 ...
 if t=='object':
  ...
  if 'prefixItems' in s:
   assert len(v)==len(s['prefixItems'])
   for x,ss in zip(v,s['prefixItems']):validate(x,ss)
  else:
