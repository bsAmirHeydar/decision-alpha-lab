...
def test_unknown_field_rejected(root):
 m=json.loads((root/'lab/11_strategy_factory/artifacts/saed_v4_12/CONTRACT_VALIDATION_MAP.JSON').read_text());p=m['pairs'][0];s=json.loads((root/p['schema']).read_text());d=json.loads((root/p['document']).read_text());d['unknown_field']=1
