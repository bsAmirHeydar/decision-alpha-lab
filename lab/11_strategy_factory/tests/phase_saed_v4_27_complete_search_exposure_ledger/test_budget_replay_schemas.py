...
def test_all_schema_pairs_validate():
    root=Path(__file__).resolve().parents[4]; ex=root/"lab/11_strategy_factory/examples/saed_v4_27"; ar=root/"lab/11_strategy_factory/artifacts/saed_v4_27"; sc=root/"lab/11_strategy_factory/schemas/saed_v4_27"
...
def test_schema_unknown_root_rejected():
    root=Path(__file__).resolve().parents[4]; p=root/"lab/11_strategy_factory/artifacts/saed_v4_27/GOLDEN_COMPLETENESS_AUDIT.JSON"; s=root/"lab/11_strategy_factory/schemas/saed_v4_27/GOLDEN_COMPLETENESS_AUDIT.SCHEMA.JSON"
