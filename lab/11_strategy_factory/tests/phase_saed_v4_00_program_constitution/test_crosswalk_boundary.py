...
def test_snapshot_stable(tmp_path):
 p=tmp_path/'lab/11_strategy_factory/python/strategy_factory_x'; p.mkdir(parents=True); (p/'a.py').write_text('x')
...
def test_snapshot_detects_modify(tmp_path):
 p=tmp_path/'lab/11_strategy_factory/python/strategy_factory_x'; p.mkdir(parents=True); f=p/'a.py'; f.write_text('x'); a=capture_snapshot(tmp_path); f.write_text('y'); b=capture_snapshot(tmp_path); assert compare_snapshots(a,b)['status']=='reject'
