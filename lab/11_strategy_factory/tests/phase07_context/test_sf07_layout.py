...
def test_no_unsupported_long_to_string():
    text='\n'.join(p.read_text(encoding='utf-8',errors='ignore') for p in list(MQL.rglob('*.mqh'))+list((ROOT/'mql5/Experts/StrategyFactory').glob('SF07_*.mq5'))+list((ROOT/'mql5/Experts/StrategyFactoryTests').glob('SF07_*.mq5')))
...
def test_docs_and_status_exist():
    ...
    assert (ROOT/'lab/11_strategy_factory/implementation_program/phase_status/PHASE_07.json').exists()
