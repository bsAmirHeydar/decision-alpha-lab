...
def artifact_file(tmp_path):
    p=tmp_path/"lab/11_strategy_factory/contexts/ctx-test/doctrine/context.md"; p.parent.mkdir(parents=True); p.write_text("context\n",encoding="utf-8"); return p
...
def descriptor(artifact_file,tmp_path):
    ...
    return ArtifactDescriptor(ArtifactIdentity(aid,"alpha","research","context_doctrine","ctx-test",version,digest_file(artifact_file)),"context_authored","lab/11_strategy_factory/contexts/ctx-test/doctrine/context.md","domain","OWNER_TEST","schema:test:1","CONFIDENTIAL",ArtifactMutability.AUTHORED,metadata={"interfaces":{"context.read":"1.0.0"},"schema_semver":"1.0.0","visibility":"PUBLIC"})
