...
def test_cli_allows_valid_bundle(tmp_path:Path):
    p=subprocess.run([sys.executable,"-m","tools.strategy_factory.acl_os.acl_00.cli","evaluate",str(FIX/"valid_semantic_transition.json"),"--ledger",str(tmp_path/"audit.jsonl"),"--as-of","2026-07-17T12:30:00Z"],cwd=REPO_ROOT,text=True,capture_output=True); assert p.returncode==0,p.stderr; assert json.loads(p.stdout)["decision"]=="ALLOW"
