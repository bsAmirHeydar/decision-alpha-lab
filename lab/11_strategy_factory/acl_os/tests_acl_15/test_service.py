...
def test_reference_fixture(root): assert verify_output(root/'lab/11_strategy_factory/acl_os/fixtures/acl_15/reference_fleet_closure')['passed']
def test_output_has_zero_execution(root):
 d=json.loads((root/'lab/11_strategy_factory/acl_os/fixtures/acl_15/reference_fleet_closure/execution/runtime_and_order_manifest.json').read_text()); assert sum(d[k] for k in ['pilot_execution_count','runtime_generation_count','live_order_count','capital_activation_count'])==0
def test_output_handoff_closed(root):
 d=json.loads((root/'lab/11_strategy_factory/acl_os/fixtures/acl_15/reference_fleet_closure/handoff/lifecycle_closure_handoff.json').read_text()); assert d['program_reference_lifecycle_closed'] is True
