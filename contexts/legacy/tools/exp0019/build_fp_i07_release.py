...
STATUS = 'contexts/legacy/infrastructure/exp0019_faerie_protocol/phase_i07'
...
def owned(root: Path) -> list[Path]:
    ...
    for rel in (
        STATUS,
        'docs/operations/execution/EXP0019_faerie_protocol_contextual_divergence/implementation_program/phase_deliveries/fp_i07',
        ...
        if base.exists():
            for path in base.rglob('*'):
                if path.is_file() and '__pycache__' not in path.parts and '.pytest_cache' not in path.parts and path.suffix != '.pyc':
                    paths.add(path)
    for path in (root / 'docs/history/obsidian/deep/01_concepts').glob('FP-I07_*.md'):
        paths.add(path)
    for rel in (
        ...
        'mql5/Tests/Experts/EXP0019/FaerieProtocol/EXP0019_FP_I07_ConfirmationSelfTest.mq5',
        'contexts/legacy/tools/exp0019/check_fp_i07_boundaries.py',
        'contexts/legacy/tools/exp0019/check_fp_i07_mql5_static.py',
        'contexts/legacy/tools/exp0019/validate_fp_i07_delivery.py',
        'contexts/legacy/tools/exp0019/build_fp_i07_release.py',
    ):
        path = root / rel
        if path.is_file():
            paths.add(path)
