...
META={'EXP0019_FP_I05_FILE_INDEX.txt','EXP0019_FP_I05_FILE_HASHES.sha256','EXP0019_FP_I05_PATCH_MANIFEST.json'}
...
def main():
    ...
    (root/'EXP0019_FP_I05_FILE_INDEX.txt').write_text('\n'.join(p.relative_to(root).as_posix() for p in final)+'\n',encoding='utf-8')
    (root/'EXP0019_FP_I05_PATCH_MANIFEST.json').write_text(json.dumps(manifest,indent=2,sort_keys=True)+'\n',encoding='utf-8')
    final=owned(root)
    (root/'EXP0019_FP_I05_FILE_HASHES.sha256').write_text('\n'.join(f'{digest(p)}  {p.relative_to(root).as_posix()}' for p in final if p.name!='EXP0019_FP_I05_FILE_HASHES.sha256')+'\n',encoding='utf-8')
    final=owned(root);manifest['file_count']=len(final);(root/'EXP0019_FP_I05_PATCH_MANIFEST.json').write_text(json.dumps(manifest,indent=2,sort_keys=True)+'\n',encoding='utf-8')
    if args.zip:
