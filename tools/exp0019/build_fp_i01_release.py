...
META={'EXP0019_FP_I01_FILE_INDEX.txt','EXP0019_FP_I01_FILE_HASHES.sha256','EXP0019_FP_I01_PATCH_MANIFEST.json'}
...
def main():
 ...
 (root/'EXP0019_FP_I01_FILE_INDEX.txt').write_text('\n'.join(rels)+'\n')
 ...
 (root/'EXP0019_FP_I01_PATCH_MANIFEST.json').write_text(json.dumps(manifest,indent=2,sort_keys=True)+'\n')
 paths=owned(root);(root/'EXP0019_FP_I01_FILE_INDEX.txt').write_text('\n'.join(p.relative_to(root).as_posix() for p in paths)+'\n')
 paths=owned(root);(root/'EXP0019_FP_I01_FILE_HASHES.sha256').write_text('\n'.join(f'{h(p)}  {p.relative_to(root).as_posix()}' for p in paths if p.name!='EXP0019_FP_I01_FILE_HASHES.sha256')+'\n')
 if a.zip:
