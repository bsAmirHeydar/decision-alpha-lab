...
STATUS='lab/10_infrastructure/EXP0019_faerie_protocol/phase_i04';META={'releases/history/exp0019/indexes/EXP0019_FP_I04_FILE_INDEX.txt','releases/history/exp0019/hashes/EXP0019_FP_I04_FILE_HASHES.sha256','releases/history/exp0019/manifests/EXP0019_FP_I04_PATCH_MANIFEST.json'}
...
def main():
    ...
    (root/'releases/history/exp0019/indexes/EXP0019_FP_I04_FILE_INDEX.txt').write_text('\n'.join(p.relative_to(root).as_posix() for p in final)+'\n',encoding='utf-8')
    (root/'releases/history/exp0019/manifests/EXP0019_FP_I04_PATCH_MANIFEST.json').write_text(json.dumps(manifest,indent=2,sort_keys=True)+'\n',encoding='utf-8')
    final=owned(root)
    (root/'releases/history/exp0019/hashes/EXP0019_FP_I04_FILE_HASHES.sha256').write_text('\n'.join(f'{digest(p)}  {p.relative_to(root).as_posix()}' for p in final if p.name!='releases/history/exp0019/hashes/EXP0019_FP_I04_FILE_HASHES.sha256')+'\n',encoding='utf-8')
    final=owned(root)
    manifest['file_count']=len(final);(root/'releases/history/exp0019/manifests/EXP0019_FP_I04_PATCH_MANIFEST.json').write_text(json.dumps(manifest,indent=2,sort_keys=True)+'\n',encoding='utf-8')
    if args.zip:
