...
mods={p.name for p in (root/f'{phase}/python/fp_i05_reference').glob('*.py')}
...
if len(tests)!=11:errors.append(f'test module count {len(tests)} != 11')
...
if len(docs)!=35:errors.append(f'FP-I05 doc count {len(docs)} != 35')
for path in docs:
...
status_path=root/f'{phase}/artifacts/FP_I05_PHASE_STATUS.json'
if status_path.exists():
    d=json.loads(status_path.read_text())
    expected={'python_module_count':17,'phase_test_count':51,'cumulative_fp_test_count':330,'previous_context_regression_count':50,'public_schema_count':12,'public_contract_count':12,'mql5_include_count':9,'mql5_entrypoint_count':2,'delivery_doc_count':35,'atomic_concept_count':7,'conformance_check_count':5}
...
print(f'FP-I05 delivery validation PASS: {len(mods)} Python modules, {len(tests)} test modules, {len(schemas)} schemas, {len(docs)} docs, 7 concepts, 12 contracts, 9 MQL5 includes, no runtime authority.')
