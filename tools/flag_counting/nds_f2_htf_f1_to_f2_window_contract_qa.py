...
def main() -> int:
    ...
    require(phase, 'NDS-F2-HTF-F-PHASE-02', "phase module version", errors)
    ...
    require(phase, "e.lifecycle_can_spawn_f2", "F1 lifecycle authority", errors)
    require(phase, "e.f2_can_spawn_f3", "F2 lifecycle authority", errors)
    require(phase, "events[i].parent_event_id != f1.event_id", "direct parent identity", errors)
    require(phase, "events[i].parent_sequence_id != f1.sequence_id", "parent sequence identity", errors)
    require(phase, "e.sequence_id != selected.sequence_id", "same sequence identity", errors)
    require(phase, "e.scale_L != selected.scale_L", "same scale identity", errors)
    require(phase, "FP_NDSF2HTFEvaluateF1ToF2ConfirmationWindow", "window evaluation", errors)
