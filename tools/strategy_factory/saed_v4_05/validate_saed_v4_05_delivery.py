...
index = [line.strip() for line in (ROOT / "SAED_V4_05_FILE_INDEX.txt").read_text(encoding="utf-8").splitlines() if line.strip()]
...
expected = {}
for line in (ROOT / "SAED_V4_05_FILE_HASHES.sha256").read_text(encoding="utf-8").splitlines():
    if not line.strip():
        continue
...
manifest = json.loads((ROOT / "SAED_V4_05_PATCH_MANIFEST.json").read_text(encoding="utf-8"))
...
if set(expected) != set(index) - {"SAED_V4_05_FILE_HASHES.sha256"}:
