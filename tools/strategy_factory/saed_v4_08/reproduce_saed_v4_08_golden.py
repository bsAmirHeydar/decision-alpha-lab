...
import json,sys
ROOT=Path(__file__).resolve().parents[3];PY=ROOT/'lab/11_strategy_factory/python';sys.path.insert(0,str(PY))
...
lattice=load('lab/11_strategy_factory/artifacts/saed_v4_07/GOLDEN_ACTION_LATTICE.JSON');handoff=load('lab/11_strategy_factory/artifacts/saed_v4_07/V4_07_TO_V4_08_HANDOFF.JSON');ctx=ContextSnapshot.from_mapping(load('lab/11_strategy_factory/examples/saed_v4_08/golden_context_snapshot.json'));obs=[PriceObservation.from_mapping(x) for x in load('lab/11_strategy_factory/examples/saed_v4_08/golden_market_path.json')['observations']];pol=OutcomePolicy.from_mapping(load('lab/11_strategy_factory/examples/saed_v4_08/outcome_policy.json'));reg=CostRegistry.from_mapping(load('lab/11_strategy_factory/examples/saed_v4_08/cost_registry.json'));cube=build_cube(lattice,handoff,ctx,obs,pol,reg,'fixed_conservative','1.0.0')
out=ROOT/'lab/11_strategy_factory/artifacts/saed_v4_08';out.mkdir(parents=True,exist_ok=True)
