...
import hashlib,json
ROOT=Path(__file__).resolve().parents[3];index=[x.strip() for x in (ROOT/'SAED_V4_08_FILE_INDEX.txt').read_text().splitlines() if x.strip()]
...
expected={}
for line in (ROOT/'SAED_V4_08_FILE_HASHES.sha256').read_text().splitlines():
...
m=json.loads((ROOT/'SAED_V4_08_PATCH_MANIFEST.json').read_text())
if m['file_count']!=len(index) or m['hash_count']!=len(expected):raise SystemExit('manifest count mismatch')
if set(expected)!=set(index)-{'SAED_V4_08_FILE_HASHES.sha256'}:raise SystemExit('hash ledger coverage mismatch')
