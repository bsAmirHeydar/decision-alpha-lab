...
INDEX=ROOT/"SAED_V4_28_FILE_INDEX.txt"; LEDGER=ROOT/"SAED_V4_28_FILE_HASHES.sha256"; MANIFEST=ROOT/"SAED_V4_28_PATCH_MANIFEST.json"; QA=ROOT/"SAED_V4_28_QA_REPORT.json"; INVENTORY=ROOT/"SAED_V4_28_ARTIFACT_INVENTORY.csv"
...
assert set(expected)==set(paths)-{"SAED_V4_28_FILE_HASHES.sha256"}
for path,digest in expected.items(): assert hashlib.sha256((ROOT/path).read_bytes()).hexdigest()==digest,path
manifest=json.loads(MANIFEST.read_text(encoding="utf-8")); qa=json.loads(QA.read_text(encoding="utf-8")); status=json.loads((ROOT/"lab/11_strategy_factory/phase_status/SAED_V4_28.json").read_text(encoding="utf-8"))
...
A=ROOT/"lab/11_strategy_factory/artifacts/saed_v4_28"
