...
ROOT=Path(__file__).resolve().parents[2];paths=[x.strip() for x in (ROOT/'releases/history/ucee/indexes/UCEE_I15_FILE_INDEX.txt').read_text().splitlines() if x.strip()]
h=[]
for rel in paths:
