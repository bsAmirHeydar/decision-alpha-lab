...
import re,sys
root=Path(sys.argv[1] if len(sys.argv)>1 else '.').resolve();targets=list((root/'mql5/Include/AlphaLab/StrategyFactory/ClassicalAlgorithms').rglob('*.mqh'))+[root/'mql5/Experts/StrategyFactory/UCE_I08_ClassicalAlgorithmsDiagnostic.mq5',root/'mql5/Experts/StrategyFactoryTests/UCE_I08_ClassicalAlgorithmsSelfTest.mq5',root/'mql5/Experts/StrategyFactoryTests/UCE_I08_CatalogParitySelfTest.mq5'];errors=[]
for f in targets:
...
print(f'UCE-I08 MQL5 static PASS: {len(targets)} files, local includes, delimiters, compatibility, and no-authority checks')
