...
import hashlib,json
ROOT=Path(__file__).resolve().parents[3];index=[x.strip() for x in (ROOT/'releases/history/saed/indexes/SAED_V4_07_FILE_INDEX.txt').read_text().splitlines() if x.strip()]
...
expected={}
for line in (ROOT/'releases/history/saed/hashes/SAED_V4_07_FILE_HASHES.sha256').read_text().splitlines():
...
m=json.loads((ROOT/'releases/history/saed/manifests/SAED_V4_07_PATCH_MANIFEST.json').read_text());
if m['file_count']!=len(index) or m['hash_count']!=len(expected):raise SystemExit('manifest count mismatch')
if set(expected)!=set(index)-{'releases/history/saed/hashes/SAED_V4_07_FILE_HASHES.sha256'}:raise SystemExit('hash ledger coverage mismatch')
