...
import json,hashlib
ROOT=Path(__file__).resolve().parents[2];errors=[];index=ROOT/'UCEE_I15_FILE_INDEX.txt'
paths=[x.strip() for x in index.read_text().splitlines() if x.strip()] if index.exists() else []
if not index.exists():errors.append('missing UCEE_I15_FILE_INDEX.txt')
...
hp=ROOT/'UCEE_I15_FILE_HASHES.sha256'
if not hp.exists():errors.append('missing UCEE_I15_FILE_HASHES.sha256')
else:
 declared={}
 for line in hp.read_text().splitlines():
  if line.strip():digest,rel=line.split('  ',1);declared[rel]=digest
 expected=set(paths)-{'UCEE_I15_FILE_HASHES.sha256'}
