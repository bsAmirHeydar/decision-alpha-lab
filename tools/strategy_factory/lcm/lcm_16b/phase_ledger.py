        ...
        rows.append(
            {
                ...
                "tool_module": (
                    (repo_root / "tools/strategy_factory/lcm" / token.lower()).relative_to(repo_root).as_posix()
                    if (repo_root / "tools/strategy_factory/lcm" / token.lower()).is_dir()
                    else None
                ),
                "direct_tests": (
                    (repo_root / "lab/11_strategy_factory/migration" / f"tests_{token.lower()}").relative_to(repo_root).as_posix()
                    if (repo_root / "lab/11_strategy_factory/migration" / f"tests_{token.lower()}").is_dir()
                    else None
                ),
            }
        )
