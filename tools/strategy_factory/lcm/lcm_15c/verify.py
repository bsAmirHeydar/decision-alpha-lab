...
def verify_package(repo_root:Path,package_root:Path)->VerificationResult:
 ...
 if not pathspec.is_file() or pathspec.read_bytes()!=b'' or file_digest(pathspec)!=EMPTY_SHA256:raise ValueError('deletion pathspec is not exact empty set')
 ...
 if len(lock_rows)!=2168 or len(blocked_rows)!=2168 or deletion_rows:raise ValueError('record count mismatch')
 ...
 if handoff['source_handoff_digest']!=EXPECTED_UPSTREAM or handoff['deleted_path_count']!=0:raise ValueError('handoff mismatch')
 if handoff['allowed_next_actions']!=['LCM16A_FULL_SYSTEM_AUDIT']:raise ValueError('next action mismatch')
