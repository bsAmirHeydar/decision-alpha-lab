...
def suite_matrix() -> tuple[Suite, ...]:
    ...
    migration = "lab/11_strategy_factory/migration"
    for name in (
    ...
    for phase in range(16):
        suites.append(
            _pytest(
                f"ACL_OS_{phase:02d}",
                f"lab/11_strategy_factory/acl_os/tests_acl_{phase:02d}",
                extra=("lab/11_strategy_factory/python",),
            )
        )
    ...
    for suite_id, directory in core:
        suites.append(_pytest(
            suite_id,
            ".",
            cwd=f"lab/11_strategy_factory/tests/{directory}",
            extra=("lab/11_strategy_factory/python",),
        ))
    ...
    for suite_id, directory in (
        ...
        suites.append(_pytest(
            suite_id,
            f"lab/11_strategy_factory/tests/{directory}",
            extra=("lab/11_strategy_factory/python",),
        ))

    suites.append(_pytest(
        "FP_I15_AND_NDS_864",
        "lab/10_infrastructure/EXP0019_faerie_protocol/phase_i15/tests",
        extra=(
            "lab/11_strategy_factory/python",
            "lab/10_infrastructure/EXP0019_faerie_protocol/phase_i15/python",
        ),
    ))
    suites.append(_pytest(
        "NDS_864_REFERENCE",
