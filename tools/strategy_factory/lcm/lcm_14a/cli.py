...
def main(argv=None) -> int:
    ...
    command.add_argument("--deprecation-root", required=True)
    ...
    if args.command == "verify-patch":
        ...
        errors = verify_package(Path(args.deprecation_root))
        ...
        errors += ["INDEX_MISSING:" + relative for relative in index if not (Path(args.repo_root) / relative).is_file()]
    else:
        errors = run_qa(Path(args.deprecation_root), Path(args.schema_root), Path(args.module_root))
    if errors:
        print("\n".join(errors))
        return 1
    print("PASS")
