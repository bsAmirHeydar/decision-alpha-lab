...
def role_for(row, caps: set[str]):
    ...
    if fam in PROTECTED_FAMILIES or p.startswith('lab/11_strategy_factory/acl_os/') or p.startswith('tools/strategy_factory/lcm/'):
...
def classify(row, caps: set[str], entry: set[str], inbound: int, duplicate: bool):
    ...
    protected=row['family_candidate'] in PROTECTED_FAMILIES or row['path'].startswith(('lab/11_strategy_factory/acl_os/','tools/strategy_factory/lcm/','registry/legacy_context_migration/'))
