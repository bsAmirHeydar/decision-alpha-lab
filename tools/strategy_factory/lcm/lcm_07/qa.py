...
def run(repo_root:Path,shared_engine_root:Path):
    package=verify_package(shared_engine_root);schemas=validate(repo_root/"registry/legacy_context_migration/lcm_07/schemas/v1",shared_engine_root);findings=scan(repo_root/"tools/strategy_factory/lcm/lcm_07")
