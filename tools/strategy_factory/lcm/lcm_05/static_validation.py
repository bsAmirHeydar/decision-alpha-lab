...
def validate(repo_root):
    policies=list((repo_root/'registry/legacy_context_migration/lcm_05/policies').glob('*.json'));[json.loads(p.read_text(encoding='utf-8')) for p in policies];forbidden=[]
    for p in (repo_root/'tools/strategy_factory/lcm/lcm_05').glob('*.py'):
