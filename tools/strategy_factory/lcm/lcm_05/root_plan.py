...
def map_root(row):
    ...
    elif 'COMMIT' in cls:outcome='PROPOSE_RELOCATION';target=f"registry/releases/commit_records/{sha[:12]}_{name}"
    ...
    elif 'RELEASE' in cls or 'MANIFEST' in name.upper() or 'QA_REPORT' in name.upper() or 'FILE_HASH' in name.upper() or 'FILE_INDEX' in name.upper():outcome='PROPOSE_RELOCATION';target=f"registry/releases/legacy_root/{sha[:16]}/{name}"
    elif row.get('review_required','').lower()=='true':outcome='PROPOSE_QUARANTINE';target=f"lab/11_strategy_factory/migration/quarantine/root_review/{sha[:16]}/{name}"
    else:outcome='PROPOSE_RELOCATION';target=f"registry/releases/program_artifacts/{sha[:16]}/{name}"
    validate(target)
