...
def _role_base(rec):return PACKAGE_ROOTS.get(rec['artifact_role'],'lab/11_strategy_factory/migration/quarantine/unknown_role')
def map_artifact(rec,identity_by_path):
    ...
    elif disp=='QUARANTINE_UNCERTAIN':outcome='PROPOSE_QUARANTINE';target=f"lab/11_strategy_factory/migration/quarantine/uncertain/{family}/{sha[:16]}/{unique}_{name}"
    ...
    elif disp=='MERGE_AFTER_EQUIVALENCE_PROOF':outcome='BLOCKED_EQUIVALENCE_REVIEW';target=f"lab/11_strategy_factory/migration/quarantine/equivalence_pending/{family}/{sha[:16]}/{unique}_{name}"
