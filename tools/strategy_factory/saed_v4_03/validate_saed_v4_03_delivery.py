...
idx=[x.strip() for x in (ROOT/'releases/history/saed/indexes/SAED_V4_03_FILE_INDEX.txt').read_text().splitlines() if x.strip()]
missing=[p for p in idx if not (ROOT/p).is_file()]
if missing:raise SystemExit('missing indexed files: '+','.join(missing))
expected={}
for line in (ROOT/'releases/history/saed/hashes/SAED_V4_03_FILE_HASHES.sha256').read_text().splitlines():
...
manifest=json.loads((ROOT/'releases/history/saed/manifests/SAED_V4_03_PATCH_MANIFEST.json').read_text())
