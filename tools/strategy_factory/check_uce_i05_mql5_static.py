...
import sys,re
root=Path(sys.argv[1] if len(sys.argv)>1 else '.').resolve(); files=list((root/'mql5/Include/AlphaLab/StrategyFactory/Economics').rglob('*.mqh'))+list((root/'mql5/Experts/StrategyFactory').glob('UCE_I05*.mq5'))+list((root/'mql5/Experts/StrategyFactoryTests').glob('UCE_I05*.mq5'))
errors=[]
for f in files:
