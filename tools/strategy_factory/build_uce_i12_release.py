...
STATUS='lab/11_strategy_factory/implementation_program/universal_context_exploitation_engine/v3_implementation'
SCHEMAS=(
...
META={'UCEE_I12_FILE_INDEX.txt','UCEE_I12_FILE_HASHES.sha256','UCEE_I12_PATCH_MANIFEST.json'}
def digest(p:Path)->str:return sha256(p.read_bytes()).hexdigest()
def owned(root:Path)->list[Path]:
    paths=set()
    for rel in ('lab/11_strategy_factory/python/strategy_factory_promotion_v3','lab/11_strategy_factory/tests/phase_uce_i12_promotion','lab/11_strategy_factory/examples/uce_i12','docs/strategy_factory_universal_context_exploitation_engine/implementation_program/phase_deliveries/uce_i12','mql5/Include/AlphaLab/StrategyFactory/StatisticalPromotion'):
        base=root/rel
        for p in base.rglob('*') if base.exists() else ():
            if p.is_file() and p.suffix!='.pyc' and '__pycache__' not in p.parts: paths.add(p)
    specific=['README_STRATEGY_FACTORY_UCEE_I12_IMPLEMENTATION.md','INSTALL_STRATEGY_FACTORY_UCEE_I12_IMPLEMENTATION.md','EXPAND_REMOVE_UCEE_I12_PATCH.ps1','COMMIT_MESSAGE.md','UCEE_I12_QA_REPORT.json','UCEE_I12_FILE_INDEX.txt','UCEE_I12_FILE_HASHES.sha256','UCEE_I12_PATCH_MANIFEST.json','lab/11_strategy_factory/python/pyproject.toml','lab/11_strategy_factory/test_vectors/v3/uce_i12_promotion_conformance_vectors.json','docs/strategy_factory_universal_context_exploitation_engine/implementation_program/phases/UCE_I12_STATISTICAL_AND_ANTI_OVERFIT_PROMOTION_GATE.md','mql5/Experts/StrategyFactory/UCE_I12_StatisticalPromotionDiagnostic.mq5','mql5/Experts/StrategyFactoryTests/UCE_I12_PromotionContractsSelfTest.mq5','mql5/Experts/StrategyFactoryTests/UCE_I12_NonCompensatoryGateSelfTest.mq5',f'{STATUS}/phase_status/UCE_I12.json',f'{STATUS}/phase_status/UCE_I12_HANDOFF_TO_UCE_I13.json',f'{STATUS}/artifacts/UCE_I12_ACCEPTANCE_EVIDENCE.json',f'{STATUS}/artifacts/UCE_I12_ARTIFACT_INVENTORY.csv','tools/strategy_factory/check_uce_i12_boundaries.py','tools/strategy_factory/check_uce_i12_mql5_static.py','tools/strategy_factory/compile_uce_i12_statistical_promotion.ps1','tools/strategy_factory/generate_uce_i12_vectors.py','tools/strategy_factory/run_uce_i12_tests.ps1','tools/strategy_factory/validate_uce_i12_delivery.py','tools/strategy_factory/build_uce_i12_release.py']
    specific += [f'lab/11_strategy_factory/schemas/v3/{n}.schema.json' for n in SCHEMAS]
...
def main()->int:
    ...
    (root/'UCEE_I12_FILE_INDEX.txt').write_text('\n'.join(rels)+'\n',encoding='utf-8')
    ...
    manifest={'patch_id':'decision-alpha-lab-ucee-i12-unified-statistical-anti-overfit-promotion-governance','patch_version':'1.0.0','phase_id':'UCE-I12','created_at_utc':generated,'title':'Unified Statistical, Anti-Overfit, and Promotion Governance Gate','authority_boundary':'offline statistical evidence and promotion governance only; no trading or network authority','python_module_count':18,'suite_count':7,'public_schema_count':len(SCHEMAS),'delivery_document_count':len(docs),'atomic_concept_count':len(concepts),'phase_test_count':a.phase_tests,'cumulative_ucee_test_count':a.cumulative_tests,'engineering_policy_check_count':a.engineering_checks,'mql5_file_count':11,'metaeditor_compile_status':'pending_local_windows','file_count':len(rels),'file_index':'UCEE_I12_FILE_INDEX.txt','file_hashes':'UCEE_I12_FILE_HASHES.sha256','next_phase':'UCE-I13 Manual AI Hybrid Policy Graph'}
    (root/'UCEE_I12_PATCH_MANIFEST.json').write_text(json.dumps(manifest,indent=2,sort_keys=True)+'\n',encoding='utf-8')
    paths=owned(root); rels=[p.relative_to(root).as_posix() for p in paths]; (root/'UCEE_I12_FILE_INDEX.txt').write_text('\n'.join(rels)+'\n',encoding='utf-8'); manifest['file_count']=len(rels); (root/'UCEE_I12_PATCH_MANIFEST.json').write_text(json.dumps(manifest,indent=2,sort_keys=True)+'\n',encoding='utf-8')
    paths=owned(root); (root/'UCEE_I12_FILE_HASHES.sha256').write_text('\n'.join(f'{digest(p)}  {p.relative_to(root).as_posix()}' for p in paths if p.name!='UCEE_I12_FILE_HASHES.sha256')+'\n',encoding='utf-8')
    if a.zip_path:
