...
def main():
 q=argparse.ArgumentParser();q.add_argument('root',nargs='?',default='.');q.add_argument('--verify-only',action='store_true');a=q.parse_args();root=Path(a.root).resolve();sys.path.insert(0,str(root/'lab/11_strategy_factory/python'))
 from strategy_factory_classical_v3 import run_conformance,ClassicalAlgorithmRegistry
 c=run_conformance();s=ClassicalAlgorithmRegistry().freeze().snapshot();payload=json.dumps(c,indent=2,sort_keys=True,default=default)+'\n';sp=json.dumps(asdict(s),indent=2,sort_keys=True,default=default)+'\n';targets=[(root/'lab/11_strategy_factory/test_vectors/v3/uce_i08_classical_conformance_vectors.json',payload),(root/'lab/11_strategy_factory/test_vectors/v3/uce_i08_algorithm_registry_snapshot.json',sp)]
 if a.verify_only:
