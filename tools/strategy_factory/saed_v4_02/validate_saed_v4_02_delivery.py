...
idx=[x.strip() for x in (ROOT/'SAED_V4_02_FILE_INDEX.txt').read_text().splitlines() if x.strip()]
missing=[p for p in idx if not (ROOT/p).is_file()]
if missing:raise SystemExit('missing indexed files: '+','.join(missing))
expected={}
for line in (ROOT/'SAED_V4_02_FILE_HASHES.sha256').read_text().splitlines():
...
manifest=json.loads((ROOT/'SAED_V4_02_PATCH_MANIFEST.json').read_text())
