...
ROOT=Path(__file__).resolve().parents[3];idx=ROOT/'SAED_V4_01_FILE_INDEX.txt';ledger=ROOT/'SAED_V4_01_FILE_HASHES.sha256';errors=[];files=[x.strip() for x in idx.read_text().splitlines() if x.strip()] if idx.exists() else []
...
missing=sorted(set(files)-set(expected)-{'SAED_V4_01_FILE_HASHES.sha256'});extra=sorted(set(expected)-set(files))
