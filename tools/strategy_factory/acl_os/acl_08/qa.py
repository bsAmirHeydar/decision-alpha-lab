...
def run_qa(repo_root:Path)->dict:
    reference=repo_root/'lab/11_strategy_factory/acl_os/fixtures/acl_08/reference_report'; static=validate_registry_files(); replay=verify_generated_root(reference); delivery=validate_delivery(repo_root)
    proc=subprocess.run([sys.executable,'-m','compileall','-q',str(repo_root/'tools/strategy_factory/acl_os/acl_08')],capture_output=True,text=True)
