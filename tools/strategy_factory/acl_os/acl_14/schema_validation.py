...
def load_schema(name:str)->dict:
    ...
    if not p.is_file(): raise ContractError(f'ACL14_SCHEMA_NOT_REGISTERED:{name}')
...
def validate_instance(name:str,instance:Any)->None:
    ...
    if errors: raise ContractError('ACL14_SCHEMA_REJECTED:'+'; '.join(e.message for e in errors[:6]))
