...
def run(root:Path)->dict:
    commands=[[sys.executable,'-m','compileall','-q',str(root/'tools/strategy_factory/acl_os/acl_15')],[sys.executable,'-m','pytest','-q',str(root/'lab/11_strategy_factory/acl_os/tests_acl_15')],[sys.executable,'-m','pytest','-q',str(root/'lab/11_strategy_factory/acl_os/tests_acl_14')]]
    results=[]
    for cmd in commands:
