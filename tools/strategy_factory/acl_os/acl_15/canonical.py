...
def _norm(v:Any)->Any:
    ...
    if isinstance(v,float):
        if not math.isfinite(v): raise ValueError('ACL15_NON_FINITE_NUMBER')
...
def stable_id(prefix:str,*parts:str,length:int=32)->str: return f"{prefix}_{hashlib.sha256('|'.join(parts).encode()).hexdigest()[:length].upper()}"
