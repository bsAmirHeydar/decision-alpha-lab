...
def validate(root:Path=REPO_ROOT)->dict:
    ...
    docs=root/"docs/alpha_lab_master_architecture/context_lifecycle_os/12_PHASE_DELIVERIES/ACL_03"; notes=list(docs.glob("*.md"));add(len(notes)>=45,"PHASE_DOCS",f"{len(notes)} phase notes")
    atomic=list((root/"docs/alpha_lab_master_architecture/context_lifecycle_os/13_ATOMIC_CONCEPTS/ACL_03").glob("*.md"));add(len(atomic)>=100,"ATOMIC_DOCS",f"{len(atomic)} atomic notes")
    mql=list((root/"lab/11_strategy_factory/mql5/Include/AlphaLab/ACL_OS/ACL03").glob("*.mqh"));add(len(mql)>=10,"MQL5_STATIC",f"{len(mql)} files")
    ...
    module=root/"tools/strategy_factory/acl_os/acl_03"; add((module/"service.py").is_file(),"MODULE_SERVICE","service exists")
    add((root/"lab/11_strategy_factory/acl_os/fixtures/acl_03/valid_context/context_manifest.yaml").is_file(),"VALID_FIXTURE","valid compiler fixture exists")
