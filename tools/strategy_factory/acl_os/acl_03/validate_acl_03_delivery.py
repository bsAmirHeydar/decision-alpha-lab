...
def main()->int:
    index=REPO_ROOT/"ACL_OS_03_FILE_INDEX.txt";ledger=REPO_ROOT/"ACL_OS_03_FILE_HASHES.sha256";errors=[];files=[]
    if not index.is_file() or not ledger.is_file(): errors.append("missing index or hash ledger")
        ...
        for rel in files:
            ...
            if rel in {"ACL_OS_03_FILE_HASHES.sha256","ACL_OS_03_ARTIFACT_INVENTORY.csv","ACL_OS_03_QA_REPORT.json"}: continue
