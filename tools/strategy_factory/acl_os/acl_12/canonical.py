...
def _norm(v:Any)->Any:
    ...
    if isinstance(v,float):
        if not math.isfinite(v): raise ValueError('ACL12_NON_FINITE_NUMBER')
...
def verify_embedded_digest(doc:dict[str,Any],field:str)->bool:
    val=doc.get(field)
    return isinstance(val,str) and DIGEST_RE.fullmatch(val) is not None and val==digest_object({k:v for k,v in doc.items() if k!=field})
