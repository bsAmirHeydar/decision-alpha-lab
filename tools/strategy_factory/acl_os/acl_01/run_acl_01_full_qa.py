...
def main()->int:
    proc=subprocess.run([sys.executable,"-m","pytest","-q","lab/11_strategy_factory/acl_os/tests_acl_01"],cwd=REPO_ROOT,text=True,capture_output=True)
    ...
    dump_json(REPO_ROOT/"ACL_OS_01_QA_REPORT.json",out); print(json.dumps(out,indent=2,sort_keys=True)); return 0 if out["passed"] else 1
