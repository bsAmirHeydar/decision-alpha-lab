...
ROOT=Path(__file__).resolve().parents[3]; files=sorted((ROOT/"mql5/Include/DecisionAlphaLab/StrategyFactory/SAED/V4_26").glob("*.mqh"))+sorted((ROOT/"mql5/Experts/DecisionAlphaLab/StrategyFactory/SAED/V4_26").glob("*.mq5")); assert len(files)==25,len(files)
