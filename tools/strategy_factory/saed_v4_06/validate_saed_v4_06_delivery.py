...
index=[x.strip() for x in (ROOT/'SAED_V4_06_FILE_INDEX.txt').read_text(encoding='utf-8').splitlines() if x.strip()]
...
expected={}
for line in (ROOT/'SAED_V4_06_FILE_HASHES.sha256').read_text(encoding='utf-8').splitlines():
    if line.strip():
...
manifest=json.loads((ROOT/'SAED_V4_06_PATCH_MANIFEST.json').read_text(encoding='utf-8'))
...
if set(expected)!=set(index)-{'SAED_V4_06_FILE_HASHES.sha256'}: raise SystemExit('hash ledger coverage mismatch')
