...
import re,sys
def main():
 root=Path(sys.argv[1] if len(sys.argv)>1 else '.').resolve(); owned=root/'mql5/Include/AlphaLab/StrategyFactory/TreatmentAtoms'; files=list(owned.glob('*.mqh'))+[root/'mql5/Experts/StrategyFactory/UCE_I03_TreatmentAtomsDiagnostic.mq5',root/'mql5/Experts/StrategyFactoryTests/UCE_I03_TreatmentAtomsSelfTest.mq5']; failures=[]
