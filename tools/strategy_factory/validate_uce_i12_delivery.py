...
PACKAGE_REL='lab/11_strategy_factory/python/strategy_factory_promotion_v3'
TEST_REL='lab/11_strategy_factory/tests/phase_uce_i12_promotion'
...
required=[
'README_STRATEGY_FACTORY_UCEE_I12_IMPLEMENTATION.md','INSTALL_STRATEGY_FACTORY_UCEE_I12_IMPLEMENTATION.md','EXPAND_REMOVE_UCEE_I12_PATCH.ps1','COMMIT_MESSAGE.md',
...
'lab/11_strategy_factory/test_vectors/v3/uce_i12_promotion_conformance_vectors.json',f'{DOC_REL}/00_UCE_I12_DELIVERY_MOC.md','docs/strategy_factory_universal_context_exploitation_engine/implementation_program/phases/UCE_I12_STATISTICAL_AND_ANTI_OVERFIT_PROMOTION_GATE.md',
'mql5/Include/AlphaLab/StrategyFactory/StatisticalPromotion/UCEI12_All.mqh','mql5/Experts/StrategyFactory/UCE_I12_StatisticalPromotionDiagnostic.mq5','mql5/Experts/StrategyFactoryTests/UCE_I12_PromotionContractsSelfTest.mq5','mql5/Experts/StrategyFactoryTests/UCE_I12_NonCompensatoryGateSelfTest.mq5',
...
for name in SCHEMAS:
    p=root/f'lab/11_strategy_factory/schemas/v3/{name}.schema.json'
...
for p in [root/'lab/11_strategy_factory/test_vectors/v3/uce_i12_promotion_conformance_vectors.json',root/f'{STATUS}/phase_status/UCE_I12.json',root/f'{STATUS}/phase_status/UCE_I12_HANDOFF_TO_UCE_I13.json',root/f'{STATUS}/artifacts/UCE_I12_ACCEPTANCE_EVIDENCE.json']+list((root/'lab/11_strategy_factory/examples/uce_i12').glob('*.json')):
...
mql=list((root/'mql5/Include/AlphaLab/StrategyFactory/StatisticalPromotion').glob('UCEI12_*.mqh'))+[root/'mql5/Experts/StrategyFactory/UCE_I12_StatisticalPromotionDiagnostic.mq5',root/'mql5/Experts/StrategyFactoryTests/UCE_I12_PromotionContractsSelfTest.mq5',root/'mql5/Experts/StrategyFactoryTests/UCE_I12_NonCompensatoryGateSelfTest.mq5']
if len(mql)!=11 or not all(p.is_file() for p in mql): errors.append(f'MQL5 file set invalid: {len(mql)}')
pyproject=(root/'lab/11_strategy_factory/python/pyproject.toml').read_text()
