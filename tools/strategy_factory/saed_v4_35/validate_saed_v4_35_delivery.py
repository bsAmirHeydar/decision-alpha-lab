...
ROOT=Path(__file__).resolve().parents[3];index=ROOT/'releases/history/saed/indexes/SAED_V4_35_FILE_INDEX.txt';paths=[x.strip() for x in index.read_text().splitlines() if x.strip()];assert paths==sorted(set(paths));missing=[p for p in paths if not (ROOT/p).is_file()];assert not missing,missing
...
excluded={'releases/history/saed/hashes/SAED_V4_35_FILE_HASHES.sha256','releases/history/saed/manifests/SAED_V4_35_PATCH_MANIFEST.json','releases/history/saed/reports/SAED_V4_35_QA_REPORT.json','lab/11_strategy_factory/phase_status/SAED_V4_35.json'}
for p in paths:
