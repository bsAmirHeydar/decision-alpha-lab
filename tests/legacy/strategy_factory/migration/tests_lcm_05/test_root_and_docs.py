...
def test_master_architecture_selected(topology_root):
    rows=read_jsonl(topology_root/'documentation/documentation_successor_map.jsonl');dups=[x for x in rows if x['relationship']=='EXACT_BYTE_DUPLICATE_TREE'];assert dups;assert all(x['canonical_successor'].startswith('docs/architecture/master/') for x in dups)
