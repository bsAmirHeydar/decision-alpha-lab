...
from src.engine.tooling.strategy_factory.lcm.lcm_16a.constants import UPSTREAM_PACKAGE_RELATIVE
...
def test_amendments_are_exactly_scoped(audit_root):
    ...
    assert all(row["candidate_path"].startswith("docs/history/aieos_legacy/") for row in rows)
