...
def test_no_execution_authority():
    paths=list((ROOT/"mql5/Include/AlphaLab/StrategyFactory/Outcome").rglob("*.mqh"))+list((ROOT/"mql5/Experts/StrategyFactory").glob("SF09_*.mq5"))+list((ROOT/"mql5/Tests/Experts/StrategyFactory").glob("SF09_*.mq5"))
...
def test_schemas_exist():
    b=ROOT/"schemas/legacy/strategy_factory/v1";assert all((b/x).exists() for x in ["price_observation.schema.json","simulation_policy.schema.json","cost_model_descriptor.schema.json","outcome_record.schema.json","outcome_run_manifest.schema.json"])
