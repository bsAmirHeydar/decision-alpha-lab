...
def test_valid_contracts(load):
 FusionConfig.from_mapping(load('examples/legacy/strategy_factory/saed_v4_15/reference_fusion_config.json'))
 [CandidateSpec.from_mapping(x) for x in load('examples/legacy/strategy_factory/saed_v4_15/reference_candidate_catalog.json')]
 SupportPolicy.from_mapping(load('examples/legacy/strategy_factory/saed_v4_15/reference_support_policy.json'));MissingnessPolicy.from_mapping(load('examples/legacy/strategy_factory/saed_v4_15/reference_missingness_policy.json'));ComputeExposureBudget.from_mapping(load('examples/legacy/strategy_factory/saed_v4_15/reference_compute_exposure_budget.json'))
@pytest.mark.parametrize('field',['unknown','extra','future'])
def test_unknown_config_fields_fail(load,field):
 x=load('examples/legacy/strategy_factory/saed_v4_15/reference_fusion_config.json');x[field]=1
...
def test_common_dim_floor(load,bad):
 x=load('examples/legacy/strategy_factory/saed_v4_15/reference_fusion_config.json');x['common_dim']=bad
 with pytest.raises(ContractError):FusionConfig.from_mapping(x)
@pytest.mark.parametrize('algo',['oracle','remote_llm','order_router'])
def test_unknown_algorithm_fails(load,algo):
 x=load('examples/legacy/strategy_factory/saed_v4_15/reference_candidate_catalog.json')[0];x['algorithm']=algo
