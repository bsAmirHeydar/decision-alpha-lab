...
def setup(records,config,load):
 o=[CorpusRecord.from_mapping(x) for x in records];sp=assign_splits(o,load('examples/legacy/strategy_factory/saed_v4_11/reference_split_policy.json'));ss=build_token_streams(o,sp);tok=build_tokenizer_spec(ss);cfg=TrainingConfig.from_mapping(config);return ss,tok,cfg
