...
def test_i08_schemas_closed():
 ...
 for n in names:
  o=json.loads((ROOT/f'schemas/legacy/strategy_factory/v3/{n}.schema.json').read_text());assert o.get('additionalProperties') is False
def test_no_live_authority():
 roots=[ROOT/'src/engine/packages/strategy_factory_classical_v3',ROOT/'mql5/Include/AlphaLab/StrategyFactory/ClassicalAlgorithms'];text='\n'.join(f.read_text(errors='ignore') for r in roots for f in r.rglob('*') if f.is_file());assert not re.search(r'\b(OrderSend|OrderCheck|PositionOpen|WebRequest)\s*\(',text)
