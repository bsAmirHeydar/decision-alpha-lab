...
def test_no_live_authority():
    owned_roots = [
        ...
        ROOT/'mql5/Tests/Experts/StrategyFactory/SF06_ReferenceAnatomySelfTest.mq5',
    ]
    files = []
...
def test_pyproject_includes_package():
    assert 'strategy_factory_anatomy*' in (ROOT/'src/engine/packages/pyproject.toml').read_text()
