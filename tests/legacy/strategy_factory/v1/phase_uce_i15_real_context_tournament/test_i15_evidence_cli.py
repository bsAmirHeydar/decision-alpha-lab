...
def test_cli_golden():
 env=dict(os.environ);env['PYTHONPATH']='src/engine/packages';p=subprocess.run([sys.executable,'-m','strategy_factory_tournament_v3.cli','golden'],cwd='.',env=env,text=True,capture_output=True,check=True);data=json.loads(p.stdout);assert data['decision']['status']=='reject'
