...
def test_candidate_is_exact_historical_ten_member_cluster(repo_root: Path) -> None:
    documents = characterize(repo_root)
    ...
    assert {(row["artifact_path"], row["function_name"]) for row in inventory["members"]} == {
        (path, function_name) for path, function_name, _ in MEMBERS
    }


def test_candidate_sources_remain_byte_identical_to_lcm_evidence(repo_root: Path) -> None:
...
def test_w1a_does_not_materialize_or_cut_over_production(repo_root: Path) -> None:
