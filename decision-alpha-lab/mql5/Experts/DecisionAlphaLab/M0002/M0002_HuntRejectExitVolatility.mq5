...
input ENUM_DALM0002MeasureMode InpMeasureMode = DAL_M0002_MEASURE_EVENT_RTV; // default = split the exact M0001 event RTV by exit-side outcome
input int InpOutcomeCandleOffsetAfterExit = 0;  // 0 = the candle that completes exit-gap; 1 = next closed candle
input int InpPostOutcomeSampleBars = 20;        // only used when InpMeasureMode=POST_OUTCOME_FIXED
input bool InpUseEventLengthForSample = false;  // only used when InpMeasureMode=POST_OUTCOME_FIXED
...
void BuildM0002Config(DALM0002Config &config)
{
   ...
   config.measure_mode = InpMeasureMode;
...
void UpdateRuntimeComment(const int bars_count, const string source_mode)
{
   ...
   Comment(
      ...
      "measureMode=", DAL_M0002MeasureModeToString(InpMeasureMode),
