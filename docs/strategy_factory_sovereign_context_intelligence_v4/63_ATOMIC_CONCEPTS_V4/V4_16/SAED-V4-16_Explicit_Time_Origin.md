...
# Explicit Time Origin

## Purpose

Every duration is measured from a declared context-detection time and cannot silently switch to fill or bar-open time. This note is normative for the deterministic synthetic-reference implementation and must be read together with the machine-readable contracts, frozen upstream evidence, source package, negative fixtures, QA report, claim ledger and authority boundary.
