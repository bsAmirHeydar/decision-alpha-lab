...
A cause-specific hazard is conditional event mass among paths still at risk, not a treatment recommendation. This note is normative for the deterministic synthetic-reference implementation and must be read together with the machine-readable contracts, frozen upstream evidence, source package, negative fixtures, QA report, claim ledger and authority boundary.
