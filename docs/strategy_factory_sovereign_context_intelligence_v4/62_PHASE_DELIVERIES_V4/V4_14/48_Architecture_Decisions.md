...
Recorded decisions: adapters not oracles, local-only reference mode, no external weights, and no online learning. This note is normative for the V4-14 reference implementation and must be read together with the machine-readable contracts, artifacts, tests, and authority boundary.
