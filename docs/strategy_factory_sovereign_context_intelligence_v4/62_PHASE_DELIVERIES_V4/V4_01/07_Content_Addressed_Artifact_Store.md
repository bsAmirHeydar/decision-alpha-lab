...
Specifies atomic writes, verification, immutable addressing and corruption detection.
...
Artifact bytes are written to a temporary file, flushed, synchronized and atomically renamed into a path derived from SHA-256. The address is therefore a claim about exact content. An existing address with different bytes is an integrity incident, not a version update. Operational locations may change; semantic identity does not.
