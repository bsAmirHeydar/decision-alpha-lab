...
Replay rebuilds the graph from the same exact source package, registry, policy, and graph version. The observed graph hash must equal the expected hash. A mismatch is an integrity error and cannot be downgraded to a warning.
