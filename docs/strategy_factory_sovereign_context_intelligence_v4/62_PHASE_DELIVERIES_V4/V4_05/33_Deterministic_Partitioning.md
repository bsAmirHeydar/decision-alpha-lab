...
Nodes and edges are assigned to a fixed partition count using stable hash prefixes. Partitioning supports scalable storage and review without mutable global state. Applying a partition manifest does not mutate an existing graph. The institutional default partition count is policy-bound, so changing that policy produces a new policy hash and new graph identity for subsequent builds.
...
- Previous: [[32_Bounded_Graph_Query]]
- Phase home: [[00_MOC_V4_05_Semantic_And_Temporal_Hypergraph]]
- Next: [[34_Caching_Idempotency_And_Rebuild]]
