...
Expose a minimal deterministic build interface suitable for automation and independent reproduction.

## Contract

The Python service accepts graph, handoff, registry, policy, capability profile and program sources. The CLI command build accepts explicit files and emits a canonical package plus its ID, hash and program count.
...
Reject implicit working-directory discovery, network fetches and mutable global configuration. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[43_Telemetry_And_Exposure_Ledger]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[45_Python_Reference_Implementation]]
