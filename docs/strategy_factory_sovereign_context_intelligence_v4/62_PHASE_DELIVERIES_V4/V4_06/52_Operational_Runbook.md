...
Specify the repeatable sequence for building and reviewing a V4-06 release.

## Contract

Operators verify upstream hashes, run the full QA tool, reproduce the golden package, inspect acceptance evidence, validate schemas and notes, run local MetaEditor compile when available, validate delivery hashes, stage only the indexed files and commit with the supplied message.
...
Stop immediately on upstream drift, nondeterminism, hash mismatch, unsupported capability or authority expansion. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[51_Quarantine_Incident_And_Recovery]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[53_Performance_And_Compute_Economics]]
