...
# V4-05 Input Contract

## Purpose

Consume only exact, immutable V4-05 semantic-temporal graph and handoff artifacts.

## Contract

Graph identity, graph hash, source package identity, registry identity, policy identity, Evidence Role and known-as-of boundaries must agree with the V4-05 handoff. The DSL package stores those identities as lineage inputs rather than copying or mutating graph truth.
...
Reject stale, cross-role, hash-mismatched, unsupported or incomplete graph/handoff pairs. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[03_Authority_And_UCEE_Boundary]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[05_Finite_Action_Constitution]]
