...
Freeze a finite, typed and exact-versioned Treatment language that converts governed strategy semantics into deterministic, inspectable declarations without granting selection or execution authority.

## Contract

The deliverable creates a stable semantic surface between Context representation and the downstream action-lattice solver. The central engineering objective is not expressive maximalism; it is bounded expressiveness with explicit denial of undefined behavior.
...
Reject any program whose meaning depends on implicit defaults, unknown primitives, mutable registry state, hidden capital semantics, or unavailable upstream evidence. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[00_MOC_V4_06_Treatment_DSL]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[02_Phase_Scope_And_Non_Goals]]
