...
Provide closed types, ranges, defaults, units and identity semantics for every primitive parameter.

## Contract

Supported types are boolean, integer, decimal, string, enum and duration_ms. Validation distinguishes booleans from integers, uses decimal-safe comparison, requires enum allowlists and enforces minimum/maximum bounds.
...
Reject implicit coercion, NaN-like values, unbounded enums, unknown parameters and type/range mismatches. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[18_Capability_Profiles]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[20_Units_And_Normalization]]
