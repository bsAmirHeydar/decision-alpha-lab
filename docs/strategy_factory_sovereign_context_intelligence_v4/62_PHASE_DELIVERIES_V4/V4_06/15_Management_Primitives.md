...
Represent post-entry management without embedding portfolio or sizing authority.

## Contract

The reference management space contains none and partial scale. Partial scale declares an activation threshold and a quantity fraction bounded strictly between zero and one, while requiring partial-exit capability. It does not set original position size.
...
Reject quantity fields that become capital allocation, unsupported partial exits or unmanaged free-form schedules. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[14_Trail_Primitives]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[16_Time_Primitives]]
