...
Mirror critical V4-06 contracts in MQL5 for static and local MetaEditor conformance.

## Contract

Headers encode authority, canonical types, parameters, registry, programs, validation, binding, state machine, integrity, telemetry and handoff. Diagnostic experts exercise self-test paths only. There are no order, trade, network or broker calls.
...
Static validation is not MetaEditor compile, terminal replay, broker parity or live authorization. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[45_Python_Reference_Implementation]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[47_Closed_Schema_Catalog]]
