...
Declare where directional intent originates without allowing hidden inference.

## Contract

The reference direction primitive records whether direction is supplied by Context or by a governed manual source. Direction is a declaration and not a learned prediction. Supported sides are explicit at both primitive and program level.
...
Reject unsupported sides, ambiguous direction sources or any attempt to infer direction from future or outcome-labelled features. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[08_Payoff_Primitives]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[10_Entry_Primitives]]
