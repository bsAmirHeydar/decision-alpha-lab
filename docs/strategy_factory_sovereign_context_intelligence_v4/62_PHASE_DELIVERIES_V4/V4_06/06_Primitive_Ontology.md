...
Organize Treatment semantics into typed primitive kinds with explicit families and capabilities.

## Contract

The ontology includes action, payoff, direction, entry, trigger, stop, target, exit, trail, management, time, cost and capability kinds. Each primitive has one exact version, typed parameters, semantic labels, supported sides, required capabilities and a path-dependence declaration.
...
Reject duplicate exact keys, duplicate definition identities, unknown kinds or malformed primitive metadata. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[05_Finite_Action_Constitution]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[07_Action_Skip_And_Abstain]]
