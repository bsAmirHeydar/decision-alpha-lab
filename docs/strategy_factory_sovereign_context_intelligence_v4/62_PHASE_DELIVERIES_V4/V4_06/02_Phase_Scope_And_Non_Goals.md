...
Define, parse, canonicalize, validate, bind, replay, diff and package finite Treatment programs.

## Contract

V4-06 owns declaration semantics only. It does not enumerate feasible combinations, optimize parameters, rank alternatives, train models, allocate capital, activate runtime, talk to a broker or submit orders. Those powers remain with later governed phases and UCEE.
...
Any implementation path that starts solving the action lattice or making a trading decision is an authority violation. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Next: [[03_Authority_And_UCEE_Boundary]]
