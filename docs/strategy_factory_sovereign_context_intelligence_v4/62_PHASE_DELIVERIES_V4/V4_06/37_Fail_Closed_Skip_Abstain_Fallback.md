---
title: Fail-Closed Skip And Abstain Fallback
status: implemented
version: 1.0.0
phase: V4-06
created: '2026-07-15'
...
# Fail-Closed Skip And Abstain Fallback

## Purpose

Provide explicit safe outputs for unsupported or uncertain cases without fabricating a Treatment.

## Contract

Skip is appropriate when no executable path should exist; Abstain is appropriate when evidence support or certainty is inadequate. They are packaged as ordinary canonical programs with system-action invariants and no ordinary components.
...
Reject fallback paths that silently map to a market entry, default Treatment or prior cached decision. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[36_Prohibited_Sizing_And_Capital_Semantics]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[38_Deterministic_Builder_Pipeline]]
