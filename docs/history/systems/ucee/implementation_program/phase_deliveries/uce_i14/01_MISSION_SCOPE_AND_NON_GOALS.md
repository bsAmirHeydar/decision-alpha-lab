...
Compile one exact accepted policy generation without creating new strategy semantics or execution authority.
...
- The chapter owner is `01_MISSION_SCOPE_AND_NON_GOALS`; downstream consumers must cite its artifact identities rather than reconstruct behavior from prose.
- The operational objective is: Compile one exact accepted policy generation without creating new strategy semantics or execution authority.
