---
title: "Trainer SDK Integration and Capability Flags"
...
Expose native I10 algorithms through the existing Trainer SDK without weakening its dataset, split, and resource contracts.
...
1. **Each trainer has an exact algorithm key and version.**
2. **Configuration requires known-time audit and accepted/non-rejected deep admission.**
3. **Task/view/schema compatibility is validated before fit.**
4. **Serialization is canonical JSON with a state hash; deserialization verifies the state.**

## Contract and implementation surface

- `five native trainer plugins`
- `sequence, raster, graph, novelty, and stacked fusion`
- `TrainerConfig and ResourceBudget`
- `DatasetSchema compatibility`
- `registry registration and capability flags`
...
|---|---|
| `trainer-specific access to hidden final-test rows` | Reject or abstain; retain code and evidence; do not silently repair. |
| `accepting shape from feature count by guesswork` | Reject or abstain; retain code and evidence; do not silently repair. |
| `loading a tampered state` | Reject or abstain; retain code and evidence; do not silently repair. |
| `registering an optional adapter as native` | Reject or abstain; retain code and evidence; do not silently repair. |
