---
title: "Stacking, Gated Fusion, and Cross-Attention Gate"
tags: [strategy-factory, universal-context-engine, uce-i10, deep-views]
status: implemented
doc_version: 1.1.0
last_updated: 2026-07-13
---
# Stacking, Gated Fusion, and Cross-Attention Gate

## Purpose

Permit more complex fusion only after OOF and late-fusion evidence justify the added capacity.
...
1. **StackedFusion refuses non-OOF base predictions.**
2. **GatedFusion combines validation-derived base weights with bounded confidence inputs.**
3. **Low total confidence produces abstention rather than fabricated certainty.**
4. **Cross-attention remains unavailable until late fusion, gated fusion, and stacking are fully evaluated.**

## Contract and implementation surface

- `OOF stacking matrix`
- `ridge meta-learner`
- `confidence-gated weights`
- `complexity admission decision`
- `fusion-family differential evidence`
...
|---|---|
| `stacking on in-sample predictions` | Reject or abstain; retain code and evidence; do not silently repair. |
| `unbounded or negative confidence weights` | Reject or abstain; retain code and evidence; do not silently repair. |
| `cross-attention as the default architecture` | Reject or abstain; retain code and evidence; do not silently repair. |
| `comparing fusion methods on different folds` | Reject or abstain; retain code and evidence; do not silently repair. |
