---
title: "Sequence Contracts, Causal Masks, and Windows"
tags: [strategy-factory, universal-context-engine, uce-i10, deep-views]
status: implemented
doc_version: 1.1.0
last_updated: 2026-07-13
---
# Sequence Contracts, Causal Masks, and Windows

## Purpose

Specify a fixed-width event-relative sequence whose values, timestamps, masks, and layout are deterministic.
...
1. **Rows with time greater than known_time_ms are excluded before ordering and validation.**
2. **Known-time source timestamps are strictly increasing when the spec requires strict time.**
3. **Left padding and missing-feature masks are explicit and serialized.**
4. **Feature order, layout, steps, pad value, and strict-time behavior are identity-bearing.**

## Contract and implementation surface

- `SequenceWindowSpec`
- `SequenceArtifact`
- `time-major and feature-major flattening`
- `mask-aware causal validation`
...
|---|---|
| `future rows changing a past artifact` | Reject or abstain; retain code and evidence; do not silently repair. |
| `duplicate known-time observations` | Reject or abstain; retain code and evidence; do not silently repair. |
| `non-finite source values` | Reject or abstain; retain code and evidence; do not silently repair. |
| `non-zero values hidden behind a zero-pad mask` | Reject or abstain; retain code and evidence; do not silently repair. |
...
- [[04_SEQUENCE_ENCODERS_AND_ADAPTER_BOUNDARIES]]
