---
title: "Regime Expert Fallback and Abstention"
tags: [strategy-factory, universal-context-engine, uce-i10, deep-views]
status: implemented
doc_version: 1.1.0
last_updated: 2026-07-13
---
# Regime Expert Fallback and Abstention

## Purpose

Route a sample only to a supported expert; otherwise use the global model or abstain.
...
1. **Novelty and severe change take precedence over expert selection.**
2. **Sparse or low-probability regimes fall back to the global model.**
3. **Expert keys are explicit and aligned one-to-one with canonical regime IDs.**
4. **Every routing result states decision, reason, support, probability, novelty, and change evidence.**

## Contract and implementation surface

- `expert, global, and abstain decisions`
- `minimum support threshold`
- `minimum regime probability`
- `novelty and change abstention limits`
- `routing evidence hash`
...
|---|---|
| `forcing an expert for every row` | Reject or abstain; retain code and evidence; do not silently repair. |
| `mapping experts by unstable cluster order` | Reject or abstain; retain code and evidence; do not silently repair. |
| `hiding global fallback as a successful expert prediction` | Reject or abstain; retain code and evidence; do not silently repair. |
| `using missing expert output as zero` | Reject or abstain; retain code and evidence; do not silently repair. |
