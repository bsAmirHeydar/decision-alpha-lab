---
title: "UCE-I10 Operator Runbook"
tags: [strategy-factory, universal-context-engine, uce-i10, deep-views]
status: implemented
doc_version: 1.1.0
last_updated: 2026-07-13
---
# UCE-I10 Operator Runbook

## Purpose

Provide an exact sequence for validating, applying, compiling, and rolling back the I10 delivery.
...
1. **Run from repository root with the Strategy Factory Python path configured.**
2. **Execute I10 tests before cumulative tests.**
3. **Run boundary, schema, vector, static MQL5, and delivery validators.**
4. **Compile MQL5 diagnostics locally and retain the MetaEditor log before changing compile status.**

## Contract and implementation surface

- `test commands`
- `vector generation/check mode`
- `safe patch expansion`
...
|---|---|
| `deleting the patch before validation succeeds` | Reject or abstain; retain code and evidence; do not silently repair. |
| `copying files over an unknown repository revision` | Reject or abstain; retain code and evidence; do not silently repair. |
| `committing cache directories` | Reject or abstain; retain code and evidence; do not silently repair. |
| `pushing without reviewing the staged diff` | Reject or abstain; retain code and evidence; do not silently repair. |
...
- [[26_PATCH_EXPAND_REMOVE_COMMIT_PUSH_RUNBOOK]]
