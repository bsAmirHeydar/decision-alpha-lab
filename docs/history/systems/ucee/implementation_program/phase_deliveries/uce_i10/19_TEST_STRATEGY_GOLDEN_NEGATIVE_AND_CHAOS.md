---
title: "Test Strategy: Golden, Negative, and Chaos"
...
Prove contract, causality, determinism, failure behavior, and integration with executable tests and retained outputs.
...
1. **Golden tests establish deterministic model and artifact identity.**
2. **Negative tests corrupt timestamps, masks, edges, schemas, dependencies, transfer boundaries, and export evidence.**
3. **Future-perturbation tests append extreme or malformed future data and require unchanged past artifacts.**
4. **Static MQL5 tests verify include closure, balanced braces, catalog counts, and forbidden authority tokens.**
...
- `conformance vectors`
- `cumulative Strategy Factory regression suite`
...
|---|---|
| `tests that merely assert files exist` | Reject or abstain; retain code and evidence; do not silently repair. |
| `marking MetaEditor compile passed without a compiler log` | Reject or abstain; retain code and evidence; do not silently repair. |
| `using the same function to generate and verify expected values` | Reject or abstain; retain code and evidence; do not silently repair. |
| `removing failed evidence from the QA record` | Reject or abstain; retain code and evidence; do not silently repair. |
