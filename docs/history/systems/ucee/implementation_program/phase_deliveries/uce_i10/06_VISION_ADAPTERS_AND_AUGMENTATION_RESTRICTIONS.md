---
title: "Vision Adapters and Augmentation Restrictions"
tags: [strategy-factory, universal-context-engine, uce-i10, deep-views]
status: implemented
doc_version: 1.1.0
last_updated: 2026-07-13
---
# Vision Adapters and Augmentation Restrictions

## Purpose

Bound high-capacity visual models by causal rendering, approved augmentation, and export evidence.
...
1. **Vision adapters consume only raster artifacts, never chart UI screenshots.**
2. **Augmentation is disabled by default and its complete policy is hash-addressed.**
3. **Time reversal, horizontal reflection, label-conditioned crops, and future-aware normalization are forbidden.**
4. **A vision candidate must beat the deterministic raster baseline under identical folds.**

## Contract and implementation surface

- `small CNN adapter contract`
- `optional pretrained encoder boundary`
- `augmentation allow-list`
- `view-specific ablation`
- `ONNX or distilled runtime path`
...
|---|---|
| `pretraining data containing target final-test rows` | Reject or abstain; retain code and evidence; do not silently repair. |
| `undocumented resizing or interpolation` | Reject or abstain; retain code and evidence; do not silently repair. |
| `random augmentation without recorded seed` | Reject or abstain; retain code and evidence; do not silently repair. |
| `accepting visual saliency as causal proof` | Reject or abstain; retain code and evidence; do not silently repair. |
