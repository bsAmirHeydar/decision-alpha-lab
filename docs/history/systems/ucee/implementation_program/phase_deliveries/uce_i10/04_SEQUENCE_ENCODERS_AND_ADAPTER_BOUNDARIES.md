---
title: "Sequence Encoders and Adapter Boundaries"
tags: [strategy-factory, universal-context-engine, uce-i10, deep-views]
status: implemented
doc_version: 1.1.0
last_updated: 2026-07-13
---
# Sequence Encoders and Adapter Boundaries

## Purpose

Provide a deterministic temporal baseline while isolating optional GRU and Transformer implementations behind capability probes.
...
1. **TemporalConvEncoder is a deterministic reference, not a disguised neural-network claim.**
2. **Optional libraries are imported lazily and never at package import time.**
3. **Adapter plans declare module versions, device, precision, seed, determinism, and export format.**
4. **A missing or incompatible dependency fails closed with retained probe evidence.**

## Contract and implementation surface

- `causal temporal convolution summaries`
- `ridge reference head`
- `GRU/Transformer descriptor contracts`
- `dependency probes`
...
|---|---|
| `silent CPU/GPU behavior drift` | Reject or abstain; retain code and evidence; do not silently repair. |
| `using bidirectional or future-attending sequence blocks` | Reject or abstain; retain code and evidence; do not silently repair. |
| `accepting an unknown dependency version without evidence` | Reject or abstain; retain code and evidence; do not silently repair. |
| `serializing framework-native pickle artifacts` | Reject or abstain; retain code and evidence; do not silently repair. |
