---
title: "UCE-I11 — Experiment Playbooks and Expected Artifacts"
...
---
# Experiment Playbooks and Expected Artifacts

## Purpose

Golden playbooks demonstrate declaration compilation, failure injection, search registry snapshot, cache/reproducibility checks, and expected manifest counts.

## Scope

- Start with the admitted classical baseline.
- Add challenger proposals under per-candidate caps.
- Execute train/validation only.
- Select/report only after ledger and reproducibility checks.

## Normative invariants

1. **Examples are fixtures, not production recommendations.**
2. **Budgets are deliberately small.**
3. **Parameter domains are identity-bearing.**
4. **Expected hashes must be regenerated only after reviewed behavior changes.**
...
immutable upstream evidence
        │
        ▼
validated experiment playbooks and expected artifacts
        │
