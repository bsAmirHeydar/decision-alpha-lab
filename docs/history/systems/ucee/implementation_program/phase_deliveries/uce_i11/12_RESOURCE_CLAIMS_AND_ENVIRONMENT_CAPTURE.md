---
title: "UCE-I11 — Resource Claims and Environment Capture"
...
---
# Resource Claims and Environment Capture

## Purpose

Each trial receives an immutable ResourceClaim. EnvironmentCapture records interpreter, implementation, platform, selected package versions, and deterministic environment flags.

## Scope

- Attach claim hash to trial node.
- Fail on missing claim.
- Capture only explicit environment keys.
- Record missing packages as evidence, not as empty success.

## Normative invariants

1. **Claim values cannot exceed compiled policy.**
2. **Environment capture hash is stable for the same allowlist.**
3. **GPU claims require GPU slots.**
4. **Deterministic-required is explicit.**
...
immutable upstream evidence
        │
        ▼
validated resource claims and environment capture
        │
