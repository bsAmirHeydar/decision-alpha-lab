---
title: "UCE-I11 — Failure Modes, Residual Risks, and Rollback"
...
---
# Failure Modes, Residual Risks, and Rollback

## Purpose

Known failure classes include invisible multiplicity, candidate explosion, stale cache, dependency drift, worker crash, timeout, resource exhaustion, ledger corruption, and rerun divergence.

## Scope

- Fail closed on identity/schema mismatch.
- Quarantine unknown worker exceptions.
- Rollback by removing only I11 indexed files.
- Retain I10 artifacts unchanged.

## Normative invariants

1. **Reference search implementations are not efficiency claims.**
2. **Sequential scheduler is the deterministic floor; parallel implementations need separate parity evidence.**
3. **Real-market edge remains unproven.**
4. **I12 owns multiplicity-aware promotion.**
...
immutable upstream evidence
        │
        ▼
validated failure modes, residual risks, and rollback
        │
