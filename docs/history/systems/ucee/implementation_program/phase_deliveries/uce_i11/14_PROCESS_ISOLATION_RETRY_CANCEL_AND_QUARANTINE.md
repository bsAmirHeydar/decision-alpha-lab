---
title: "UCE-I11 — Process Isolation, Retry, Cancellation, and Quarantine"
...
---
# Process Isolation, Retry, Cancellation, and Quarantine

## Purpose

Spawn-based local isolation prevents inherited mutable state. Parent-enforced timeout, explicit cancellation, bounded retries, and quarantine contain failures.

## Scope

- Resolve callable capability before spawn.
- Use spawn, not fork-inherited model state.
- Terminate timed-out child processes.
- Retry only RETRYABLE failures and never exceed max_retries.

## Normative invariants

1. **Unexpected exceptions are quarantined.**
2. **Cancellation is ledger evidence.**
3. **A retry gets a new worker_id and attempt.**
4. **Descendants of terminal failure are skipped, not left pending.**
...
immutable upstream evidence
        │
        ▼
validated process isolation, retry, cancellation, and quarantine
        │
