---
title: "UCE-I11 — Patch, Expand, Remove-ZIP, Commit, and Push Runbook"
...
---
# Patch, Expand, Remove-ZIP, Commit, and Push Runbook

## Purpose

The I11 patch is self-contained and indexed. The operator expands it at repository root, removes the archive, stages only indexed files, commits with the supplied message, and pushes main.

## Scope

- Verify repository root before extraction.
- Use the patch file index for git add.
- Run QA before commit.
- Do not stage unrelated working-tree changes.

## Normative invariants

1. **The ZIP path may be Downloads rather than repository root.**
2. **Patch extraction must not create an extra top-level directory.**
3. **Remove ZIP only after successful extraction.**
4. **Push failure does not invalidate the local commit.**
...
immutable upstream evidence
        │
        ▼
validated patch, expand, remove-zip, commit, and push runbook
        │
