---
title: "UCE-I11 — Test Strategy: Golden, Negative, and Chaos"
...
The test suite covers contracts, identity, compiler determinism, search, budget cutoffs, scheduler transitions, retries, cancellation, timeout, quarantine, cache, resume, ledger, schemas, isolation, and reproducibility.

## Scope

- Golden: same declaration emits same DAG/trials.
- Negative: hidden role, rejected candidate, invalid bounds, stale cache.
- Failure injection: worker exception, timeout, cancellation, budget denial.
- Differential: semantic snapshots and conformance vectors.

## Normative invariants

1. **Tests must execute, not be checklist-only.**
2. **Warnings are eliminated from the phase test run.**
3. **Synthetic fixtures do not prove market edge.**
4. **MetaEditor remains explicitly pending when unavailable.**
...
immutable upstream evidence
        │
        ▼
validated test strategy: golden, negative, and chaos
        │
