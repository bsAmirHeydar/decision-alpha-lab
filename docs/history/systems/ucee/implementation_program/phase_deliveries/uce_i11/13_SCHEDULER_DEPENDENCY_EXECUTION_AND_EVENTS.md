---
title: "UCE-I11 — Scheduler Dependency Execution and Events"
...
---
# Scheduler Dependency Execution and Events

## Purpose

The reference scheduler executes a deterministic ready queue, emits append-only transition events, and preserves an exact event-stream hash.

## Scope

- Ready requires all dependencies succeeded or cached.
- Terminal dependency failure skips descendants.
- Emit ready, claim, start, complete/fail/retry/timeout/cancel/quarantine/cache/resume events.
- Use attempt-scoped worker IDs.

## Normative invariants

1. **No node executes before dependencies.**
2. **Event sequence is contiguous.**
3. **Worker exception becomes quarantine evidence.**
4. **Same manifest and worker behavior reproduce the event stream.**
...
immutable upstream evidence
        │
        ▼
validated scheduler dependency execution and events
        │
