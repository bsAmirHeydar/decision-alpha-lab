---
title: "UCE-I11 — Budget Hierarchy and Hard Cutoffs"
...
---
# Budget Hierarchy and Hard Cutoffs

## Purpose

BudgetPolicy governs total trials, per-candidate trials, total and per-trial wall time, memory, CPU/GPU slots, retries, artifacts, seeds, folds, and candidates.

## Scope

- Assess before reservation.
- Deny on any hard ceiling.
- Account start, completion, wall time, artifact bytes, and memory high-water.
- Carry the policy hash into every trial identity.

## Normative invariants

1. **A scheduler cannot substitute a different policy.**
2. **No oversubscription warning is treated as permission.**
3. **Per-candidate caps prevent one family from consuming the universe.**
4. **Retention budget includes failed artifacts when configured.**
...
immutable upstream evidence
        │
        ▼
validated budget hierarchy and hard cutoffs
        │
