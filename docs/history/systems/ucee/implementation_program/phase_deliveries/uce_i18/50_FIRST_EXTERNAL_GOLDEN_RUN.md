---
title: "UCE-I18 — First External Golden Run"
tags: [strategy-factory, uce-i18, production-qualification, release]
status: implemented_reference_external_evidence_pending
doc_version: 1.0.0
last_updated: 2026-07-15
---
# First External Golden Run

## Decision

Defines the exact first Windows/MetaTrader execution that must produce compile, reconciliation and fail-closed restart evidence.


## Required first run

The first external run is a Windows/MetaTrader diagnostic run, not a trading run. It must compile all I18 experts, record compiler and terminal builds, load a shadow-only I17 runtime bundle, restart the terminal, and prove that no action is emitted before exact reservation-ledger reconciliation. The run must then inject a ledger-hash mismatch and prove that the kill switch engages and activation remains false. Raw logs, screenshots where required, hashes and environment metadata are archived as one evidence package.
