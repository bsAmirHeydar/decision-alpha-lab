...
Why binary or mean-only labels are insufficient and why event timing, censoring and tail outcomes are first-class research objects. This note is normative for the deterministic synthetic-reference implementation and must be read together with the machine-readable contracts, frozen upstream evidence, source package, negative fixtures, QA report, claim ledger and authority boundary.
