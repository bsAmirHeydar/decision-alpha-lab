...
Precise lower-tail definitions and the invariant that expected shortfall cannot exceed its associated lower-tail quantile. This note is normative for the deterministic synthetic-reference implementation and must be read together with the machine-readable contracts, frozen upstream evidence, source package, negative fixtures, QA report, claim ledger and authority boundary.
