...
Address every primitive by primitive_id@exact_version and hash the full definition.

## Contract

Semantic versions are exact keys, not compatibility ranges. The registry hash commits to all primitive definitions, policy-relevant metadata, authority and limitations. A changed default, range, capability or label produces a different definition and registry identity.
...
Reject version ranges, missing versions, duplicate exact keys and registry drift. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[20_Units_And_Normalization]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[22_Closed_Program_Grammar]]
