...
Bind Treatments to explicit session windows and duration units.

## Contract

Session-window primitives identify a canonical session, start offset and bounded duration in milliseconds. Time semantics are identity-affecting and must be point-in-time reproducible. They are not wall-clock scheduler instructions.
...
Reject invalid sessions, negative durations, excessive windows or timezone-implicit free text. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[15_Management_Primitives]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[17_Cost_And_Economics_Boundary]]
