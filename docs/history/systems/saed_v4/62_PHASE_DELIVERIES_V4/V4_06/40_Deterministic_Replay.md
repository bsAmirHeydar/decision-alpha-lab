...
Rebuild the package from exact inputs and compare all semantic identities.

## Contract

Replay uses immutable V4-05 graph/handoff, exact registry/policy/profile and source programs. A successful receipt requires matching package ID/hash, program IDs/hashes, binding IDs/hashes and lineage root.
...
Treat nondeterminism as a release-blocking defect; do not normalize it away after the fact. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[39_Integrity_Receipt]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[41_Semantic_Diff]]
