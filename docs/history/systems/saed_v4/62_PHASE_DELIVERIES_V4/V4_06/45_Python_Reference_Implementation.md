...
Provide a dependency-light, typed and testable implementation under saed_v4_treatment_dsl.

## Contract

Modules separate authority, canonicalization, catalog, contracts, parser, registry, static analysis, graph binding, integrity, replay, diff, partition, telemetry, conformance, handoff and service concerns. Dataclasses are immutable and serialization is deterministic.
...
The implementation remains reference-only until independent and production evidence is attached. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[44_Service_And_CLI]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[46_MQL5_Diagnostic_Mirror]]
