...
Derive a content-addressed identity from the canonical semantic payload.

## Contract

The program identifier and hash commit to exact primitive definitions, parameter values, constraints, lifecycle, Evidence Role, known-as-of, source artifact hash and semantic labels. Reordering equivalent input produces the same identity; any semantic change produces a new identity.
...
Reject identity reuse across differing payloads and quarantine hash mismatches. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[23_Canonical_Abstract_Syntax_Tree]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[25_Component_Identity]]
