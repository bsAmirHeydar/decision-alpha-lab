...
Preserve UCEE as authority of record while allowing a subordinate declarative DSL service.

## Contract

The authority object is machine-readable. Allowed operations include reading the V4-05 graph and handoff, defining the finite DSL, canonicalizing, validating, binding external descriptors, replaying and diffing. All mutation, inference, learning, selection, risk, runtime, network and order authorities are false.
...
Fail closed when the requested operation is absent from the allowlist; do not infer authority from data availability. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
