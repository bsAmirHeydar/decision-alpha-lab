...
Make every parameter unit explicit and canonical.

## Contract

Allowed units include unitless, enum, boolean, integer, decimal, milliseconds, points, R multiple, fraction, feature reference and scalar. Canonicalization preserves semantic equivalence without silently converting between economically distinct units.
...
Reject undeclared units and require upstream normalization rather than guessing conversion factors. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[19_Parameter_Type_System]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[21_Exact_Version_Registry]]
