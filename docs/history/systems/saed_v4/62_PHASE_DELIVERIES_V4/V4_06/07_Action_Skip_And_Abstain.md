...
# Action Skip And Abstain

## Purpose

Make Skip and Abstain first-class system outcomes rather than implicit absence.

## Contract

Skip means that no executable path is created. Abstain means that support or certainty is insufficient to make a governed declaration. Both are exact-versioned action primitives, use side scope none, and are required by institutional policy.
...
Reject registries that omit either primitive and reject programs that mix a system action with ordinary Treatment components. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[06_Primitive_Ontology]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[08_Payoff_Primitives]]
