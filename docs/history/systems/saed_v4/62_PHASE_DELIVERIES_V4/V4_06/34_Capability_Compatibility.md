...
Ensure every component requirement is satisfied by the exact profile before packaging.

## Contract

Primitive-required capabilities are unioned and compared to the diagnostic profile. Capability components also make platform assumptions explicit in the program itself. No capability is inferred from file presence or developer expectation.
...
Reject unsupported order, protection, expiration, trailing or partial-exit requirements. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[33_Cycle_Rejection_And_Reachability]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[35_Budgets_And_Explosion_Controls]]
