---
title: Quarantine Incident And Recovery
status: implemented
version: 1.0.0
phase: V4-06
created: '2026-07-15'
...
# Quarantine Incident And Recovery

## Purpose

Define deterministic handling for invalid, suspicious or non-reproducible artifacts.

## Contract

Rejected source programs remain outside accepted packages. Integrity failures quarantine the package and all derived handoffs. Recovery requires restoring exact inputs, rerunning full QA, reproducing identities and issuing new evidence; manual hash edits are prohibited.
...
Never downgrade an error to warning solely to preserve throughput. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[50_Security_And_Threat_Model]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[52_Operational_Runbook]]
