...
Classify two canonical programs or packages as identical, metadata-only or semantic changes.

## Contract

Diff output identifies changed component definitions, parameters, constraints, labels, state topology and source bindings. The classification supports review and release governance without claiming behavioral equivalence.
...
A semantic change requires a new artifact identity and explicit downstream review. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[40_Deterministic_Replay]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[42_Deterministic_Partitioning]]
