...
Represent path-dependent stop adaptation as finite declarations.

## Contract

The registry supports no trailing and break-even trailing. Break-even declares activation in R and an offset in points. The path-dependent flag is semantic metadata used for descriptor reconciliation and later lattice constraints.
...
Reject trailing declarations when the capability profile lacks stop modification or when activation/offset values are out of range. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[13_Target_And_Exit_Primitives]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[15_Management_Primitives]]
