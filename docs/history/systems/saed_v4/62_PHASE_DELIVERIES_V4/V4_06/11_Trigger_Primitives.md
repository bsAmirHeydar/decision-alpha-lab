...
Represent observable, known-time predicates that guard Treatment state transitions.

## Contract

A trigger references an allowlisted Context feature and a closed operator such as eq, ne, lt, le, gt, ge, in or exists. The trigger language remains declarative and cannot execute arbitrary expressions or inspect future suffixes.
...
Reject unqualified references, future/outcome tokens, unknown operators or cross-Evidence-Role feature access. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[10_Entry_Primitives]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[12_Stop_Primitives]]
