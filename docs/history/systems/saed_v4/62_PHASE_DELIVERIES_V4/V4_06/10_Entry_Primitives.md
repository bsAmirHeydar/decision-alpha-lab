...
Define finite entry mechanisms with explicit timing, feature and capability requirements.

## Contract

Reference entries include market, breakout stop and passive limit. Entry semantics carry quote-age or time-to-live limits, feature references, offsets and exact platform capability dependencies. They are descriptions of a path, not broker instructions.
...
Reject entry primitives whose required capabilities are absent or whose parameters exceed policy limits. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[09_Direction_Primitives]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[11_Trigger_Primitives]]
