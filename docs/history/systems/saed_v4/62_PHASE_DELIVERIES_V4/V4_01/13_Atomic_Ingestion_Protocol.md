---
title: Atomic Ingestion Protocol
status: implemented
version: 1.0.0
phase: V4-01
created: '2026-07-13'
...
Validates complete batches before commit and quarantines invalid inputs without partial state.
...
A batch is preflighted in full: schema, temporal stamps, quarantine state, revision continuity and identity. If any record is invalid, no valid sibling is committed. Failed subjects receive explicit quarantine records. Revision-chain preflight occurs before mutation so a late error cannot leave partial state.
