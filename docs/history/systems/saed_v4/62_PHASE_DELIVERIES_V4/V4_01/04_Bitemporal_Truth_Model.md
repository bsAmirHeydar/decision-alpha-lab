...
Separates event time from known time and prevents future-suffix information from changing historical truth.
...
The kernel stores two independent clocks. `event_time` describes when the market or source-domain event occurred. `known_time` describes the earliest instant at which Alpha Lab was entitled to know the represented fact. Historical queries are evaluated against known time first. A later correction may supersede a revision but cannot retroactively alter an earlier snapshot. This is the central future-suffix defense of the data plane.
