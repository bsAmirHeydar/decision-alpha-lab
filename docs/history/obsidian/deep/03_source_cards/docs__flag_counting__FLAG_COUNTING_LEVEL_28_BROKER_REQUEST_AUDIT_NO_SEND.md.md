...
# Source Card — FLAG_COUNTING_LEVEL_28_BROKER_REQUEST_AUDIT_NO_SEND.md

## Source

[[docs/flag_counting/FLAG_COUNTING_LEVEL_28_BROKER_REQUEST_AUDIT_NO_SEND|docs/flag_counting/FLAG_COUNTING_LEVEL_28_BROKER_REQUEST_AUDIT_NO_SEND.md]]

## Summary

Level 28 audits the no-send broker request chain after Level 27. Level 25 builds the dry-run preview. Level 26 validates the preview. Level 27 records the preview and validator result into a ledger. Level 28 audits whether the chain is internally coherent and still respects the no-send contract. This is still not execution. It does not call `OrderSend`. It does not call `OrderCheck`. It does not use `CTrade`. It does not create a broker request. It does not create a position. It does not calculate volume or account risk. Level 28 does not modify: It remains: The first file is append-only. The second file is the latest snapshot. Level 28 audits: The EA can recalculate the same audit state repeatedly. Level 28 builds a dedupe key from: If the same audit key repeats and duplicate skipping is enabled, the audit ledger does not append the same row again. Every output row explicitly includes:
