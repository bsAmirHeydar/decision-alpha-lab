...
---
type: source_card
source_path: "docs/flag_counting/FLAG_COUNTING_LEVEL_25_BROKER_DRY_RUN_ONLY.md"
source_ext: ".md"
source_size: 4240
empty: false
...
# Source Card — FLAG_COUNTING_LEVEL_25_BROKER_DRY_RUN_ONLY.md
...
Level 25 adds a broker-like request preview after the Level 24 Safety Gate. This is still not execution. It does not call `OrderSend`. It does not use `CTrade`. It does not create a broker request. It does not create a position. It does not calculate volume or account risk. The goal is to see what a future broker request would look like if the safety gate and paper intent allow it. Level 25 does not modify: It remains: The dry-run row includes: For bullish paper intent: For bearish paper intent: These are strings only. They are not MQL5 order requests. Level 25 intentionally sets: This prevents accidental interpretation as a tradable request. Volume and risk sizing are not Level 25 responsibilities. By default, Level 25 requires Level 24 to pass: If the safety gate blocks, Level 25 outputs: By default, Level 25 requires Level 21 paper intent to be allowed: If intent is blocked, Level 25
