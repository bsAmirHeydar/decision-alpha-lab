...
concepts: ["Astro ML", "Path Smoothness", "UI / React"]
entities: ["EXP0013"]
---

# Source Card — ASTRO_DASHBOARD_V5_PRO_CLEAN_GUIDE.md
...
This patch is focused on making the dashboard noticeably more professional and easier to read. removed extra clutter from the header removed the old crowded toggle cluster simplified the cockpit to the most useful cards switched the display heat scale to: low = red mid = yellow high = green improved spacing between rows and cards replaced the oversized bottom panel with a compact oscillator board Shows: Path Quality Micro M1 Macro Background Raw Axes Diagnostics Shows a larger version of the selected section: Path Micro Regime Macro Raw The lower block is no longer a huge stretched history area. It is now a compact recent-history board that shows: metric name current value current mini bar short sparkline history Top row: COCKPIT PATH MICRO REGIME MACRO RAW Second row: TEXT ON/OFF OSC ON/OFF RELOAD This version is meant to be visually cleaner while still keeping all major astro states av

## Concepts

[[docs/obsidian_deep/02_concepts/Astro_ML|Astro ML]], [[docs/obsidian_deep/02_concepts/Path_Smoothness|Path Smoothness]], [[docs/obsidian_deep/02_concepts/UI____React|UI / React]]
