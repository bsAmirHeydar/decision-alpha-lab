...
Rollback removes only files in `EXP0019_FP_I13_FILE_INDEX.txt` and restores modified files from the preceding accepted FP-I12 commit. Generated audit evidence is retained. FP-I13 checkpoints are ignored by FP-I12 because their version and release manifest are incompatible.
## Implementation linkage

The **76 Rollback Plan** contract is implemented through the versioned `fp_i13_release` Python reference package, the MQL5 `I13` include boundary, the production indicator integration, and the FP-I13 release self-test. The implementation must consume accepted FP-I10–FP-I12 outputs rather than reconstructing context from chart objects.
