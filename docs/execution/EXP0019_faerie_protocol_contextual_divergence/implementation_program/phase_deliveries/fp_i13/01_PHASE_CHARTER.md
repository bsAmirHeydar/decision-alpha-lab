...
Turn the complete Faerie Protocol indicator into a release candidate whose semantics, visual inventory, operator state, and machine outputs are reproducible under long history, incremental operation, restart, data repair, chart timeframe changes, and concurrent chart instances.

## In scope

- deterministic replay harness;
- canonical replay fixtures and event hashes;
- full/incremental inventory comparison;
- checkpoint rejection and deterministic rebuild;
- timeframe invariance;
- multi-instance collision testing;
- performance telemetry and budgets;
- object-budget degradation;
- release profiles and release manifest;
- production acceptance evidence;
- user and operator guides.

## Out of scope

- Diagnostic EA implementation belongs to FP-I14.
- Paper execution belongs to FP-I15.
- Live quota consumption remains blocked by `FP-DEC-012`.
- Performance work may not remove semantic evidence, suppressed signals, invalidations, or confirmed historical objects.
## Implementation linkage

The **01 Phase Charter** contract is implemented through the versioned `fp_i13_release` Python reference package, the MQL5 `I13` include boundary, the production indicator integration, and the FP-I13 release self-test. The implementation must consume accepted FP-I10–FP-I12 outputs rather than reconstructing context from chart objects.
