...
| Gate | Source package | Local terminal |
|---|---:|---:|
| Full/incremental parity | required | optional repeat |
| Restart parity | required | required |
| Timeframe invariance | required | required |
| Multi-instance isolation | required | required |
| Performance budgets | reference | terminal measurement |
| Static MQL5 boundary | required | n/a |
| MetaEditor compile | pending | required |
| Visual/operator inspection | fixture/static | required |
| `FP-DEC-012` | UNSET | UNSET |

Production release readiness is true only when every required local gate is attached and passed.
## Implementation linkage

The **80 Production Acceptance Matrix** contract is implemented through the versioned `fp_i13_release` Python reference package, the MQL5 `I13` include boundary, the production indicator integration, and the FP-I13 release self-test. The implementation must consume accepted FP-I10–FP-I12 outputs rather than reconstructing context from chart objects.
