---
title: "Noncritical Projection Degradation"
...
language: en
---
# Noncritical Projection Degradation

When projection exceeds a soft budget, the scheduler keeps critical current evidence first and defers noncritical historical projection. Semantic facts remain complete. Deferred object IDs and reason codes are recorded.

Critical priority includes current health, active WW, current quota winner, and confirmed current-session signals. Historical sessions and old suppressed overlays are lower priority but remain available for paged audit reconstruction.
## Implementation linkage

The **64 Degradation Policy** contract is implemented through the versioned `fp_i13_release` Python reference package, the MQL5 `I13` include boundary, the production indicator integration, and the FP-I13 release self-test. The implementation must consume accepted FP-I10–FP-I12 outputs rather than reconstructing context from chart objects.
