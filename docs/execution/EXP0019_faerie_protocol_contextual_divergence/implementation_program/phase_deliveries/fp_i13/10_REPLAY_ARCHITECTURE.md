...
The replay harness consumes an ordered stream of versioned events. Each event contains sequence, event time, type, semantic identity, payload hash, source revision, historical flag, and closed reason codes.

The reducer maintains append-only event-chain identity and materialized inventories for semantic facts, visual facts, alert identities, suppressed historical alerts, audit export identities, and health events. Duplicate event IDs with identical hashes are ignored and audited; an ID collision with different content fails closed.

The harness has three modes: `FULL`, `INCREMENTAL`, and `RESTART`. All modes use the same reducer implementation. No test-only semantic branch is permitted.
## Implementation linkage

The **10 Replay Architecture** contract is implemented through the versioned `fp_i13_release` Python reference package, the MQL5 `I13` include boundary, the production indicator integration, and the FP-I13 release self-test. The implementation must consume accepted FP-I10–FP-I12 outputs rather than reconstructing context from chart objects.
