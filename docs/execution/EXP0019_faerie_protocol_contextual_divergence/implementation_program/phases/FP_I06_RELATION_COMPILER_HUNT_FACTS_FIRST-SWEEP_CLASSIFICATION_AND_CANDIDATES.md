...
Compile AL/AN/LN/NA/NL/NN relation instances, consume generic hunt facts, enforce M1-only first-sweep semantics, assign Hunter/Protected roles, and emit raw candidates.
...
- relation instance compiler
- check-window activation
- M1 hunt adapter
- same-minute ambiguity
- candidate identity
- bullish/bearish role classification

## Explicit non-goals

- No host-timeframe confirmation
- No WW policy
- No quota/execution
...
- [ ] all six non-WW relations pass both directions.
- [ ] same-M1 dual touch produces no invented order.
- [ ] duplicate scans emit no duplicate candidate.
- [ ] missing data blocks classification.
