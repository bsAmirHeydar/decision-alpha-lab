...
## 2026-06-28 - Main-chart contract repair V3

This pass repairs the chart contract after Hook/ND became visually dominant. Hook/ND remains a context layer, while F1/F2/F3 remains the primary visible structure layer.

Changes:

- build Hook-derived F1 roots first and also inspect raw-origin fail-open roots when fail-open is enabled;
- canonicalize duplicate visual bodies after detection instead of letting Hook coverage starve flags;
- hide duplicate root sequences together with all descendants;
- hide orphan F2/F3 events when their parent is no longer visible;
- enforce that F2 below the F1 size contract is audit-only and cannot parent F3;
- disable same-direction chain pruning by default in the research chart;
- disable origin/internal/Hook count labels by default to separate main-chart rendering from audit rendering;
