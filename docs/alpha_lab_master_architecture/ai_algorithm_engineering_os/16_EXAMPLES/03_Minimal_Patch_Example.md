...
> [!abstract] Purpose
> Show a safe staged implementation of a new state field.
...
Show a safe staged implementation of a new state field. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Minimal Patch Example** as an explicit engineering contract, not an informal preference.
- Keep adding consumption state without rewriting the whole indicator traceable to project intent, domain rules, and measurable evidence.
...
1. Collect the minimum authoritative context required for adding consumption state without rewriting the whole indicator.
...
- A versioned record of adding consumption state without rewriting the whole indicator.
...
Add enum and struct fields with defaults; no behavioral change. Compile and characterize.

### Patch B

Add pure transition helper and unit/scenario tests; no renderer connection.

### Patch C

Integrate candidate filtering behind diagnostics; compare old/new logs.

### Patch D

Connect renderer and stale-object cleanup using deterministic IDs.

### Patch E

Remove temporary comparison flag after validation and update docs.
