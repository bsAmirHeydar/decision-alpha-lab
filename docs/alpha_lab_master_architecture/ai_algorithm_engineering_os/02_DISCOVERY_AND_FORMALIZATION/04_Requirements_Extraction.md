...
> [!abstract] Purpose
> Transform narrative intent into atomic functional and non-functional requirements.
...
Transform narrative intent into atomic functional and non-functional requirements. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Requirements Extraction** as an explicit engineering contract, not an informal preference.
- Keep traceable requirements with identifiers and tests traceable to project intent, domain rules, and measurable evidence.
...
- Use SHALL for mandatory behavior and SHOULD only for accepted flexibility.

## 2. Required Procedure

1. Collect the minimum authoritative context required for traceable requirements with identifiers and tests.
...
- A versioned record of traceable requirements with identifiers and tests.
...
```text
Act as a domain analyst. For requirements extraction, separate confirmed statements, ambiguous terms, assumptions, counterexamples, constraints, and questions that can change implementation. Do not propose code.
```
