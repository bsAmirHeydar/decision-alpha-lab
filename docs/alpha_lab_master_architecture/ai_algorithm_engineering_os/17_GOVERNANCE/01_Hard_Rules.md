...
> [!abstract] Purpose
> Define rules that no AI agent, developer, deadline, or optimization may bypass silently.
...
Define rules that no AI agent, developer, deadline, or optimization may bypass silently. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Hard Rules** as an explicit engineering contract, not an informal preference.
- Keep non-negotiable project controls traceable to project intent, domain rules, and measurable evidence.
...
- Hard rules require named authority and an ADR to change; temporary exceptions are explicit, scoped, and expiring.

## 2. Required Procedure

1. Collect the minimum authoritative context required for non-negotiable project controls.
...
- A versioned record of non-negotiable project controls.
...
No invented concepts; approved ontology and definitions govern behavior.

### Correctness

No consumed/stale/future-derived reference may influence a live decision.

### Engineering

No unverified broad rewrite, hidden assumption, or completion claim without evidence.

### Security

No secrets in prompts, source control, logs, screenshots, or release archives.

### Delivery

Every production change has identity, verification, rollback, and durable documentation.
