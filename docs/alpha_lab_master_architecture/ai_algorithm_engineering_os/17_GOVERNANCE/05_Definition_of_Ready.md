...
> [!abstract] Purpose
> Specify the minimum evidence required before implementation begins.
...
Specify the minimum evidence required before implementation begins. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Definition of Ready** as an explicit engineering contract, not an informal preference.
- Keep pre-code readiness traceable to project intent, domain rules, and measurable evidence.
...
- Ready means uncertainty is bounded enough to code safely, not that all uncertainty is eliminated.

## 2. Required Procedure

1. Collect the minimum authoritative context required for pre-code readiness.
...
- A versioned record of pre-code readiness.
...
Current and desired behavior are observable.

### Semantics

Terms, timing, scope, constraints, assumptions, and non-goals are explicit.

### Design

State, transitions, invariants, contracts, and test approach are approved.

### Execution

Files, commands, owner, risk, rollback, and context packet are available.
