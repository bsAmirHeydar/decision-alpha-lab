...
> [!abstract] Purpose
> Define detection, propagation, recovery, fallback, and observability for failure conditions.
...
Define detection, propagation, recovery, fallback, and observability for failure conditions. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Error Handling** as an explicit engineering contract, not an informal preference.
- Keep explicit failure semantics instead of silent continuation traceable to project intent, domain rules, and measurable evidence.
...
- Every caught error requires a reason, action, and diagnostic policy.

## 2. Required Procedure

1. Collect the minimum authoritative context required for explicit failure semantics instead of silent continuation.
...
- A versioned record of explicit failure semantics instead of silent continuation.
...
```text
Implement only the approved scope for error handling. First inspect the relevant files and contracts. State assumptions, files to modify, files not to modify, preserved invariants, risks, and verification commands. Produce the smallest safe patch; do not rewrite unrelated code.
```
