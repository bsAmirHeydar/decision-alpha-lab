...
> [!abstract] Purpose
> Build task-specific context packets that maximize relevance while preventing context pollution.
...
Build task-specific context packets that maximize relevance while preventing context pollution. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Context Engineering** as an explicit engineering contract, not an informal preference.
- Keep selection, compression, authority, freshness, and ordering of AI context traceable to project intent, domain rules, and measurable evidence.
...
- More context is not automatically better; stale or conflicting context increases confident error.

## 2. Required Procedure

1. Collect the minimum authoritative context required for selection, compression, authority, freshness, and ordering of AI context.
...
- A versioned record of selection, compression, authority, freshness, and ordering of AI context.
