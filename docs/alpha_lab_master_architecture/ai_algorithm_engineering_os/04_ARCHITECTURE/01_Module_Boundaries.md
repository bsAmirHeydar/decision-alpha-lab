...
> [!abstract] Purpose
> Partition responsibilities so changes remain local and contracts remain stable.
...
Partition responsibilities so changes remain local and contracts remain stable. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Module Boundaries** as an explicit engineering contract, not an informal preference.
- Keep cohesion, coupling, and responsibility boundaries traceable to project intent, domain rules, and measurable evidence.
...
- A module should own one reason to change and expose the smallest useful interface.

## 2. Required Procedure

1. Collect the minimum authoritative context required for cohesion, coupling, and responsibility boundaries.
...
- A versioned record of cohesion, coupling, and responsibility boundaries.
