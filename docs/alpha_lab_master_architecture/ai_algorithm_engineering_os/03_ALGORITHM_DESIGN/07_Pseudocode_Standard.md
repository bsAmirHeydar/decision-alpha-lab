...
> [!abstract] Purpose
> Express approved logic clearly enough that implementation is mechanical and reviewable.
...
- Treat **Pseudocode Standard** as an explicit engineering contract, not an informal preference.
- Keep language-neutral algorithm description traceable to project intent, domain rules, and measurable evidence.
...
- Pseudocode names domain operations and state transitions, not syntax trivia.

## 2. Required Procedure

1. Collect the minimum authoritative context required for language-neutral algorithm description.
...
- A versioned record of language-neutral algorithm description.
...
```text
Design language-neutral algorithm description. Provide entities, state variables, events, guards, transitions, invariants, pseudocode, complexity, failure modes, replay behavior, and tests. Compare alternatives before recommending one. Do not write production code.
```
