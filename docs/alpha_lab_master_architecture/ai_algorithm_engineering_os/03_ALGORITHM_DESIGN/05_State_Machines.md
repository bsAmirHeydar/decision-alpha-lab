...
> [!abstract] Purpose
> Represent lifecycle behavior through explicit states, events, guards, transitions, and terminal conditions.
...
Represent lifecycle behavior through explicit states, events, guards, transitions, and terminal conditions. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **State Machines** as an explicit engineering contract, not an informal preference.
- Keep deterministic lifecycle modeling traceable to project intent, domain rules, and measurable evidence.
...
- Illegal transitions should fail visibly rather than mutate silently.

## 2. Required Procedure

1. Collect the minimum authoritative context required for deterministic lifecycle modeling.
...
- A versioned record of deterministic lifecycle modeling.
...
```text
Design deterministic lifecycle modeling. Provide entities, state variables, events, guards, transitions, invariants, pseudocode, complexity, failure modes, replay behavior, and tests. Compare alternatives before recommending one. Do not write production code.
```
