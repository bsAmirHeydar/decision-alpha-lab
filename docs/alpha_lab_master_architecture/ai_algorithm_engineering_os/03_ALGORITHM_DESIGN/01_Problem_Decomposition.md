...
> [!abstract] Purpose
> Split a large behavior into composable responsibilities with clear interfaces.
...
Split a large behavior into composable responsibilities with clear interfaces. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Problem Decomposition** as an explicit engineering contract, not an informal preference.
- Keep functional decomposition without losing end-to-end semantics traceable to project intent, domain rules, and measurable evidence.
...
- Decompose by responsibility and state ownership, not arbitrary file size.

## 2. Required Procedure

1. Collect the minimum authoritative context required for functional decomposition without losing end-to-end semantics.
...
- A versioned record of functional decomposition without losing end-to-end semantics.
...
```text
Design functional decomposition without losing end-to-end semantics. Provide entities, state variables, events, guards, transitions, invariants, pseudocode, complexity, failure modes, replay behavior, and tests. Compare alternatives before recommending one. Do not write production code.
```
