---
id: AIEOS-1011F5729F
title: "Entities, States, and Events"
type: guide
status: active
...
tags:
  ...
  - guide
---
# Entities, States, and Events

> [!abstract] Purpose
> Model what exists, what persists, and what causes change.
...
Model what exists, what persists, and what causes change. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Entities, States, and Events** as an explicit engineering contract, not an informal preference.
- Keep the minimum state model required for correct future decisions traceable to project intent, domain rules, and measurable evidence.
...
- Do not store derivable state unless the performance trade-off is measured.

## 2. Required Procedure

1. Collect the minimum authoritative context required for the minimum state model required for correct future decisions.
...
- A versioned record of the minimum state model required for correct future decisions.
...
```text
Design the minimum state model required for correct future decisions. Provide entities, state variables, events, guards, transitions, invariants, pseudocode, complexity, failure modes, replay behavior, and tests. Compare alternatives before recommending one. Do not write production code.
```
