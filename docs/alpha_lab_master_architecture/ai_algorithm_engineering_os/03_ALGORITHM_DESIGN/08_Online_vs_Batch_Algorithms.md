---
id: AIEOS-8AF6C3446E
title: "Online vs Batch Algorithms"
type: guide
status: active
...
> [!abstract] Purpose
> Choose correctly between incremental event processing and full-dataset recomputation.
...
Choose correctly between incremental event processing and full-dataset recomputation. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Online vs Batch Algorithms** as an explicit engineering contract, not an informal preference.
- Keep latency, reconstruction, consistency, and update semantics traceable to project intent, domain rules, and measurable evidence.
...
- Live algorithms require explicit late-event, duplicate-event, and restart behavior.

## 2. Required Procedure

1. Collect the minimum authoritative context required for latency, reconstruction, consistency, and update semantics.
...
- A versioned record of latency, reconstruction, consistency, and update semantics.
...
```text
Design latency, reconstruction, consistency, and update semantics. Provide entities, state variables, events, guards, transitions, invariants, pseudocode, complexity, failure modes, replay behavior, and tests. Compare alternatives before recommending one. Do not write production code.
```
