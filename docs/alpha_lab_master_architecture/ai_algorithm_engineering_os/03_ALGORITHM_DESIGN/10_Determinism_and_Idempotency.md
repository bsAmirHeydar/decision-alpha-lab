...
> [!abstract] Purpose
> Prevent duplicate effects and unstable results across repeated calculation.
...
Prevent duplicate effects and unstable results across repeated calculation. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Determinism and Idempotency** as an explicit engineering contract, not an informal preference.
- Keep stable ordering, keys, side effects, and retries traceable to project intent, domain rules, and measurable evidence.
...
- Persistent effects require deterministic identity and upsert/delete semantics.

## 2. Required Procedure

1. Collect the minimum authoritative context required for stable ordering, keys, side effects, and retries.
...
- A versioned record of stable ordering, keys, side effects, and retries.
...
```text
Design stable ordering, keys, side effects, and retries. Provide entities, state variables, events, guards, transitions, invariants, pseudocode, complexity, failure modes, replay behavior, and tests. Compare alternatives before recommending one. Do not write production code.
```
