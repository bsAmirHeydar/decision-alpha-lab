...
> [!abstract] Purpose
> Add bounded behavior to an existing system while preserving compatibility and invariants.
...
Add bounded behavior to an existing system while preserving compatibility and invariants. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Feature Workflow** as an explicit engineering contract, not an informal preference.
- Keep feature intake, impact analysis, minimal patch, and release traceable to project intent, domain rules, and measurable evidence.
...
1. Characterize current behavior and identify the exact extension point.
2. Write requirement delta and non-goals; avoid rewriting the parent subsystem.
3. Update affected tests before or with the implementation.
4. Run targeted and expanded regression according to dependency impact.
...
- A versioned record of feature intake, impact analysis, minimal patch, and release.
...
```text
Execute the Feature Workflow. Do not skip directly to coding. At each phase produce the required artifact, list unresolved risks, and stop on failed gates. Use repository evidence and report all tool results honestly.
```
