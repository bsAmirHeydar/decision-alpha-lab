...
> [!abstract] Purpose
> Promote an experimental algorithm through validation, hardening, operational controls, and release.
...
- Treat **Research to Production** as an explicit engineering contract, not an informal preference.
- Keep transition from exploratory evidence to production reliability traceable to project intent, domain rules, and measurable evidence.
...
1. Freeze experiment version, data, assumptions, metrics, and result interpretation.
2. Remove notebook/chat-only logic by writing a formal specification.
3. Implement production core independently from research UI and ad hoc data paths.
4. Validate causal parity, robustness, performance, observability, risk limits, and rollback.
...
- A versioned record of transition from exploratory evidence to production reliability.
...
```text
Execute the Research to Production. Do not skip directly to coding. At each phase produce the required artifact, list unresolved risks, and stop on failed gates. Use repository evidence and report all tool results honestly.
```
