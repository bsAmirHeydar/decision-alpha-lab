...
> [!abstract] Purpose
> Change structure through behavior-preserving stages backed by characterization tests.
...
Change structure through behavior-preserving stages backed by characterization tests. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Refactor Workflow** as an explicit engineering contract, not an informal preference.
- Keep safe architectural cleanup traceable to project intent, domain rules, and measurable evidence.
...
1. Capture the observable baseline and public contracts.
2. Separate movement/renaming from semantic simplification.
3. Keep each step buildable and revertible.
4. Do not mix new behavior unless explicitly split into a later patch.
...
- A versioned record of safe architectural cleanup.
...
```text
Execute the Refactor Workflow. Do not skip directly to coding. At each phase produce the required artifact, list unresolved risks, and stop on failed gates. Use repository evidence and report all tool results honestly.
```
