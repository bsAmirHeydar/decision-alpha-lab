...
> [!abstract] Purpose
> Measure latency, throughput, allocations, redraws, history scaling, and resource use against budgets.
...
Measure latency, throughput, allocations, redraws, history scaling, and resource use against budgets. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Performance Testing** as an explicit engineering contract, not an informal preference.
- Keep repeatable workload-based performance evidence traceable to project intent, domain rules, and measurable evidence.
...
- Record environment and dataset; otherwise numbers are not comparable.

## 2. Required Procedure

1. Collect the minimum authoritative context required for repeatable workload-based performance evidence.
...
- A versioned record of repeatable workload-based performance evidence.
...
```text
Create a verification plan for repeatable workload-based performance evidence. Include normal, boundary, invalid, duplicate, missing, reordered, restart, replay, scale, and regression cases. For each test specify setup, event sequence, expected state, expected output, failure signal, and evidence.
```
