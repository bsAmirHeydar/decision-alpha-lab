...
> [!abstract] Purpose
> Design a layered verification system aligned with failure cost and architecture.
...
Design a layered verification system aligned with failure cost and architecture. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Test Strategy** as an explicit engineering contract, not an informal preference.
- Keep the allocation of unit, integration, replay, visual, performance, and release tests traceable to project intent, domain rules, and measurable evidence.
...
- Tests should fail for meaningful behavioral reasons, not implementation trivia.

## 2. Required Procedure

1. Collect the minimum authoritative context required for the allocation of unit, integration, replay, visual, performance, and release tests.
...
- A versioned record of the allocation of unit, integration, replay, visual, performance, and release tests.
...
```text
Create a verification plan for the allocation of unit, integration, replay, visual, performance, and release tests. Include normal, boundary, invalid, duplicate, missing, reordered, restart, replay, scale, and regression cases. For each test specify setup, event sequence, expected state, expected output, failure signal, and evidence.
```
