...
> [!abstract] Purpose
> Create, update, deduplicate, hide, and delete chart objects in sync with algorithm state.
...
Create, update, deduplicate, hide, and delete chart objects in sync with algorithm state. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Chart Object Lifecycle** as an explicit engineering contract, not an informal preference.
- Keep object ownership and lifecycle traceable to project intent, domain rules, and measurable evidence.
...
- Every object has deterministic ID, owner prefix, source state, and cleanup rule.

## 2. Required Procedure

1. Collect the minimum authoritative context required for object ownership and lifecycle.
...
- A versioned record of object ownership and lifecycle.
...
```text
Review this MQL5 task for object ownership and lifecycle. Inspect callback lifecycle, series indexing, history readiness, persistent state, object IDs, cleanup, recalculation, performance, and terminal-specific errors. Produce a minimal patch and a Strategy Tester plus manual chart verification matrix.
```
