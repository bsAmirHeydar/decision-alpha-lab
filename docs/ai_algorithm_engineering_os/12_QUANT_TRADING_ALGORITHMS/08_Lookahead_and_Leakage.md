...
> [!abstract] Purpose
> Eliminate future information, revised labels, improper cross-validation, and hidden dataset contamination.
...
Eliminate future information, revised labels, improper cross-validation, and hidden dataset contamination. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Lookahead and Leakage** as an explicit engineering contract, not an informal preference.
- Keep causal data availability and experimental integrity traceable to project intent, domain rules, and measurable evidence.
...
- Detection confirmation time must be distinguished from the historical location it labels.

## 2. Required Procedure

1. Collect the minimum authoritative context required for causal data availability and experimental integrity.
...
- A versioned record of causal data availability and experimental integrity.
...
```text
Formalize causal data availability and experimental integrity for a causal trading system. Use only the approved project ontology. Define event time, knowledge time, state lifecycle, invalidation, replay behavior, lookahead safeguards, audit fields, and tests. Do not infer generic trading rules.
```
