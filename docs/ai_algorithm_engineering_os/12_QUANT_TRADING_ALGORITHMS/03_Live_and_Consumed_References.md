---
id: AIEOS-ED7D9958A9
title: "Live and Consumed References"
type: guide
status: active
domain: quant
version: 1.0.0
...
tags:
  ...
  - guide
---
# Live and Consumed References

> [!abstract] Purpose
> Track whether extrema, zones, structures, or evidence remain valid for future decisions.
...
Track whether extrema, zones, structures, or evidence remain valid for future decisions. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Live and Consumed References** as an explicit engineering contract, not an informal preference.
- Keep reference lifecycle, invalidation, consumption, and reuse traceable to project intent, domain rules, and measurable evidence.
...
- Consumed references may remain in audit history but cannot silently return to active selection.

## 2. Required Procedure

1. Collect the minimum authoritative context required for reference lifecycle, invalidation, consumption, and reuse.
...
- A versioned record of reference lifecycle, invalidation, consumption, and reuse.
...
```text
Formalize reference lifecycle, invalidation, consumption, and reuse for a causal trading system. Use only the approved project ontology. Define event time, knowledge time, state lifecycle, invalidation, replay behavior, lookahead safeguards, audit fields, and tests. Do not infer generic trading rules.
```
