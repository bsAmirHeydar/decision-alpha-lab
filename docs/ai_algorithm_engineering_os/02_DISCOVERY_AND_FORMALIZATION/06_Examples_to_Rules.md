...
> [!abstract] Purpose
> Generalize examples and counterexamples into rules without overfitting to screenshots or isolated cases.
...
Generalize examples and counterexamples into rules without overfitting to screenshots or isolated cases. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Examples to Rules** as an explicit engineering contract, not an informal preference.
- Keep inductive formalization from observed cases traceable to project intent, domain rules, and measurable evidence.
...
- For every inferred rule, search for a counterexample and boundary case.

## 2. Required Procedure

1. Collect the minimum authoritative context required for inductive formalization from observed cases.
...
- A versioned record of inductive formalization from observed cases.
...
```text
Act as a domain analyst. For examples to rules, separate confirmed statements, ambiguous terms, assumptions, counterexamples, constraints, and questions that can change implementation. Do not propose code.
```
