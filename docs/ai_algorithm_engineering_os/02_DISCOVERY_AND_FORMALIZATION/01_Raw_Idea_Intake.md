...
> [!abstract] Purpose
> Capture intuition, pain, examples, and desired effect without prematurely committing to implementation.
...
Capture intuition, pain, examples, and desired effect without prematurely committing to implementation. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Raw Idea Intake** as an explicit engineering contract, not an informal preference.
- Keep the faithful intake of a raw request traceable to project intent, domain rules, and measurable evidence.
...
- Preserve the user’s vocabulary before normalization.

## 2. Required Procedure

1. Collect the minimum authoritative context required for the faithful intake of a raw request.
...
- A versioned record of the faithful intake of a raw request.
...
```text
Act as a domain analyst. For raw idea intake, separate confirmed statements, ambiguous terms, assumptions, counterexamples, constraints, and questions that can change implementation. Do not propose code.
```
