...
> [!abstract] Purpose
> Document intent, invariants, non-obvious trade-offs, and operational behavior without narrating syntax.
...
Document intent, invariants, non-obvious trade-offs, and operational behavior without narrating syntax. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Comments and Documentation** as an explicit engineering contract, not an informal preference.
- Keep durable explanation close to code and in the vault traceable to project intent, domain rules, and measurable evidence.
...
- Comments explain why a constraint exists; code explains what it executes.

## 2. Required Procedure

1. Collect the minimum authoritative context required for durable explanation close to code and in the vault.
...
- A versioned record of durable explanation close to code and in the vault.
...
```text
Implement only the approved scope for comments and documentation. First inspect the relevant files and contracts. State assumptions, files to modify, files not to modify, preserved invariants, risks, and verification commands. Produce the smallest safe patch; do not rewrite unrelated code.
```
