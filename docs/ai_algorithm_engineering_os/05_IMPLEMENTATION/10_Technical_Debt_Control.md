...
> [!abstract] Purpose
> Record and constrain temporary compromises so speed does not silently become permanent fragility.
...
Record and constrain temporary compromises so speed does not silently become permanent fragility. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Technical Debt Control** as an explicit engineering contract, not an informal preference.
- Keep debt identity, interest, owner, trigger, and repayment traceable to project intent, domain rules, and measurable evidence.
...
- “Temporary” without a removal trigger is permanent debt.

## 2. Required Procedure

1. Collect the minimum authoritative context required for debt identity, interest, owner, trigger, and repayment.
...
- A versioned record of debt identity, interest, owner, trigger, and repayment.
...
```text
Implement only the approved scope for technical debt control. First inspect the relevant files and contracts. State assumptions, files to modify, files not to modify, preserved invariants, risks, and verification commands. Produce the smallest safe patch; do not rewrite unrelated code.
```
