...
> [!abstract] Purpose
> Connect a new module to existing callbacks, state, APIs, rendering, and deployment safely.
...
Connect a new module to existing callbacks, state, APIs, rendering, and deployment safely. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Integration Protocol** as an explicit engineering contract, not an informal preference.
- Keep boundary wiring and staged activation traceable to project intent, domain rules, and measurable evidence.
...
- Integrate behind diagnostics or flags when risk cannot be isolated otherwise.

## 2. Required Procedure

1. Collect the minimum authoritative context required for boundary wiring and staged activation.
...
- A versioned record of boundary wiring and staged activation.
...
```text
Implement only the approved scope for integration protocol. First inspect the relevant files and contracts. State assumptions, files to modify, files not to modify, preserved invariants, risks, and verification commands. Produce the smallest safe patch; do not rewrite unrelated code.
```
