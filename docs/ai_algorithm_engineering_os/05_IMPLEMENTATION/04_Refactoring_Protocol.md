...
> [!abstract] Purpose
> Improve internal structure without silently changing externally observable behavior.
...
Improve internal structure without silently changing externally observable behavior. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Refactoring Protocol** as an explicit engineering contract, not an informal preference.
- Keep behavior preservation, characterization tests, and staged cleanup traceable to project intent, domain rules, and measurable evidence.
...
- A refactor begins with a behavior baseline and ends with the same baseline passing.

## 2. Required Procedure

1. Collect the minimum authoritative context required for behavior preservation, characterization tests, and staged cleanup.
...
- A versioned record of behavior preservation, characterization tests, and staged cleanup.
...
```text
Implement only the approved scope for refactoring protocol. First inspect the relevant files and contracts. State assumptions, files to modify, files not to modify, preserved invariants, risks, and verification commands. Produce the smallest safe patch; do not rewrite unrelated code.
```
