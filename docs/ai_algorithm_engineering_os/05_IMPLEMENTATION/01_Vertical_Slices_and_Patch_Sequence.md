---
id: AIEOS-341A560BDF
title: "Vertical Slices and Patch Sequence"
type: guide
status: active
...
tags:
  ...
  - guide
---
# Vertical Slices and Patch Sequence

> [!abstract] Purpose
> Break implementation into observable, reversible increments that preserve a coherent architecture.
...
Break implementation into observable, reversible increments that preserve a coherent architecture. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Vertical Slices and Patch Sequence** as an explicit engineering contract, not an informal preference.
- Keep sequencing types, state, logic, integration, rendering, diagnostics, and optimization traceable to project intent, domain rules, and measurable evidence.
...
- A slice should produce verifiable behavior or enable the next slice without speculative infrastructure.

## 2. Required Procedure

1. Collect the minimum authoritative context required for sequencing types, state, logic, integration, rendering, diagnostics, and optimization.
...
- A versioned record of sequencing types, state, logic, integration, rendering, diagnostics, and optimization.
...
```text
Implement only the approved scope for vertical slices and patch sequence. First inspect the relevant files and contracts. State assumptions, files to modify, files not to modify, preserved invariants, risks, and verification commands. Produce the smallest safe patch; do not rewrite unrelated code.
```
