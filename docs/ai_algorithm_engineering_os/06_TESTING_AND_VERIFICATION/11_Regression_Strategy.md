...
> [!abstract] Purpose
> Protect existing behavior using risk-based suites, baselines, and targeted reruns.
...
Protect existing behavior using risk-based suites, baselines, and targeted reruns. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Regression Strategy** as an explicit engineering contract, not an informal preference.
- Keep selection and maintenance of regression evidence traceable to project intent, domain rules, and measurable evidence.
...
- A changed shared helper expands regression scope beyond the feature.

## 2. Required Procedure

1. Collect the minimum authoritative context required for selection and maintenance of regression evidence.
...
- A versioned record of selection and maintenance of regression evidence.
...
```text
Create a verification plan for selection and maintenance of regression evidence. Include normal, boundary, invalid, duplicate, missing, reordered, restart, replay, scale, and regression cases. For each test specify setup, event sequence, expected state, expected output, failure signal, and evidence.
```
