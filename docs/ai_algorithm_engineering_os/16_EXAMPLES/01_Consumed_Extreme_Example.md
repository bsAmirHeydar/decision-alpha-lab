---
id: AIEOS-2171E783F8
title: "Consumed Extreme Example"
type: example
status: active
domain: example
version: 1.0.0
...
tags:
  ...
  - example
---
# Consumed Extreme Example

> [!abstract] Purpose
> Show how a trading intuition becomes a formal lifecycle rule.
...
Show how a trading intuition becomes a formal lifecycle rule. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Consumed Extreme Example** as an explicit engineering contract, not an informal preference.
- Keep a price extreme that becomes ineligible after consumption traceable to project intent, domain rules, and measurable evidence.
...
1. Collect the minimum authoritative context required for a price extreme that becomes ineligible after consumption.
...
- A versioned record of a price extreme that becomes ineligible after consumption.
...
“When a low has already been taken, it must not be used later as a divergence reference.”

### Formal rule

An `ExtremeReference` has states `ACTIVE`, `CONSUMED`, and `ARCHIVED`. A qualifying sweep event transitions `ACTIVE → CONSUMED`. Candidate selection filters strictly for `ACTIVE`. Historical display may include consumed references, but analysis may not reactivate them without a new entity identity.

### Invariant

`INV-EXT-001: no candidate references an extreme whose state is not ACTIVE at candidate knowledge time.`

### Tests

Sweep by wick, break by close, repeated sweep, equal low, restart after consumption, historical replay, and a new lower low creating a distinct active entity.
