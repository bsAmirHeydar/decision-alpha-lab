...
> [!abstract] Purpose
> Give every change a stable identifier, purpose, scope, risk, and lifecycle.
...
Give every change a stable identifier, purpose, scope, risk, and lifecycle. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Patch Identity** as an explicit engineering contract, not an informal preference.
- Keep traceability across specification, diff, tests, release, and rollback traceable to project intent, domain rules, and measurable evidence.
...
- One patch identity maps to one coherent reason for change.

## 2. Required Procedure

1. Collect the minimum authoritative context required for traceability across specification, diff, tests, release, and rollback.
...
- A versioned record of traceability across specification, diff, tests, release, and rollback.
