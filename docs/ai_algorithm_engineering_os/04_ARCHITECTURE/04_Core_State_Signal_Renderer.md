---
id: AIEOS-651AAC34D4
title: "Core–State–Signal–Renderer Architecture"
type: guide
status: active
domain: architecture
version: 1.0.0
...
tags:
  ...
  - guide
---
# Core–State–Signal–Renderer Architecture

> [!abstract] Purpose
> Separate domain calculation, state management, derived signals, and presentation.
...
Separate domain calculation, state management, derived signals, and presentation. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Core–State–Signal–Renderer Architecture** as an explicit engineering contract, not an informal preference.
- Keep layered separation for algorithmic and chart systems traceable to project intent, domain rules, and measurable evidence.
...
- Rendering may observe algorithm output but must not define domain truth.

## 2. Required Procedure

1. Collect the minimum authoritative context required for layered separation for algorithmic and chart systems.
...
- A versioned record of layered separation for algorithmic and chart systems.
