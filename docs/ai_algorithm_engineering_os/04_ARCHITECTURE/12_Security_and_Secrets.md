...
> [!abstract] Purpose
> Protect credentials, license material, private endpoints, and sensitive logs.
...
Protect credentials, license material, private endpoints, and sensitive logs. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Security and Secrets** as an explicit engineering contract, not an informal preference.
- Keep secret boundaries and least privilege traceable to project intent, domain rules, and measurable evidence.
...
- Secrets never belong in prompts, source control, screenshots, or diagnostic dumps.

## 2. Required Procedure

1. Collect the minimum authoritative context required for secret boundaries and least privilege.
...
- A versioned record of secret boundaries and least privilege.
