...
> [!abstract] Purpose
> Design data flow, caching, incremental updates, and allocation before micro-optimization.
...
Design data flow, caching, incremental updates, and allocation before micro-optimization. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Performance Architecture** as an explicit engineering contract, not an informal preference.
- Keep system-level latency and resource behavior traceable to project intent, domain rules, and measurable evidence.
...
- Measure the dominant work before optimizing syntax.

## 2. Required Procedure

1. Collect the minimum authoritative context required for system-level latency and resource behavior.
...
- A versioned record of system-level latency and resource behavior.
