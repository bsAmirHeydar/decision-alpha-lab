...
> [!abstract] Purpose
> Ensure high-level domain policy does not depend on UI, broker, filesystem, or framework details.
...
Ensure high-level domain policy does not depend on UI, broker, filesystem, or framework details. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Dependency Direction** as an explicit engineering contract, not an informal preference.
- Keep stable inward dependencies and adapters traceable to project intent, domain rules, and measurable evidence.
...
- Framework callbacks should translate into domain calls, not contain domain logic.

## 2. Required Procedure

1. Collect the minimum authoritative context required for stable inward dependencies and adapters.
...
- A versioned record of stable inward dependencies and adapters.
