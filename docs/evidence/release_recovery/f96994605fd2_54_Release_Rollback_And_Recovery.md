---
title: Release Rollback And Recovery
status: implemented
version: 1.0.0
phase: V4-06
created: '2026-07-15'
...
Package every changed file with an index, hash ledger, manifest, inventory and QA report.

## Contract

The patch is additive except for intentional pyproject and commit-message updates. Rollback removes indexed new files and restores modified files from the parent commit. A release cannot be reconstructed from filenames alone; exact SHA-256 coverage is mandatory.
...
Reject partial extraction, unindexed files and commits whose staged diff differs from the validated index. The implementation returns an explicit rejected or quarantined artifact where applicable. It never substitutes the nearest primitive, widens a range, drops an unknown field, merges Evidence Roles, reuses a stale hash or falls back to an executable default.
...
- Previous: [[53_Performance_And_Compute_Economics]]
- Phase home: [[00_MOC_V4_06_Treatment_DSL]]
- Next: [[55_Acceptance_Criteria]]
