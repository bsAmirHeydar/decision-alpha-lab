...
> [!abstract] Purpose
> Define properties that no valid transition may violate.
...
Define properties that no valid transition may violate. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Invariants** as an explicit engineering contract, not an informal preference.
- Keep global correctness properties and local guards traceable to project intent, domain rules, and measurable evidence.
...
- Each invariant needs an enforcement point and a test.

## 2. Required Procedure

1. Collect the minimum authoritative context required for global correctness properties and local guards.
...
- A versioned record of global correctness properties and local guards.
...
```text
Design global correctness properties and local guards. Provide entities, state variables, events, guards, transitions, invariants, pseudocode, complexity, failure modes, replay behavior, and tests. Compare alternatives before recommending one. Do not write production code.
```
