...
> [!abstract] Purpose
> Generate and compare multiple valid approaches before selecting an implementation.
...
Generate and compare multiple valid approaches before selecting an implementation. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Algorithm Alternatives** as an explicit engineering contract, not an informal preference.
- Keep trade-offs among correctness, state, complexity, latency, and maintainability traceable to project intent, domain rules, and measurable evidence.
...
- At least one rejected alternative should expose why the selected design is safer.

## 2. Required Procedure

1. Collect the minimum authoritative context required for trade-offs among correctness, state, complexity, latency, and maintainability.
...
- A versioned record of trade-offs among correctness, state, complexity, latency, and maintainability.
...
```text
Design trade-offs among correctness, state, complexity, latency, and maintainability. Provide entities, state variables, events, guards, transitions, invariants, pseudocode, complexity, failure modes, replay behavior, and tests. Compare alternatives before recommending one. Do not write production code.
```
