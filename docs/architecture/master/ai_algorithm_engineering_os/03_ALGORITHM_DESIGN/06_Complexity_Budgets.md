...
> [!abstract] Purpose
> Set time, memory, allocation, I/O, and redraw budgets before implementation.
...
Set time, memory, allocation, I/O, and redraw budgets before implementation. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Complexity Budgets** as an explicit engineering contract, not an informal preference.
- Keep measurable performance constraints traceable to project intent, domain rules, and measurable evidence.
...
- A budget is tied to dataset size, event frequency, and target environment.

## 2. Required Procedure

1. Collect the minimum authoritative context required for measurable performance constraints.
...
- A versioned record of measurable performance constraints.
...
```text
Design measurable performance constraints. Provide entities, state variables, events, guards, transitions, invariants, pseudocode, complexity, failure modes, replay behavior, and tests. Compare alternatives before recommending one. Do not write production code.
```
