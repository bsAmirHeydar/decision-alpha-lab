...
> [!abstract] Purpose
> Record significant choices, alternatives, rationale, evidence, and consequences.
...
Record significant choices, alternatives, rationale, evidence, and consequences. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Decision Log** as an explicit engineering contract, not an informal preference.
- Keep durable decision history independent of chat traceable to project intent, domain rules, and measurable evidence.
...
- A decision without rejected alternatives loses future explanatory power.

## 2. Required Procedure

1. Collect the minimum authoritative context required for durable decision history independent of chat.
...
- A versioned record of durable decision history independent of chat.
...
```text
Act as a domain analyst. For decision log, separate confirmed statements, ambiguous terms, assumptions, counterexamples, constraints, and questions that can change implementation. Do not propose code.
```
