...
> [!abstract] Purpose
> Expose interpretations that could produce different implementations and resolve them explicitly.
...
Expose interpretations that could produce different implementations and resolve them explicitly. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Ambiguity Resolution** as an explicit engineering contract, not an informal preference.
- Keep semantic, temporal, numerical, and lifecycle ambiguity traceable to project intent, domain rules, and measurable evidence.
...
- Questions should target implementation divergence, not generic curiosity.

## 2. Required Procedure

1. Collect the minimum authoritative context required for semantic, temporal, numerical, and lifecycle ambiguity.
...
- A versioned record of semantic, temporal, numerical, and lifecycle ambiguity.
...
```text
Act as a domain analyst. For ambiguity resolution, separate confirmed statements, ambiguous terms, assumptions, counterexamples, constraints, and questions that can change implementation. Do not propose code.
```
