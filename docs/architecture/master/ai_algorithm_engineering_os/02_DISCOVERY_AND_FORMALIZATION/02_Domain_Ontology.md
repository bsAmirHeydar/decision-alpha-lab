...
> [!abstract] Purpose
> Define project-owned entities, relationships, classifications, and forbidden substitutions.
...
Define project-owned entities, relationships, classifications, and forbidden substitutions. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Domain Ontology** as an explicit engineering contract, not an informal preference.
- Keep a stable vocabulary and conceptual graph traceable to project intent, domain rules, and measurable evidence.
...
- Every term has definition, examples, counterexamples, lifecycle, and owner.

## 2. Required Procedure

1. Collect the minimum authoritative context required for a stable vocabulary and conceptual graph.
...
- A versioned record of a stable vocabulary and conceptual graph.
...
```text
Act as a domain analyst. For domain ontology, separate confirmed statements, ambiguous terms, assumptions, counterexamples, constraints, and questions that can change implementation. Do not propose code.
```
