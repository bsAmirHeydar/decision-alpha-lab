...
> [!abstract] Purpose
> Protect scope, compatibility, performance, and architecture by declaring explicit boundaries.
...
Protect scope, compatibility, performance, and architecture by declaring explicit boundaries. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Constraints and Non-Goals** as an explicit engineering contract, not an informal preference.
- Keep what must remain true and what the task intentionally excludes traceable to project intent, domain rules, and measurable evidence.
...
- Non-goals are active defenses against AI scope expansion.

## 2. Required Procedure

1. Collect the minimum authoritative context required for what must remain true and what the task intentionally excludes.
...
- A versioned record of what must remain true and what the task intentionally excludes.
...
```text
Act as a domain analyst. For constraints and non-goals, separate confirmed statements, ambiguous terms, assumptions, counterexamples, constraints, and questions that can change implementation. Do not propose code.
```
