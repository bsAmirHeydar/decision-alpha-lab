...
> [!abstract] Purpose
> Create an implementation-independent, testable behavioral contract.
...
Create an implementation-independent, testable behavioral contract. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Formal Specification** as an explicit engineering contract, not an informal preference.
- Keep inputs, outputs, state, transitions, timing, errors, complexity, and acceptance traceable to project intent, domain rules, and measurable evidence.
...
- Use normative language and identify every temporal reference frame.

## 2. Required Procedure

1. Collect the minimum authoritative context required for inputs, outputs, state, transitions, timing, errors, complexity, and acceptance.
...
- A versioned record of inputs, outputs, state, transitions, timing, errors, complexity, and acceptance.
...
```text
Act as a domain analyst. For formal specification, separate confirmed statements, ambiguous terms, assumptions, counterexamples, constraints, and questions that can change implementation. Do not propose code.
```
