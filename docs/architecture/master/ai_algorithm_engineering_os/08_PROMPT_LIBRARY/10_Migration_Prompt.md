...
> [!abstract] Purpose
> Plan safe schema, state, API, or architecture migration.
...
Plan safe schema, state, API, or architecture migration. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Migration Prompt** as an explicit engineering contract, not an informal preference.
...
```text
Design a migration with current and target contracts, compatibility window, version detection, data/state transformation, partial-failure behavior, rollback, observability, validation, and removal plan. Include old/new reader-writer compatibility and replay equivalence. Split irreversible steps from reversible steps and require backups or reconstruction paths.
```
