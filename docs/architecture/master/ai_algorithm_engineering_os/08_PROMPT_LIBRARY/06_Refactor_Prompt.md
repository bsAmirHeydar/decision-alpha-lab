...
> [!abstract] Purpose
> Improve structure while proving behavior preservation.
...
Improve structure while proving behavior preservation. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Refactor Prompt** as an explicit engineering contract, not an informal preference.
...
```text
Act as a refactoring architect. First characterize current observable behavior and identify missing tests. Define the exact structural problem, target architecture, compatibility constraints, and non-goals. Split the refactor into behavior-preserving stages. For each stage specify files, dependency changes, characterization tests, rollback, and acceptance evidence. Do not mix feature changes with refactoring.
```
