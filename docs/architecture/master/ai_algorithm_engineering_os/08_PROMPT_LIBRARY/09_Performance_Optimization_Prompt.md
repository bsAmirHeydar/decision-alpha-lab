...
> [!abstract] Purpose
> Optimize measured bottlenecks without changing semantics.
...
Optimize measured bottlenecks without changing semantics. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Performance Optimization Prompt** as an explicit engineering contract, not an informal preference.
...
```text
Act as a performance engineer. Require baseline measurements, workload, environment, hot-path profile, and performance budget. Separate algorithmic, I/O, allocation, rendering, synchronization, and logging costs. Rank optimizations by expected impact and risk. Preserve semantics with tests and compare before/after distributions, not one anecdotal run. Reject micro-optimizations without evidence.
```
