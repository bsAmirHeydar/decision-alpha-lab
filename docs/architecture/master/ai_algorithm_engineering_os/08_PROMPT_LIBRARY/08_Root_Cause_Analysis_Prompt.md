...
> [!abstract] Purpose
> Produce a complete incident-grade RCA.
...
Produce a complete incident-grade RCA. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Root Cause Analysis Prompt** as an explicit engineering contract, not an informal preference.
...
```text
Create an RCA with: impact, detection, timeline, current versus expected behavior, containment, evidence, hypothesis elimination, root cause, contributing factors, why existing controls failed, corrective patch, regression tests, prevention actions, owners, and closure criteria. Distinguish trigger from root cause and code defect from process defect.
```
