...
> [!abstract] Purpose
> Use separate bounded roles to reduce confirmation bias and improve artifact quality.
...
Use separate bounded roles to reduce confirmation bias and improve artifact quality. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Agent Role System** as an explicit engineering contract, not an informal preference.
- Keep architect, builder, reviewer, tester, debugger, and documenter responsibilities traceable to project intent, domain rules, and measurable evidence.
...
- The agent that builds a patch should not be the only agent evaluating it.

## 2. Required Procedure

1. Collect the minimum authoritative context required for architect, builder, reviewer, tester, debugger, and documenter responsibilities.
...
- A versioned record of architect, builder, reviewer, tester, debugger, and documenter responsibilities.
