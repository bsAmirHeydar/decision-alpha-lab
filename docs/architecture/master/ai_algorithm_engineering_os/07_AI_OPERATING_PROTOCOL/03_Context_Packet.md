...
> [!abstract] Purpose
> Supply AI with the smallest authoritative set of task-specific information.
...
Supply AI with the smallest authoritative set of task-specific information. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Context Packet** as an explicit engineering contract, not an informal preference.
- Keep context selection, authority order, freshness, and token budget traceable to project intent, domain rules, and measurable evidence.
...
- Place hard rules and acceptance criteria before supporting background.

## 2. Required Procedure

1. Collect the minimum authoritative context required for context selection, authority order, freshness, and token budget.
...
- A versioned record of context selection, authority order, freshness, and token budget.
