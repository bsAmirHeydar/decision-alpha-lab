...
> [!abstract] Purpose
> Change the smallest responsible surface while satisfying the complete approved behavior.
...
Change the smallest responsible surface while satisfying the complete approved behavior. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Minimal Safe Patch** as an explicit engineering contract, not an informal preference.
- Keep scope control, compatibility, reversibility, and proof traceable to project intent, domain rules, and measurable evidence.
...
- Minimal does not mean incomplete; it means no unrelated changes and no missing safety work.

## 2. Required Procedure

1. Collect the minimum authoritative context required for scope control, compatibility, reversibility, and proof.
...
- A versioned record of scope control, compatibility, reversibility, and proof.
...
```text
Implement only the approved scope for minimal safe patch. First inspect the relevant files and contracts. State assumptions, files to modify, files not to modify, preserved invariants, risks, and verification commands. Produce the smallest safe patch; do not rewrite unrelated code.
```
