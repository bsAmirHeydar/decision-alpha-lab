...
> [!abstract] Purpose
> Standardize recurring artifacts and enable structured queries without requiring fragile automation.
...
Standardize recurring artifacts and enable structured queries without requiring fragile automation. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Templates and Dataview** as an explicit engineering contract, not an informal preference.
- Keep template fields, metadata, and optional queries traceable to project intent, domain rules, and measurable evidence.
...
- Templates encode minimum evidence; they should not create ceremonial filler.

## 2. Required Procedure

1. Collect the minimum authoritative context required for template fields, metadata, and optional queries.
...
- A versioned record of template fields, metadata, and optional queries.
