...
> [!abstract] Purpose
> Give notes machine-readable identity, type, status, domain, version, dates, and tags.
...
Give notes machine-readable identity, type, status, domain, version, dates, and tags. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Frontmatter Standard** as an explicit engineering contract, not an informal preference.
- Keep metadata consistency for search, automation, and governance traceable to project intent, domain rules, and measurable evidence.
...
- Stable IDs survive renaming and enable external references.

## 2. Required Procedure

1. Collect the minimum authoritative context required for metadata consistency for search, automation, and governance.
...
- A versioned record of metadata consistency for search, automation, and governance.
