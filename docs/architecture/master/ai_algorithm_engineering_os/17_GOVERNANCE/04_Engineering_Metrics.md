...
> [!abstract] Purpose
> Measure flow, quality, reliability, and knowledge health without rewarding code volume.
...
Measure flow, quality, reliability, and knowledge health without rewarding code volume. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Engineering Metrics** as an explicit engineering contract, not an informal preference.
- Keep leading and lagging indicators traceable to project intent, domain rules, and measurable evidence.
...
- Metrics are diagnostic signals, not targets to game.

## 2. Required Procedure

1. Collect the minimum authoritative context required for leading and lagging indicators.
...
- A versioned record of leading and lagging indicators.
...
Lead time by phase, patch size, review cycle time, blocked time.

### Quality

Escaped defects, regression rate, invariant violations, rollback frequency.

### Verification

Requirement-test traceability, flaky tests, skipped critical tests.

### AI

Acceptance rate after review, hallucination incidents, rework by role/prompt version.

### Knowledge

Broken links, stale normative notes, undocumented releases, orphan decisions.
