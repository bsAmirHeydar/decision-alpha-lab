...
> [!abstract] Purpose
> Specify the mandatory evidence required to close a work item.
...
Specify the mandatory evidence required to close a work item. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Definition of Done** as an explicit engineering contract, not an informal preference.
- Keep verified completion traceable to project intent, domain rules, and measurable evidence.
...
- Done includes behavior, operations, delivery, and knowledge—not only merged code.

## 2. Required Procedure

1. Collect the minimum authoritative context required for verified completion.
...
- A versioned record of verified completion.
...
Acceptance criteria and invariants pass.

### Quality

Build, tests, review, performance, security, and compatibility gates pass as applicable.

### Delivery

Patch/commit is atomic, versioned, and rollback-ready.

### Knowledge

Specifications, architecture, tests, changelog, and decision links are current.

### Ownership

Residual risk and future work have explicit owners and triggers.
