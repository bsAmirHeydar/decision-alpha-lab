...
> [!abstract] Purpose
> Verify that rendered output faithfully represents algorithm state across zoom, scroll, reload, and timeframe changes.
...
Verify that rendered output faithfully represents algorithm state across zoom, scroll, reload, and timeframe changes. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Visual Validation** as an explicit engineering contract, not an informal preference.
- Keep chart/UI correctness as a projection of state traceable to project intent, domain rules, and measurable evidence.
...
- Visual plausibility cannot override state logs or deterministic expected objects.

## 2. Required Procedure

1. Collect the minimum authoritative context required for chart/UI correctness as a projection of state.
...
- A versioned record of chart/UI correctness as a projection of state.
...
```text
Create a verification plan for chart/UI correctness as a projection of state. Include normal, boundary, invalid, duplicate, missing, reordered, restart, replay, scale, and regression cases. For each test specify setup, event sequence, expected state, expected output, failure signal, and evidence.
```
