---
id: AIEOS-C3601863D0
title: "Golden Master and Characterization"
type: guide
status: active
domain: verification
version: 1.0.0
...
tags:
  ...
  - guide
---
# Golden Master and Characterization

> [!abstract] Purpose
> Capture current behavior before risky changes when a complete specification is absent.
...
Capture current behavior before risky changes when a complete specification is absent. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Golden Master and Characterization** as an explicit engineering contract, not an informal preference.
- Keep behavior snapshots, tolerances, and intentional updates traceable to project intent, domain rules, and measurable evidence.
...
- A golden master protects legacy behavior but does not prove that behavior is correct.

## 2. Required Procedure

1. Collect the minimum authoritative context required for behavior snapshots, tolerances, and intentional updates.
...
- A versioned record of behavior snapshots, tolerances, and intentional updates.
...
```text
Create a verification plan for behavior snapshots, tolerances, and intentional updates. Include normal, boundary, invalid, duplicate, missing, reordered, restart, replay, scale, and regression cases. For each test specify setup, event sequence, expected state, expected output, failure signal, and evidence.
```
