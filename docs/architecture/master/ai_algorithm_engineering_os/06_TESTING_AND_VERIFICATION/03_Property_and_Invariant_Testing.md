---
id: AIEOS-ADEA727581
title: "Property and Invariant Testing"
type: guide
status: active
domain: verification
version: 1.0.0
...
> [!abstract] Purpose
> Validate general truths across generated or broad input spaces rather than isolated examples.
...
Validate general truths across generated or broad input spaces rather than isolated examples. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Property and Invariant Testing** as an explicit engineering contract, not an informal preference.
- Keep properties that must hold for all valid sequences traceable to project intent, domain rules, and measurable evidence.
...
- Generate event sequences specifically to attack state invariants.

## 2. Required Procedure

1. Collect the minimum authoritative context required for properties that must hold for all valid sequences.
...
- A versioned record of properties that must hold for all valid sequences.
...
```text
Create a verification plan for properties that must hold for all valid sequences. Include normal, boundary, invalid, duplicate, missing, reordered, restart, replay, scale, and regression cases. For each test specify setup, event sequence, expected state, expected output, failure signal, and evidence.
```
