...
> [!abstract] Purpose
> Verify end-to-end domain narratives across multiple events and components.
...
Verify end-to-end domain narratives across multiple events and components. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Scenario Testing** as an explicit engineering contract, not an informal preference.
- Keep representative sequences, expected state, output, and invalidation traceable to project intent, domain rules, and measurable evidence.
...
- Scenarios include explicit initial state and event order.

## 2. Required Procedure

1. Collect the minimum authoritative context required for representative sequences, expected state, output, and invalidation.
...
- A versioned record of representative sequences, expected state, output, and invalidation.
...
```text
Create a verification plan for representative sequences, expected state, output, and invalidation. Include normal, boundary, invalid, duplicate, missing, reordered, restart, replay, scale, and regression cases. For each test specify setup, event sequence, expected state, expected output, failure signal, and evidence.
```
