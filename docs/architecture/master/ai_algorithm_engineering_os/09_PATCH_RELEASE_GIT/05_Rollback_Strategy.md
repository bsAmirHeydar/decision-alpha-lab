...
> [!abstract] Purpose
> Restore the previous valid state quickly and prove restoration.
...
Restore the previous valid state quickly and prove restoration. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Rollback Strategy** as an explicit engineering contract, not an informal preference.
- Keep revert, backup, migration reversal, and state reconstruction traceable to project intent, domain rules, and measurable evidence.
...
- Rollback is designed before release, especially for persistent state or schema changes.

## 2. Required Procedure

1. Collect the minimum authoritative context required for revert, backup, migration reversal, and state reconstruction.
...
- A versioned record of revert, backup, migration reversal, and state reconstruction.
