...
tags:
  ...
  - guide
---
# Quickstart One Feature

> [!abstract] Purpose
> Provide a compact but complete path for shipping one feature safely.
...
Provide a compact but complete path for shipping one feature safely. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Quickstart One Feature** as an explicit engineering contract, not an informal preference.
- Keep a single feature from request to release traceable to project intent, domain rules, and measurable evidence.
...
1. Collect the minimum authoritative context required for a single feature from request to release.
...
7. Create a feature folder and instantiate the Algorithm Spec, Context Packet, Patch Manifest, Test Matrix, and Release Checklist templates.
8. Run the feature workflow and stop at any failed gate.
9. After release, link the completed packet from the project changelog and relevant module documentation.
...
- A versioned record of a single feature from request to release.
...
```text
We are starting one bounded feature. First create a context packet with current behavior, desired behavior, domain rules, constraints, non-goals, affected files, acceptance tests, and Done criteria. Do not write code until ambiguities and invariants are explicit.
```
