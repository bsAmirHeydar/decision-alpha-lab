...
> [!abstract] Purpose
> Define source of truth, snapshots, cache invalidation, migrations, and recovery.
...
Define source of truth, snapshots, cache invalidation, migrations, and recovery. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Persistence and Caching** as an explicit engineering contract, not an informal preference.
- Keep durable state and derived acceleration traceable to project intent, domain rules, and measurable evidence.
...
- A cache must be disposable or have an explicit consistency protocol.

## 2. Required Procedure

1. Collect the minimum authoritative context required for durable state and derived acceleration.
...
- A versioned record of durable state and derived acceleration.
