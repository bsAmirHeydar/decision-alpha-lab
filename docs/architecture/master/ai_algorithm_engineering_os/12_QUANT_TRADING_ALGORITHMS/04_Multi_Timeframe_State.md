...
> [!abstract] Purpose
> Synchronize independently evolving timeframe states without temporal leakage or accidental index alignment.
...
Synchronize independently evolving timeframe states without temporal leakage or accidental index alignment. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Multi-Timeframe State** as an explicit engineering contract, not an informal preference.
- Keep time mapping, closure rules, aggregation, and dependency traceable to project intent, domain rules, and measurable evidence.
...
- A higher-timeframe value is available only according to its defined completion semantics.

## 2. Required Procedure

1. Collect the minimum authoritative context required for time mapping, closure rules, aggregation, and dependency.
...
- A versioned record of time mapping, closure rules, aggregation, and dependency.
...
```text
Formalize time mapping, closure rules, aggregation, and dependency for a causal trading system. Use only the approved project ontology. Define event time, knowledge time, state lifecycle, invalidation, replay behavior, lookahead safeguards, audit fields, and tests. Do not infer generic trading rules.
```
