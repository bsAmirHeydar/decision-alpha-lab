...
> [!abstract] Purpose
> Diagnose compile, runtime, history, object, trade, permission, and environment failures systematically.
...
Diagnose compile, runtime, history, object, trade, permission, and environment failures systematically. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **MQL5 Error Taxonomy** as an explicit engineering contract, not an informal preference.
- Keep error classes and evidence collection traceable to project intent, domain rules, and measurable evidence.
...
- Compiler cascades often originate from one type/signature error; fix in dependency order.

## 2. Required Procedure

1. Collect the minimum authoritative context required for error classes and evidence collection.
...
- A versioned record of error classes and evidence collection.
...
```text
Review this MQL5 task for error classes and evidence collection. Inspect callback lifecycle, series indexing, history readiness, persistent state, object IDs, cleanup, recalculation, performance, and terminal-specific errors. Produce a minimal patch and a Strategy Tester plus manual chart verification matrix.
```
