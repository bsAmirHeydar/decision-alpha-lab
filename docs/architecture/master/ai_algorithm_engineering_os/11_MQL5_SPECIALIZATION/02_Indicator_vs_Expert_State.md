---
id: AIEOS-66F9D8EF2F
title: "Indicator vs Expert State"
type: guide
status: active
domain: mql5
version: 1.0.0
...
tags:
  ...
  - guide
---
# Indicator vs Expert State

> [!abstract] Purpose
> Model state differently for recalculating indicators and event-driven expert advisors.
...
Model state differently for recalculating indicators and event-driven expert advisors. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Indicator vs Expert State** as an explicit engineering contract, not an informal preference.
- Keep buffer recalculation, trading state, persistence, and side effects traceable to project intent, domain rules, and measurable evidence.
...
- Indicator recalculation must not duplicate persistent or visual effects.

## 2. Required Procedure

1. Collect the minimum authoritative context required for buffer recalculation, trading state, persistence, and side effects.
...
- A versioned record of buffer recalculation, trading state, persistence, and side effects.
