...
> [!abstract] Purpose
> Translate platform callbacks into deterministic internal events and bounded work.
...
Translate platform callbacks into deterministic internal events and bounded work. The objective is not to maximize the amount of generated code. The objective is to reduce ambiguity, preserve domain truth, make changes reversible, and produce evidence that the implementation satisfies the approved intent.

### Core principles

- Treat **Event Callbacks** as an explicit engineering contract, not an informal preference.
- Keep OnInit, OnDeinit, OnCalculate, OnTick, OnTimer, OnChartEvent traceable to project intent, domain rules, and measurable evidence.
...
- Each callback documents reentrancy, frequency, and allowed side effects.

## 2. Required Procedure

1. Collect the minimum authoritative context required for OnInit, OnDeinit, OnCalculate, OnTick, OnTimer, OnChartEvent.
...
- A versioned record of OnInit, OnDeinit, OnCalculate, OnTick, OnTimer, OnChartEvent.
