...
This contract defines the finite vocabulary from which Setup expressions and actions are composed. It is part of ACL-04 and therefore operates only after an exact `ACL03_TO_ACL04` handoff has been validated.

## Ownership and authority boundary

Unknown IDs and duplicate IDs fail. Future-derived atoms are restricted to diagnostic baselines and may not enter eligible candidate behavior.
...
registered atom/action definitions with arity, lane permissions and known-time classification. Every material input carries an exact identity, semantic version and SHA-256 digest. Missing or ambiguous identities are blockers, not warnings.

## Produced artifacts

content-addressed registry and compatibility evidence. Structured JSON is authoritative. Obsidian notes are generated or reviewed projections and cannot mutate behavior.
