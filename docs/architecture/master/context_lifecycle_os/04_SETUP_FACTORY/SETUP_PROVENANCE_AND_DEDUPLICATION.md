---
title: Setup Provenance and Deduplication
status: accepted-reference
version: 1.0.0
updated: 2026-07-18
tags: [acl-os, acl-04, setup-factory]
---
# Setup Provenance and Deduplication

## Purpose

This contract preserves source lineage while preventing equivalent behavior from masquerading as independent discoveries. It is part of ACL-04 and therefore operates only after an exact `ACL03_TO_ACL04` handoff has been validated.

## Ownership and authority boundary

Human preference in representative selection is deterministic governance, not evidence preference. Duplicate origins remain visible but count as one behavior hypothesis.
...
all source candidates and behavior digests. Every material input carries an exact identity, semantic version and SHA-256 digest. Missing or ambiguous identities are blockers, not warnings.

## Produced artifacts

deduplication groups, canonical representative and provenance graph. Structured JSON is authoritative. Obsidian notes are generated or reviewed projections and cannot mutate behavior.
