---
title: Setup Canonicalization and Hashing
status: accepted-reference
version: 1.0.0
updated: 2026-07-18
tags: [acl-os, acl-04, setup-factory]
---
# Setup Canonicalization and Hashing

## Purpose

This contract creates stable behavior identity independent of authoring order and origin. It is part of ACL-04 and therefore operates only after an exact `ACL03_TO_ACL04` handoff has been validated.

## Ownership and authority boundary

Mappings are key-sorted, commutative expression children are normalized, non-finite numbers are rejected, and SHA-256 is used. Origin and provenance never alter behavior identity.
...
validated Policy IR. Every material input carries an exact identity, semantic version and SHA-256 digest. Missing or ambiguous identities are blockers, not warnings.

## Produced artifacts

canonical bytes, policy digest, behavior digest, candidate ID and Setup ID. Structured JSON is authoritative. Obsidian notes are generated or reviewed projections and cannot mutate behavior.
