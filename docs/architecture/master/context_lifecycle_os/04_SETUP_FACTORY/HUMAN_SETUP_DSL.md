...
This contract converts an approved trader idea into a closed, reviewable Setup definition. It is part of ACL-04 and therefore operates only after an exact `ACL03_TO_ACL04` handoff has been validated.

## Ownership and authority boundary

The DSL is declarative JSON/YAML. Dynamic evaluation, arbitrary code, undeclared atoms, implicit defaults and hidden execution behavior are forbidden.
...
Context identity/version, treatment family, parameters, risk contract and typed entry/invalidation/exit/management/expiry/abstention clauses. Every material input carries an exact identity, semantic version and SHA-256 digest. Missing or ambiguous identities are blockers, not warnings.

## Produced artifacts

Setup Policy IR with stable clause order and digest. Structured JSON is authoritative. Obsidian notes are generated or reviewed projections and cannot mutate behavior.
