...
`Root Relative Path` is an atomic LCM-05 concept. Its meaning is fixed by the closed target-topology registries and the topology package. It cannot be used to infer owner approval, semantic equivalence, behavioral parity, path materialization, cutover or execution authority.
