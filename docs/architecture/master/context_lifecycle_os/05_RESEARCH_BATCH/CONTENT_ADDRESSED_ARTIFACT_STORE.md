...
Stores exact immutable bytes at a path derived from SHA-256.

## Contract model

The address is `objects/sha256/<first2>/<remaining62>.blob`; overwrite, symlinks and digest collisions fail closed. Semantic digest and byte digest remain distinct.

## Failure semantics

Filename identity, mutable object keys, silent replacement and hash-only records without size/path evidence are rejected.
