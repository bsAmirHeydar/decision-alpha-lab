...
The synchronizer validates config/calendar hashes, normalizes bars, builds the expected axis, emits cells, calculates coverage/gaps/conflicts, classifies health, and creates one semantic result hash.

## Normative invariants

1. **Input order cannot affect output.**
2. **Duplicate conflicts block the result.**
3. **Gaps degrade but do not erase aligned evidence.**
4. **Every result owns an exact DataRevision.**
