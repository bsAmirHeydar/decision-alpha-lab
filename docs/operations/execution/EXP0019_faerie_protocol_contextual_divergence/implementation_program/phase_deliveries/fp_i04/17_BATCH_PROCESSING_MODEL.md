...
Batch synchronization accepts a complete bar set for a half-open range and emits the canonical row sequence and revision root.

## Normative invariants

1. **Batch output is the differential oracle.**
2. **No full-history loop is hidden inside a per-tick entry point.**
3. **Range boundaries must be M1 aligned.**
4. **Semantic output does not depend on ingestion order.**
