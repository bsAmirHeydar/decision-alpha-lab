---
title: "FP-I02 — Symbol Pair Contract and Canonical Ordering"
...
language: en
---
# Symbol Pair Contract and Canonical Ordering

## Purpose

SymbolPair stores operator-facing primary/secondary order while pair_id uses sorted canonical symbols. This prevents one economic pair from receiving different IDs merely because the host chart or input order is reversed.

## Normative invariants

1. **Symbols must be distinct and non-empty.**
2. **pair_id is order invariant.**
3. **Role assignment remains event-specific and is not encoded by pair input order.**
4. **Counterpart lookup rejects symbols outside the pair.**

## Primary surfaces

- `Primary/secondary display order`
- `Canonical sorted identity order`
- `Pair version`
- `pair_id`
