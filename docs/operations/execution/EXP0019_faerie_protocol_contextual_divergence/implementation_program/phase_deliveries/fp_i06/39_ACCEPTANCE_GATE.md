...
Acceptance requires all six non-WW relations in both directions, no invented same-M1 order, no duplicate candidate IDs, missing-data blocking, bounded invalidation, and upstream regression success.

## Boundary

MetaEditor compile is reported as pending until real Windows evidence exists.
