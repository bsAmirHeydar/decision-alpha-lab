...
A HuntFact is a positive, immutable contact observation for one symbol, one reference, one side, one M1 bar, and one source revision.

## Boundary

HIGH contact requires bar.high >= own high reference. LOW contact requires bar.low <= own low reference.
