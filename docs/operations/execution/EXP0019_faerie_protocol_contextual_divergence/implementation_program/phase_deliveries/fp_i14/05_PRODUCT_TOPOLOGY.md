...
This note governs **Product Topology**. Acceptance requires deterministic behavior across all three products and a reason-coded report for every negative branch. The operational owner must be able to reproduce the result from the trace and manifest alone, without inspecting mutable chart state.
