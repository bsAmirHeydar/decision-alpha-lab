---
title: "FP-I00 — Baseline Manifest Model and Identity"
...
language: en
---
# Baseline Manifest Model and Identity

## Purpose

The baseline manifest is the machine-readable root of trust for the Faerie Protocol implementation program.

## Normative scope

- Record experiment, context, phase, program, source, decision, relation, documentation, dependency, test, ownership, authority, and source-control evidence.
- Canonicalize the complete material before assigning `manifest_hash`.
- Include every behavior-bearing upstream contract that can alter later implementation semantics.

## Non-bypassable invariants

1. **Identical repository evidence produces an identical manifest.**
2. **A changed decision, relation, dependency hash, program version, or authority field changes the manifest hash.**
3. **The manifest never claims Git evidence that was unavailable in an archive build.**
