---
title: "FP-I00 — Reuse Classification and No-Fork Policy"
...
language: en
---
# Reuse Classification and No-Fork Policy

## Purpose

Dependency evidence separates direct reuse, adapter reuse, pattern reuse, and future-only consumption.

## Normative scope

- Use `DIRECT_REUSE` only when the public contract and semantics already match.
- Use `ADAPTER_REUSE` when the generic primitive is sound but FP input/output or deadline policy differs.
- Use `PATTERN_REUSE` when architecture is useful but FP semantics require an isolated implementation.

## Non-bypassable invariants

1. **No copied DST, hashing, risk, ledger, or synchronization logic is accepted without documented justification.**
2. **A generic extraction requires at least two context consumers and old-context regression evidence.**
3. **Adapters translate contracts and policy; they do not mutate shared state.**
