---
title: "FP-I00 — Operator Runbook and Local Acceptance"
...
language: en
---
# Operator Runbook and Local Acceptance

## Purpose

The operator applies the patch, regenerates local Git-aware evidence, runs phase tests, optionally verifies the original source package, and reviews unrelated changes.

## Normative scope

- Run the PowerShell phase check from repository root.
- Inspect baseline source-control evidence and validation report.
- Confirm that only phase-owned files are staged.

## Non-bypassable invariants

1. **Do not commit an unreviewed unrelated change.**
2. **Do not claim original-source verification unless SourceRoot was supplied.**
3. **Do not start FP-I01 while the report is BLOCKED.**
