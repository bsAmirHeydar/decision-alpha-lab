...
FP-I13 starts only from an accepted FP-I12 file/hash manifest. Exact I03–I12 composition versions are required. Indicator source, visual renderer, operator UX, checkpoint, alert, and export contracts must be present. Any unrelated working-tree change is excluded from the patch.

The local MetaEditor gate is not assumed. Its absence is represented as `PENDING_LOCAL_WINDOWS`, not as a fabricated pass.
## Implementation linkage

The **03 Entry Criteria** contract is implemented through the versioned `fp_i13_release` Python reference package, the MQL5 `I13` include boundary, the production indicator integration, and the FP-I13 release self-test. The implementation must consume accepted FP-I10–FP-I12 outputs rather than reconstructing context from chart objects.
