...
FP-I13 is a release-proof phase, not a new signal phase. Behavior-bearing fields inherited from upstream are frozen. The replay reducer observes accepted semantic and projection events; it does not recompute relations from prices.

### Permitted changes

- performance instrumentation;
- replay and parity harnesses;
- release configuration and profiles;
- noncritical projection scheduling;
- diagnostics and acceptance evidence;
- local compile and runtime runbooks.

### Prohibited changes

- new relation rules;
- alternative Hunter/Protected assignment;
- confirmation changes;
- WW newest-wins changes;
- quota winner changes;
- signal identity changes;
- hidden deletion of suppressed evidence;
- order APIs or live consumption.
## Implementation linkage

The **02 Release Boundary** contract is implemented through the versioned `fp_i13_release` Python reference package, the MQL5 `I13` include boundary, the production indicator integration, and the FP-I13 release self-test. The implementation must consume accepted FP-I10–FP-I12 outputs rather than reconstructing context from chart objects.
